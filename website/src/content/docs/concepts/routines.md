---
title: Routine calls
description: Author stored-procedure calls with explicit output, return, and heterogeneous result-set contracts.
---
...
| --- | --- |
| PostgreSQL / `pg` | Scalar OUT/INOUT values come from the `CALL` output row. Mark `refcursor` OUT/INOUT with `postgresParameter.refcursor()`; SQLBraid fetches and closes each transaction-bound portal and removes it from `output`. A refcursor call requires an existing `db.tx(...)` scope; SQLBraid never creates a hidden transaction. |
...
PV15 exposes materialized `db.call()` only. Use `db.stream(sql.rows(...))` for ordinary row-producing queries and set-returning functions. A future routine stream must solve multi-cursor ownership and transaction lifetime; SQLBraid does not pretend that a routine's cursor result sets are ordinary independent streams today.

See [SQL tags and result kinds](/SQLBraid/concepts/sql-tags/), [streaming](/SQLBraid/runtime/streaming/), and [diagnostics](/SQLBraid/reference/errors/).
