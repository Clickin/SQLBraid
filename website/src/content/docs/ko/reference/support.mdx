...
현재 지원 매트릭스는 revision별입니다. phase-J SPI 변경의 영향을 받은
target은 하나의 정확한 최종 SHA에서 새로운 Runtime, Documentation, Release
실행 gate가 통과할 때까지 **Pending** 또는 **Compatible**입니다. 인접한
서버 버전, 런타임, 프로필, 로컬 binding 또는 과거 workflow에서 새 Official
label을 추론하지 마세요.

provenance로 보존하는 마지막 exact-SHA 검증은 revision
`8da8167e027320fcc9bb2aac16b0903c64147940`입니다.
...
세 run은 해당 revision에서 성공했습니다. 이는 역사적 증거일 뿐 현재
tree를 인증하지 않으며 발행을 승인하지 않습니다.
...
역사 link와 prose만으로 label을 승격할 수 없습니다. 이 페이지는 npm/tag/Pages
발행을 의미하지 않습니다.
