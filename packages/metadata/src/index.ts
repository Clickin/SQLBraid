...
function scanSnapshotValues(value: unknown, path: string, diagnostics: SnapshotDiagnostic[]): void {
  ...
  if (Array.isArray(value)) { value.forEach((entry, index) => scanSnapshotValues(entry, `${path}[${index}]`, diagnostics)); return; }
  if (isRecord(value)) for (const [key, entry] of Object.entries(value)) scanSnapshotValues(entry, path ? `${path}.${key}` : key, diagnostics);
}

function validateColumn(value: unknown, path: string, diagnostics: SnapshotDiagnostic[]): void {
  if (!isRecord(value)) { add(diagnostics, "SNAPSHOT_COLUMN", "Column must be an object.", path); return; }
  if (typeof value.name !== "string" || !value.name) add(diagnostics, "SNAPSHOT_COLUMN_NAME", "Column name must be non-empty.", `${path}.name`);
...
function validateRelation(value: unknown, path: string, diagnostics: SnapshotDiagnostic[]): void {
  ...
  value.columns.forEach((column, index) => {
    ...
    if (typeof column.name === "string" && names.has(column.name)) add(diagnostics, "SNAPSHOT_DUPLICATE_COLUMN", `Duplicate column: ${column.name}.`, `${path}.columns[${index}].name`);
    if (typeof column.name === "string") names.add(column.name);
    if (isFiniteNumber(column.ordinal) && ordinals.has(column.ordinal)) add(diagnostics, "SNAPSHOT_DUPLICATE_ORDINAL", `Duplicate column ordinal: ${column.ordinal}.`, `${path}.columns[${index}].ordinal`);
...
function normalizeSnapshot(snapshot: MetadataSnapshot, includeVolatile: boolean): MetadataSnapshot {
  ...
  for (const [key, relation] of Object.entries(snapshot.relations).sort(([left], [right]) => compareKeys(left, right))) {
    relations[key] = {
      ...relation,
      columns: [...relation.columns].sort((left, right) => left.ordinal - right.ordinal || compareKeys(left.name, right.name)),
      constraints: relation.constraints ? [...relation.constraints].sort((left, right) => compareKeys(left.name, right.name)) : undefined,
...
function canonicalValue(value: unknown): string {
  ...
  if (isRecord(value)) return `{${Object.entries(value).filter(([, entry]) => entry !== undefined).sort(([left], [right]) => compareKeys(left, right)).map(([key, entry]) => `${JSON.stringify(key)}:${canonicalValue(entry)}`).join(",")}}`;
...
export function validateSnapshot(snapshot: unknown): asserts snapshot is MetadataSnapshot {
  ...
  if (isRecord(snapshot.routines)) for (const [key, value] of Object.entries(snapshot.routines)) {
    if (!Array.isArray(value)) { add(diagnostics, "SNAPSHOT_ROUTINES", "Routine entry must be an array.", `routines.${key}`); continue; }
...
function collectDrift(before: unknown, after: unknown, path: string, output: SnapshotDrift[]): void {
  ...
  if (isRecord(before) && isRecord(after)) {
    ...
    for (const key of [...keys].sort(compareKeys)) collectDrift(before[key], after[key], path ? `${path}.${key}` : key, output);
    return;
  }
...
export function diffSnapshots(before: MetadataSnapshot, after: MetadataSnapshot): readonly SnapshotDrift[] {
  ...
  collectDrift(JSON.parse(canonicalValue(normalizeSnapshot(before, false))), JSON.parse(canonicalValue(normalizeSnapshot(after, false))), "", output);
  return output;
}
