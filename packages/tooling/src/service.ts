...
type RelationUse = { readonly tokens: readonly LexToken[]; readonly text: string; readonly relation?: RelationSnapshot; readonly cte: boolean; readonly alias?: string };
type RoutineUse = { readonly token: LexToken; readonly routine?: RoutineSnapshot; readonly next: LexToken };
type ColumnUse = { readonly owner: RelationSnapshot; readonly column: ColumnSnapshot; readonly token: LexToken; readonly qualified: boolean; readonly relationToken: LexToken };
...
function unique<T>(values: readonly T[], key: (value: T) => string): T[] {
  ...
  const result: T[] = [];
  for (const value of values) { const identity = key(value); if (!seen.has(identity)) { seen.add(identity); result.push(value); } }
  return result;
}
function bounded<T>(values: readonly T[], maxEntries: number): readonly T[] { return values.slice(0, Math.max(0, maxEntries)); }
...
function location(uriPath: string, sourceText: string, range: Range): Location {
  return { uri: uriPath.startsWith("file:") ? uriPath : pathToFileURL(uriPath).href, range: { start: sourcePosition(sourceText, range.start), end: sourcePosition(sourceText, range.end) } };
}
...
function buildLexicalText(query: DiscoveredQuery, sourceText: string): { text: string; map: number[]; code: boolean[]; quoted: boolean[]; staticRanges: Range[]; mappingReliable: boolean } {
  ...
  for (let index = 0; index < query.strings.length; index += 1) {
    ...
    if (index < query.strings.length - 1) { text.push(" "); map.push(-1); code.push(true); quoted.push(false); }
  }
...
function markLexicalProtection(text: string, code: boolean[], quoted: boolean[], profile: { readonly lineCommentPrefixes: readonly string[]; readonly supportsNestedBlockComments?: boolean; readonly supportsDollarQuotes?: boolean; readonly supportsBacktickIdentifiers?: boolean; readonly supportsBracketIdentifiers?: boolean; readonly supportsOracleQQuotes?: boolean; readonly backslashEscapes?: boolean; readonly requireDashDashWhitespace?: boolean }, mysql: boolean): void {
  ...
  let depth = 0;
  let dollar = "";
  const setProtected = (start: number, end: number): void => { for (let position = start; position < end; position += 1) code[position] = false; };
  while (index < text.length) {
    const current = text[index];
    const next = text[index + 1];
    if (state === "line") { setProtected(index, index + 1); if (current === "\n" || current === "\r") state = "code"; index += 1; continue; }
    if (state === "block") {
      setProtected(index, index + 1);
      if (profile.supportsNestedBlockComments !== false && current === "/" && next === "*") { setProtected(index + 1, index + 2); depth += 1; index += 2; continue; }
      if (current === "*" && next === "/") { setProtected(index + 1, index + 2); depth -= 1; index += 2; if (depth <= 0) state = "code"; continue; }
      index += 1; continue;
    }
    if (state === "single") {
      setProtected(index, index + 1);
      if (current === "\\" && profile.backslashEscapes !== false) { setProtected(index + 1, index + 2); index += 2; continue; }
      if (current === "'" && next === "'") { setProtected(index + 1, index + 2); index += 2; continue; }
    ...
    if (current === '"' || current === "`" && profile.supportsBacktickIdentifiers) {
      ...
      while (index < text.length) {
        setProtected(index, index + 1);
        if (text[index] === quote && text[index + 1] === quote) { setProtected(index + 1, index + 2); index += 2; continue; }
    ...
    if (profile.supportsDollarQuotes !== false && current === "$") {
      ...
      if (marker) { setProtected(index, index + marker.length); state = "dollar"; dollar = marker; index += marker.length; continue; }
    }
    index += 1;
  }
}
...
function scanTokens(text: string, map: readonly number[], code: readonly boolean[], quoted: readonly boolean[]): readonly LexToken[] {
  ...
  while (index < text.length) {
    const start = index;
    if (!code[index]) {
      if (!quoted[index]) { index += 1; continue; }
      const quote = text[index];
      if (quote !== '"' && quote !== '`' && quote !== "[") { index += 1; continue; }
      const endQuote = quote === "[" ? "]" : quote;
      index += 1;
      while (index < text.length) {
        if (text[index] === endQuote && text[index + 1] === endQuote) { index += 2; continue; }
        if (text[index] === endQuote) { index += 1; break; }
        index += 1;
      }
      const range = mapSource(map, start, index, 0);
      tokens.push({ kind: "identifier", text: text.slice(start, index), start, end: index, sourceStart: range.start, sourceEnd: range.end });
      continue;
    }
    ...
    if (!/\s/u.test(text[index] ?? "")) {
      index += 1;
      const range = mapSource(map, start, index, 0);
      tokens.push({ kind: "punctuation", text: text.slice(start, index), start, end: index, sourceStart: range.start, sourceEnd: range.end });
      continue;
    }
    index += 1;
  }
...
function allRelations(options: LanguageServiceOptions): readonly RelationSnapshot[] {
  ...
  for (const relation of metadataEvidence(options).flatMap((item) => Object.values(item.snapshot.relations))) groups.set(relation.identity, [...(groups.get(relation.identity) ?? []), relation]);
  return [...groups.values()].filter((group) => new Set(group.map(evidenceKey)).size === 1).map((group) => group[0]).filter((relation): relation is RelationSnapshot => relation !== undefined);
}
function allRoutines(options: LanguageServiceOptions): readonly RoutineSnapshot[] {
  const groups = new Map<string, RoutineSnapshot[]>();
  for (const routine of metadataEvidence(options).flatMap((item) => Object.values(item.snapshot.routines).flat())) groups.set(routine.identity, [...(groups.get(routine.identity) ?? []), routine]);
...
function cteNames(tokens: readonly LexToken[]): ReadonlySet<string> {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token.text === "(") { parentheses += 1; continue; }
    if (token.text === ")") { parentheses = Math.max(0, parentheses - 1); continue; }
    if (token.kind !== "identifier") continue;
    if (token.text.toLowerCase() === "with") { withSeen = true; continue; }
    if (!withSeen) continue;
    if (parentheses > 0) continue;
    if (token.text.toLowerCase() === "select" || token.text.toLowerCase() === "insert" || token.text.toLowerCase() === "update" || token.text.toLowerCase() === "delete") break;
    const next = tokens[index + 1];
    const previous = tokens[index - 1];
    if ((next?.text.toLowerCase() === "as" || next?.text === "(") && (!previous || previous.text === "," || previous.text.toLowerCase() === "with" || previous.text.toLowerCase() === "recursive")) names.add(identifierKey(token.text));
  }
  return names;
}
function queryRelations(tokens: readonly LexToken[], options: LanguageServiceOptions, fold: boolean): readonly RelationUse[] {
  const ctes = cteNames(tokens); const uses: RelationUse[] = [];
  for (let index = 0; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token.kind !== "identifier" || !["from", "join", "update", "into"].includes(token.text.toLowerCase())) continue;
    const parts: LexToken[] = []; let cursor = index + 1;
    if (tokens[cursor]?.kind !== "identifier") continue;
    parts.push(tokens[cursor++]);
    while (tokens[cursor]?.text === "." && tokens[cursor + 1]?.kind === "identifier") { parts.push(tokens[cursor + 1]); cursor += 2; }
    ...
    if (tokens[cursor]?.text.toLowerCase() === "as" && tokens[cursor + 1]?.kind === "identifier") alias = tokens[cursor + 1]?.text;
    else if (tokens[cursor]?.kind === "identifier" && !SQL_KEYWORDS.has(tokens[cursor].text.toLowerCase())) alias = tokens[cursor]?.text;
...
function queryRoutineUses(tokens: readonly LexToken[], options: LanguageServiceOptions, fold: boolean): readonly RoutineUse[] {
...
function queryColumns(tokens: readonly LexToken[], relations: readonly RelationUse[], options: LanguageServiceOptions, fold: boolean): readonly ColumnUse[] {
  const result: ColumnUse[] = [];
  for (let index = 0; index + 2 < tokens.length; index += 1) {
    const qualifier = tokens[index]; const dot = tokens[index + 1]; const columnToken = tokens[index + 2];
    if (qualifier.kind !== "identifier" || dot.text !== "." || columnToken.kind !== "identifier") continue;
    const owner = relationForQualifier(qualifier.text, relations, options); if (!owner) continue;
  ...
  const known = unique(relations.map((use) => use.relation).filter((relation): relation is RelationSnapshot => relation !== undefined), (relation) => relation.identity);
  ...
  if (known.length === 1 && !hasUnknownRelation && !hasCte && !tokens.some((token) => token.text === ",")) {
    for (let index = 1; index + 1 < tokens.length; index += 1) {
      const token = tokens[index]!;
      if (tokens[index - 1]?.text.toLowerCase() !== "select" || tokens[index + 1]?.text.toLowerCase() !== "from") continue;
      if (token.kind !== "identifier" || SQL_KEYWORDS.has(token.text.toLowerCase())) continue;
      if (result.some((item) => item.token.start === token.start) || relations.some((use) => use.tokens.some((part) => part.start === token.start))) continue;
      const matches = known[0].columns.filter((column) => column.name === identifierKey(token.text, fold));
      if (matches.length === 1) result.push({ owner: known[0], column: matches[0], token, qualified: false, relationToken: token });
    }
  }
  return result;
}
...
function offsetInStatic(lexical: LexicalQuery, offset: number): boolean {
  ...
  return lexical.staticRanges.some((range, index) => offset >= range.start && (offset < range.end || offset === range.end && index === lexical.staticRanges.length - 1)) && (() => {
...
function tokenAt(lexical: LexicalQuery, offset: number): LexToken | undefined {
  return lexical.tokens.find((token) => offset >= token.sourceStart && offset < token.sourceEnd)
...
function queryAt(analysis: FileAnalysis, offset: number, staticOnly = false): LexicalQuery | undefined {
  return analysis.queries.find((query) => offset >= query.query.range.start && offset <= query.query.range.end && (!staticOnly || offsetInStatic(query, offset)));
}
function completionPrefix(sourceText: string, offset: number): { readonly prefix: string; readonly qualified: boolean } {
...
function targetForRelation(relation: RelationSnapshot, options: LanguageServiceOptions): { readonly target: ToolingTarget; readonly model: NonNullable<CodegenResult["models"]>[number] } | undefined {
...
function generatedIndexes(options: LanguageServiceOptions): readonly GeneratedIndex[] {
  ...
  for (const target of options.targets ?? []) {
    if (!target.outFile || !target.generatedSource || !target.generation || target.generatedSource !== target.generation.source) continue;
    const file = ts.createSourceFile(target.outFile, target.generatedSource, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    const declarations = new Map<string, Range>(); const properties = new Map<string, Range>();
    function visit(node: ts.Node): void {
      if (ts.isInterfaceDeclaration(node) && node.name) {
        declarations.set(node.name.text, sourceRange(node.name.getStart(file), node.name.getEnd()));
        for (const member of node.members) if (ts.isPropertySignature(member) && member.name) {
          const name = ts.isStringLiteral(member.name) || ts.isIdentifier(member.name) ? member.name.text : undefined;
          if (name) properties.set(`${node.name.text}\0${name}`, sourceRange(member.name.getStart(file), member.name.getEnd()));
        }
      }
      ts.forEachChild(node, visit);
    }
    visit(file); indexes.push({ target, declarations, properties });
  }
  return indexes;
}
function metadataObjectProperty(object: ts.ObjectLiteralExpression, key: string, file: ts.SourceFile): ts.PropertyAssignment | undefined {
  return object.properties.find((property): property is ts.PropertyAssignment => ts.isPropertyAssignment(property) && (ts.isStringLiteral(property.name) ? property.name.text : property.name.getText(file)) === key);
}
...
function metadataIndex(target: ToolingTarget): MetadataIndex | undefined {
  ...
  const root = file.statements[0] && ts.isExpressionStatement(file.statements[0]) && ts.isObjectLiteralExpression(file.statements[0].expression) ? file.statements[0].expression : undefined;
  ...
  if (relationContainer && ts.isObjectLiteralExpression(relationContainer)) for (const property of relationContainer.properties) {
    if (!ts.isPropertyAssignment(property) || !ts.isObjectLiteralExpression(property.initializer)) continue;
    const identity = jsonObjectIdentity(property.initializer, file); if (!identity) continue;
    ...
    if (columnContainer && ts.isArrayLiteralExpression(columnContainer)) for (const item of columnContainer.elements) if (ts.isObjectLiteralExpression(item)) {
    ...
    relations.set(identity, { range: sourceRange(property.initializer.getStart(file), property.initializer.getEnd()), columns });
  }
  const routines = new Map<string, Range>();
  const routineContainer = metadataObjectProperty(root, "routines", file)?.initializer;
  if (routineContainer && ts.isObjectLiteralExpression(routineContainer)) for (const property of routineContainer.properties) if (ts.isPropertyAssignment(property) && ts.isArrayLiteralExpression(property.initializer)) for (const item of property.initializer.elements) if (ts.isObjectLiteralExpression(item)) {
    const identity = jsonObjectIdentity(item, file); if (identity) routines.set(identity, sourceRange(item.getStart(file), item.getEnd()));
  }
  return { target, relations, routines };
}
function metadataRange(indexes: readonly MetadataIndex[], target: ToolingTarget, identity: string, column?: string, routine = false): Location | undefined {
  const index = indexes.find((candidate) => candidate.target === target); if (!index || !target.metadataPath || !target.metadataSource) return undefined;
...
function generatedLocation(relation: RelationSnapshot, column: string | undefined, options: LanguageServiceOptions, indexes: readonly GeneratedIndex[]): Location | undefined {
  const linked = targetForRelation(relation, options); if (!linked) return undefined;
  ...
  const current = candidates[0]; if (!current) return undefined;
  const index = current.index;
  const name = column ? linked.model.rowName : linked.model.rowName; const range = column ? index.properties.get(`${name}\0${column}`) : index.declarations.get(name);
  return range && current.target.outFile ? location(current.target.outFile, current.target.generatedSource ?? "", range) : undefined;
}
function targetMetadataLocation(relation: RelationSnapshot, column: string | undefined, options: LanguageServiceOptions, indexes: readonly MetadataIndex[]): Location | undefined {
  for (const evidence of metadataEvidence(options)) if (evidence.target) { const found = metadataRange(indexes, evidence.target, relation.identity, column); if (found) return found; }
  return undefined;
}
function relationHover(relation: RelationSnapshot, range: Range, options: LanguageServiceOptions): HoverResult {
  ...
  const evidence = metadataEvidence(options).find((item) => Object.values(item.snapshot.relations).some((candidate) => candidate.identity === relation.identity));
  ...
  if (model) lines.push(`models: ${model.rowName}${model.insertName ? `, ${model.insertName}` : ""}${model.updateName ? `, ${model.updateName}` : ""}`);
  if (linked?.target.generation) lines.push(`metadataHash: ${linked.target.generation.metadataHash}`, `optionsHash: ${linked.target.generation.optionsHash}`);
  return { contents: boundedText(lines), range };
}
function columnHover(owner: RelationSnapshot, column: ColumnSnapshot, range: Range, options: LanguageServiceOptions, indexes: readonly GeneratedIndex[]): HoverResult {
  ...
  const evidence = metadataEvidence(options).find((item) => Object.values(item.snapshot.relations).some((candidate) => candidate.identity === owner.identity));
  if (evidence?.snapshot.metadata.introspectionScope) lines.push(`introspectionScope: ${evidence.snapshot.metadata.introspectionScope}`);
  const linked = targetForRelation(owner, options);
  if (linked?.target.generation) lines.push(`metadataHash: ${linked.target.generation.metadataHash}`, `optionsHash: ${linked.target.generation.optionsHash}`);
  if (linked) lines.push(`generated property: ${linked.model.rowName}.${column.name}`);
  const model = generatedLocation(owner, column.name, options, indexes); if (model) lines.push(`generated: ${model.uri}`);
  return { contents: boundedText(lines), range };
}
function routineHover(routine: RoutineSnapshot, range: Range, options: LanguageServiceOptions): HoverResult {
  ...
  const lines = [`Routine ${routine.identity}`, `kind: ${routine.kind}`, `result: ${result}`, `known arguments: ${knownArguments}`, `argumentsComplete: ${complete}`];
  const evidence = metadataEvidence(options).find((item) => Object.values(item.snapshot.routines).flat().some((candidate) => candidate.identity === routine.identity));
  if (evidence?.target?.generation) lines.push(`metadataHash: ${evidence.target.generation.metadataHash}`, `optionsHash: ${evidence.target.generation.optionsHash}`);
  return { contents: boundedText(lines), range };
}
function queryHover(lexical: LexicalQuery, options: LanguageServiceOptions): HoverResult {
  ...
  if (target?.generation) lines.push(`metadataHash: ${target.generation.metadataHash}`, `optionsHash: ${target.generation.optionsHash}`);
  return { contents: boundedText(lines), range: lexical.query.range };
}
function relationCandidates(options: LanguageServiceOptions): CompletionItem[] { return allRelations(options).map((relation) => ({ label: relation.name, kind: "relation", detail: relation.kind })); }
function columnCandidates(owner: RelationSnapshot): CompletionItem[] { return owner.columns.map((column) => ({ label: column.name, kind: "column", detail: column.type })); }
function routineCandidates(options: LanguageServiceOptions): CompletionItem[] { return allRoutines(options).map((routine) => ({ label: routine.name, kind: "routine", detail: routine.kind })); }
function routineSignature(routine: RoutineSnapshot, activeParameter: number): SignatureResult {
  ...
  return { label: `${routine.identity}(${parameters.join(", ")})`, parameters, activeParameter: Math.max(0, Math.min(activeParameter, Math.max(0, parameters.length - 1))) };
}
function querySymbols(analysis: FileAnalysis): readonly QuerySymbol[] { return analysis.queries.map((lexical) => { const kind = lexical.query.declaredResultKind; const contract = lexical.query.declaredRowType; return { name: lexical.query.tagName, detail: `${kind}${contract ? `<${contract}>` : ""}`, range: lexical.query.range, selectionRange: lexical.query.templateRange }; }); }
...
export function createLanguageService(options: LanguageServiceOptions): SqlBraidLanguageService {
  ...
  function generated(): readonly GeneratedIndex[] {
    ...
    const cached = indexCache.get(key); if (cached) return cached;
    const value = generatedIndexes(options); cacheSet(indexCache, key, value, maxEntries); return value;
  }
  function metadata(): readonly MetadataIndex[] {
    const key = (options.targets ?? []).map((target) => `${target.metadataPath ?? ""}\0${target.metadataSource ?? ""}`).join("\u0001");
    const cached = metadataIndexCache.get(key); if (cached) return cached;
    const value = (options.targets ?? []).map(metadataIndex).filter((index): index is MetadataIndex => index !== undefined);
    cacheSet(metadataIndexCache, key, value, maxEntries); return value;
  }
  function diagnostics(sourceText: string, fileName: string): readonly ToolingDiagnostic[] {
    const key = contentKey(fileName, sourceText); const cached = diagnosticsCache.get(key); if (cached) return cached;
  ...
  function hover(sourceText: string, fileName: string, offset: number): HoverResult | undefined {
    const current = analysis(sourceText, fileName); const lexical = queryAt(current, offset); if (!lexical) return undefined;
    if (!lexical.mappingReliable) return undefined;
    const generatedEvidence = generated();
    const token = tokenAt(lexical, offset);
    const relationUse = lexical.relationUses.find((use) => use.tokens.some((part) => token && part.start === token.start));
    if (relationUse?.relation && !relationUse.cte) return relationHover(relationUse.relation, sourceRange(token?.sourceStart ?? offset, token?.sourceEnd ?? offset), semanticOptions);
    const column = lexical.columnUses.find((use) => token && use.token.start === token.start);
    if (column) return columnHover(column.owner, column.column, sourceRange(column.token.sourceStart, column.token.sourceEnd), semanticOptions, generatedEvidence);
    const routine = lexical.routineUses.find((use) => token && use.token.start === token.start)?.routine;
    if (routine) return routineHover(routine, sourceRange(token?.sourceStart ?? offset, token?.sourceEnd ?? offset), semanticOptions);
    ...
    if (inTag || offsetInStatic(lexical, offset)) {
      const contract = overlay(sourceText, fileName).queryTypes.find((candidate) => candidate.range.start === lexical.query.range.start);
      const result = queryHover(lexical, semanticOptions);
      if (contract && contract.rowType !== "unknown" && !lexical.query.declaredRowType) return { ...result, contents: result.contents.replace("unknown", contract.rowType) };
      return result;
    }
    return undefined;
  }
  function complete(sourceText: string, fileName: string, offset: number): readonly CompletionItem[] {
    ...
    if (relationContext) candidates = relationCandidates(semanticOptions);
      ...
      if (owner) candidates = columnCandidates(owner);
        ...
        const cursorIndex = cursorToken ? lexical.tokens.indexOf(cursorToken) : nextTokenIndex >= 0 ? nextTokenIndex : lexical.tokens.length;
        const qualifierIndex = cursorIndex - 2;
        const qualifiedRelationContext = qualifierIndex > 0 && ["from", "join", "update", "into"].includes(lexical.tokens[qualifierIndex - 1]?.text.toLowerCase() ?? "");
        const relations = qualifier ? allRelations(semanticOptions).filter((relation) => relation.namespace === identifierKey(qualifier, lexical.foldIdentifiers)) : [];
        const routines = qualifier ? allRoutines(semanticOptions).filter((routine) => routine.schema === identifierKey(qualifier, lexical.foldIdentifiers)) : [];
        candidates = qualifiedRelationContext ? relations.map((relation) => ({ label: relation.name, kind: "relation" as const, detail: relation.kind })) : routines.map((routine) => ({ label: routine.name, kind: "routine" as const, detail: routine.kind }));
      }
    } else candidates = [...relationCandidates(semanticOptions), ...routineCandidates(semanticOptions), ...unique(lexical.relationUses.map((use) => use.relation).filter((relation): relation is RelationSnapshot => relation !== undefined), (relation) => relation.identity).flatMap(columnCandidates)];
    const wanted = lower(prefix); return bounded(unique(candidates.filter((item) => lower(item.label).startsWith(wanted)), (item) => `${item.kind}\0${item.label}`), maxEntries);
  }
  function definition(sourceText: string, fileName: string, offset: number): Location | undefined {
    const current = analysis(sourceText, fileName); const lexical = queryAt(current, offset, true); if (!lexical) return undefined; const token = tokenAt(lexical, offset); if (!token) return undefined;
    const generatedEvidence = generated();
    const relationUse = lexical.relationUses.find((use) => use.tokens.some((part) => part.start === token.start));
    if (relationUse?.relation && !relationUse.cte) return generatedLocation(relationUse.relation, undefined, semanticOptions, generatedEvidence) ?? targetMetadataLocation(relationUse.relation, undefined, semanticOptions, metadata());
    const column = lexical.columnUses.find((use) => use.token.start === token.start); if (column) return generatedLocation(column.owner, column.column.name, semanticOptions, generatedEvidence) ?? targetMetadataLocation(column.owner, column.column.name, semanticOptions, metadata());
    ...
    const visit = async (document: SourceDocument): Promise<boolean> => {
      ...
      for (const item of indexed.queries) {
        if (relation) for (const use of item.relationUses) if (use.relation?.identity === relation.identity && !use.cte) {
          ...
          if (first && last) results.push(location(document.fileName, document.sourceText, sourceRange(first.sourceStart, last.sourceEnd)));
        }
  ...
  function workspaceSymbols(query: string): readonly WorkspaceSymbol[] {
    const wanted = lower(query); const result: WorkspaceSymbol[] = [];
    const generatedEvidence = generated();
    for (const relation of allRelations(semanticOptions)) {
      ...
      const definitionLocation = generatedLocation(relation, undefined, semanticOptions, generatedEvidence) ?? targetMetadataLocation(relation, undefined, semanticOptions, metadata()); if (!definitionLocation) continue;
    ...
    for (const routine of allRoutines(semanticOptions)) {
      ...
      for (const evidence of metadataEvidence(semanticOptions)) if (evidence.target) { definitionLocation = metadataRange(metadata(), evidence.target, routine.identity, undefined, true); if (definitionLocation) break; }
      if (definitionLocation) result.push({ name: routine.name, kind: "routine", location: definitionLocation, detail: routine.identity });
    }
    for (const index of generated()) for (const [name, range] of index.declarations) if (lower(name).includes(wanted) && index.target.outFile && index.target.generatedSource) result.push({ name, kind: "model", location: location(index.target.outFile, index.target.generatedSource, range) });
    return bounded(unique(result, (item) => `${item.kind}\0${item.name}\0${item.location.uri}`), maxEntries);
  }
  ...
  function reload(metadata: MetadataSnapshot): SqlBraidLanguageService { return createLanguageService({ ...options, metadata }); }
  return { diagnostics, hover, complete, definition, references, documentSymbols, workspaceSymbols, signatureHelp, reload };
}
