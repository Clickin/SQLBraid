...
function objectRow(value: unknown): Row {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("MYSQL_INSPECT_ROW: metadata row is not an object.");
...
export function createMysqlInspector(connection: Mysql2ConnectionLike): MetadataInspector {
  return {
    ...
    async inspect(): Promise<MetadataSnapshot> {
      ...
      if (/mariadb/iu.test(`${version} ${product}`))
        throw new Error("MYSQL_PRODUCT_UNSUPPORTED: MariaDB requires a separate dialect profile.");
        ...
        const routine: RoutineSnapshot = {
          ...
          result:
            text(entry, "routine_type") === "PROCEDURE"
              ? { kind: "void" }
              : { kind: "scalar", type: resultType, nullable: true },
