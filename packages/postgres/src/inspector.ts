...
function objectRow(value: unknown): CatalogRow {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("PG_INSPECT_ROW: catalog row is not an object.");
  ...
  return {
    ...
    async inspect(): Promise<MetadataSnapshot> {
      ...
      for (const relation of tableRows) {
        ...
        const columns = columnRows
          ...
          .map((column) => {
            ...
            const generated =
              generatedEvidence === undefined && generationExpression === undefined
                ? undefined
            ...
            return {
              ...
              type: domainName
                ? qualifiedIdentity(text(column, "domain_schema") ?? schema, domainName)
                : qualifiedIdentity(text(column, "udt_schema") ?? "pg_catalog", text(column, "udt_name") ?? dataType),
              nullable: text(column, "is_nullable") === "YES",
              ...(numberValue(column, "numeric_precision") === undefined
                ? {}
                : { precision: numberValue(column, "numeric_precision") }),
              ...(numberValue(column, "numeric_scale") === undefined
                ? {}
                : { scale: numberValue(column, "numeric_scale") }),
              ...(text(column, "column_default") === undefined
                ? {}
                : { defaultExpression: text(column, "column_default") }),
              ...(generated === undefined ? {} : { generated }),
              ...(generated === true
                ? { insertable: false, updatable: false, ...(generationExpression ? { generationExpression } : {}) }
                : {}),
              ...(identityGenerated ? { identity: true } : {}),
              ...(text(column, "identity_generation")
                ? { identityGeneration: text(column, "identity_generation") }
                : {}),
            };
          });
        relations[identity] = {
          ...
          name,
          namespace: schema,
          kind: text(relation, "table_type") === "VIEW" ? "view" : "table",
          columns,
        };
      }
      const routineRows = await rows(
        client,
        "SELECT routine_schema, routine_name, routine_type, data_type, specific_name FROM information_schema.routines WHERE routine_schema NOT IN ('pg_catalog', 'information_schema') ORDER BY routine_schema, routine_name, specific_name",
      );
      ...
      for (const entry of routineRows) {
        ...
        if (!schema || !name) continue;
        const identity = qualifiedIdentity(schema, text(entry, "specific_name") ?? name);
        ...
        const routine: RoutineSnapshot = {
          ...
          result:
            text(entry, "routine_type") === "PROCEDURE"
              ? { kind: "void" }
              : { kind: "scalar", type: dataType, nullable: true },
        };
      ...
      const catalogTypes = new Map<
        string,
        { readonly row: CatalogRow; readonly schema: string; readonly name: string }
      >();
      for (const column of columnRows) {
        ...
        const identity = qualifiedIdentity(schema, name);
        const element =
          schema === "pg_catalog" && Object.hasOwn(builtinArrayElements, name) ? builtinArrayElements[name] : undefined;
        if (!types[identity]) {
      ...
      try {
        ...
        for (const row of catalogRows) {
          ...
          if (schema && name) catalogTypes.set(qualifiedIdentity(schema, name), { row, schema, name });
        }
      } catch {
      ...
      const identityForOid = (oid: number | undefined): string | undefined => {
        ...
        for (const [identity, entry] of catalogTypes) {
          if (numberValue(entry.row, "type_oid") === oid) return identity;
        }
        return undefined;
      };
      for (const [identity, entry] of catalogTypes) {
        const { row, schema, name } = entry;
      ...
      return {
        ...
        server: { product: "postgres", version, ...(majorVersion ? { majorVersion: Number(majorVersion) } : {}) },
        namespaces: {},
        types,
        relations,
        routines,
        metadata: {
          source: "postgres information_schema",
