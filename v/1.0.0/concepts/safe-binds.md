...
`${value}` uses the driver's normal inference. `${sql.bind(value, hint)}` requests an explicit database parameter type. SQLBraid does not infer a universal database type from a TypeScript `number`, `string`, or `Date`; this API is parameter typing, not application input validation or a codec framework. See [parameter type hints](/SQLBraid/v/1.0.0/concepts/parameter-hints.md).

## Structural input is opt-in

Identifiers and SQL fragments are different from data. Use the explicit helpers in [structural SQL fragments](/SQLBraid/v/1.0.0/concepts/structural-fragments.md):
