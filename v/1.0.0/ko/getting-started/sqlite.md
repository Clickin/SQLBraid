...
쓰기에는 `sql.command`와 `db.execute`를 사용하세요. 정확히 한 행에는 `db.one`을 사용합니다. 결과에 한 행이 없으면 카디널리티 오류가 발생합니다. [SQL 태그와 결과 종류](/SQLBraid/v/1.0.0/concepts/sql-tags.md)를 참고하세요.
