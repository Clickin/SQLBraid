...
`${value}`는 드라이버의 일반 추론을 사용합니다. `${sql.bind(value, hint)}`는 명시적인 데이터베이스 파라미터 타입을 요청합니다. SQLBraid는 TypeScript `number`, `string`, `Date`에서 보편적인 데이터베이스 타입을 추론하지 않습니다. 이 API는 애플리케이션 입력 검증이나 codec 프레임워크가 아닌 파라미터 타입 지정입니다. [파라미터 타입 힌트](/SQLBraid/v/1.0.1/concepts/parameter-hints.md)를 참고하세요.

## 구조적 입력은 선택 사항입니다

식별자와 SQL 조각은 데이터와 다릅니다. [구조적 SQL 조각](/SQLBraid/v/1.0.1/concepts/structural-fragments.md)의 명시적 헬퍼를 사용하세요.
