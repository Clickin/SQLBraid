...
The [runtime and driver support matrix](/SQLBraid/v/1.0.1/reference/support.md) records
...
For a write, use `sql.command` and `db.execute`. For exactly one row, use `db.one`; it throws a cardinality error unless the result contains one row. See [SQL tags and result kinds](/SQLBraid/v/1.0.1/concepts/sql-tags.md).
