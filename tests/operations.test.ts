...
test("manifest preserves declared result kinds for opaque SQL", () => {
  ...
  assert.deepEqual(
    queries.map((query) => createManifest(query).resultKind),
    ["rows", "command", "call", "unknown"],
  );
  assert.deepEqual(
    queries.map((query) => Object.keys(createManifest(query))),
    queries.map(() => ["fingerprint", "templateFamilyFingerprint", "resultKind"]),
  );
});

test("evidence manifest omits non-portable source paths", () => {
  assert.deepEqual(
    createManifestFromEvidence({
...
test("executable fingerprints include parameter hints without capturing values", () => {
  // oxlint-disable-next-line consistent-function-scoping -- Keep this parameterized template site local to its fingerprint comparisons.
  ...
  // oxlint-disable-next-line consistent-function-scoping -- This list template is the paired hint-length fixture for this test only.
  const list = (length: number) =>
    sql`SELECT ${sql.list([1, sql.bind("secret", { databaseType: "VARCHAR", length })])}`;
