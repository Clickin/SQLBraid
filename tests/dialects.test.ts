...
test('dialects own identifier rendering while transports own placeholders', () => {
  ...
  assert.deepEqual(pg.parameters.map(({ value }) => value), [1, 2]);
  assert.equal(parameterizedSql(pg, (index) => `$${index}`), 'SELECT $1, $2');
  const my = mysql`SELECT ${1}, ${2}`.render();
  assert.deepEqual(my.segments, ['SELECT ', ', ', '']);
  assert.deepEqual(my.parameters.map(({ value }) => value), [1, 2]);
  assert.equal(parameterizedSql(my, () => '?'), 'SELECT ?, ?');
  const sq = sqlite`SELECT ${1}`.render();
  assert.deepEqual(sq.segments, ['SELECT ', '']);
  assert.deepEqual(sq.parameters.map(({ value }) => value), [1]);
  assert.equal(parameterizedSql(sq, () => '?'), 'SELECT ?');
