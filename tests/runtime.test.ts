...
test('database wrappers share ownership for the same executor object', async () => {
  ...
  const transaction = first.tx(async (tx) => { await tx.execute(sql`SELECT 'inside'`); await gate; });
...
test('same-tick root transactions serialize their executor ownership', async () => {
  ...
  const first = db.tx(async (tx) => { await tx.execute(sql`SELECT 1`); await gate; });
  const second = db.tx(async (tx) => { await tx.execute(sql`SELECT 2`); });
...
test('root handle use from its transaction callback fails before queueing', async () => {
  ...
  await assert.rejects(() => db.tx(async () => db.execute(sql`SELECT 1`)), (error: unknown) => error instanceof Error && 'code' in error && error.code === 'BRAID_TX_SCOPE');
...
test('nested rollback releases its savepoint and preserves the parent scope', async () => {
  ...
  await db.tx(async (tx) => {
    await assert.rejects(() => tx.tx(async () => { throw new Error('nested failure'); }), /nested failure/);
...
test('transaction cleanup failure is not reported as success', async () => {
  ...
  await assert.rejects(() => db.tx(async () => { throw application; }), (error) => {
  ...
  await assert.rejects(() => db.tx(async () => undefined), (error) => error instanceof DatabaseScopeError && error.code === 'BRAID_CONNECTION_POISONED');
});
...
test('poisoned ownership rejects every wrapper sharing the physical resource', async () => {
  ...
  await assert.rejects(() => first.tx(async () => { throw new Error('application failed'); }), AggregateError);
...
test('successful transaction cleanup leaves physical ownership reusable', async () => {
  ...
  await assert.rejects(() => db.tx(async () => { throw new Error('application failed'); }));
...
test('nested rollback and release cleanup failures poison the parent ownership', async () => {
  ...
  await assert.rejects(() => rollbackFailure.tx(async (tx) => tx.tx(async () => { throw new Error('nested application failed'); })), AggregateError);
