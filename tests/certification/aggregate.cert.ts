...
test("aggregate the complete certification catalog", async () => {
  ...
  if (!inputDirectory || !outputPath) throw new Error("SQLBRAID_CERT_AGGREGATE_INPUT and SQLBRAID_CERT_AGGREGATE_OUTPUT are required.");
  const artifacts = await Promise.all((await files(resolve(inputDirectory))).map(async (path) => JSON.parse(await readFile(path, "utf8")) as CertificationArtifact));
  const requiredTargets = Object.freeze(Object.keys(CERTIFICATION_CONTRACTS));
  const candidate = process.env.SQLBRAID_CERT_CANDIDATE_JSON === undefined
    ? undefined
  ...
  const aggregate = aggregateCertificationArtifacts(artifacts, {
    sourceSha,
    requiredTargets,
    requiredTargetContracts: Object.fromEntries(requiredTargets.map((id) => [id, CERTIFICATION_CONTRACTS[id].expectedCapabilities])),
    requiredTargetOptionContracts: Object.fromEntries(requiredTargets.map((id) => [id, CERTIFICATION_CONTRACTS[id].expectedTransactionOptions])),
    requiredTargetGuardedCaseContracts: Object.fromEntries(requiredTargets.map((id) => [id, CERTIFICATION_CONTRACTS[id].expectedGuardedCases])),
  ...
  await writeFile(`${resolve(outputPath)}.summary`, `${JSON.stringify({ targets: requiredTargets.length, cases: Object.values(aggregate.targets).reduce((total, artifact) => total + Object.keys(artifact.cases).length, 0) })}\n`);
});
