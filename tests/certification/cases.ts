...
function emptyResultError(
  context: CaseContext,
): { readonly feature: string; readonly code: `BRAID_${string}` } | undefined {
  ...
  if (configured !== undefined) {
    ...
    assert.deepEqual(
      configured,
      independent,
      "empty-result error metadata must match the independent target contract.",
    );
    return configured;
  }
...
async function assertEmptyResult(context: CaseContext, operation: () => unknown | Promise<unknown>): Promise<void> {
  ...
  await assert.rejects(operation, (error: unknown) => {
    assert.ok(
      isPublicUnsupportedFeatureError(error),
      `Expected a registered empty-result UnsupportedFeatureError, got ${textError(error)}`,
    );
...
async function unsupported(
  ...
  if (expected.status !== "unsupported")
    throw new Error(`${id} cannot use an unsupported proof while ${feature} is ${expected.status}.`);
  ...
  assert.ok(
    isPublicUnsupportedFeatureError(error),
    `${id} must reject with a registered public UnsupportedFeatureError, got ${textError(error)}`,
  );
  ...
  if (probe.expectedErrorFeature === undefined)
    assert.equal(error.code, expected.unsupportedCode ?? probe.expectedCode);
  assert.equal(probe.sideEffects(), before, `${id} performed a side effect before rejecting ${feature}`);
  return {
    status: "pass-unsupported",
    name: id,
    feature,
    rejectionFeature: expectedErrorFeature === feature ? undefined : expectedErrorFeature,
    code: error.code,
  };
}
...
async function supported(
  ...
  if (expected.status === "guarded" && !expected.conditionCode)
    throw new Error(`${feature} is guarded without a conditionCode.`);
  await operation(context);
  return { status: "pass", name: id, feature };
}

async function supportedAll(
  context: CaseContext,
  id: CertificationCaseId,
...
function expectedValue(fixture: CertificationFixture, key: CertificationCaseId): unknown {
  ...
  if (!Object.hasOwn(fixture.queries.expected?.special ?? {}, key))
    throw new Error(`${key} has no independent expected value.`);
  return expected;
}
...
function assertCardinality(error: unknown, expected: "one" | "maybeOne", actual: number): void {
  assert.ok(
    (error as { readonly name?: unknown })?.name === "DatabaseCardinalityError",
    `Expected DatabaseCardinalityError, got ${textError(error)}`,
  );
...
async function runTransactionOption(
  context: CaseContext,
  id: CertificationCaseId,
  ...
  if (status === "unsupported") {
    ...
    return {
      ...
      feature: key,
      rejectionFeature:
        (probe.expectedErrorFeature ?? probe.feature) === key
          ? undefined
          : (probe.expectedErrorFeature ?? probe.feature),
      code: error.code,
    };
  }
    ...
    for (const db of databases(fixture)) {
      await assert.rejects(
        ...
        (error: unknown) => {
          ...
          assert.ok(
            candidate.code === expectedError.code || String(candidate.message ?? "").includes(expectedError.code),
            `${id} error must expose ${expectedError.code}.`,
          );
...
const operations: Readonly<Record<CertificationCaseId, (context: CaseContext) => Promise<CertificationCaseResult>>> = {
  ...
  QRY002: async ({ fixture }) => {
    for (const db of databases(fixture)) {
      ...
      if (fixture.queries.expected?.commandAffectedRows !== undefined)
        assert.equal(result.command.affectedRows, fixture.queries.expected.commandAffectedRows);
    }
    return { status: "pass", name: "QRY002" };
  },
  QRY010: async (context) => {
    for (const db of databases(context.fixture)) {
      const expected = emptyResultError(context);
      if (expected === undefined) assert.deepEqual(await db.all(context.fixture.queries.zero), []);
      else await assertEmptyResult(context, () => db.all(context.fixture.queries.zero));
    }
    return { status: "pass", name: "QRY010" };
  },
  QRY011: async ({ fixture }) => {
    for (const db of databases(fixture))
      assert.deepEqual(await db.all(fixture.queries.one), [fixture.queries.expected?.one]);
    return { status: "pass", name: "QRY011" };
  },
  QRY012: async ({ fixture }) => {
    for (const db of databases(fixture))
      assert.deepEqual(await db.all(fixture.queries.many), fixture.queries.expected?.many);
    return { status: "pass", name: "QRY012" };
  },
  QRY020: async ({ fixture }) => {
    for (const db of databases(fixture))
      assert.deepEqual(await db.one(fixture.queries.one), fixture.queries.expected?.one);
    return { status: "pass", name: "QRY020" };
  },
  QRY021: async (context) => {
    for (const db of databases(context.fixture)) {
      const expected = emptyResultError(context);
      if (expected === undefined)
        await assert.rejects(
          () => db.one(context.fixture.queries.zero),
          (error: unknown) => {
            assertCardinality(error, "one", 0);
            return true;
          },
        );
      else await assertEmptyResult(context, () => db.one(context.fixture.queries.zero));
    }
    return { status: "pass", name: "QRY021" };
  },
  QRY022: async ({ fixture }) => {
    for (const db of databases(fixture))
      await assert.rejects(
        () => db.one(fixture.queries.many),
        (error: unknown) => {
          assertCardinality(error, "one", 2);
          return true;
        },
      );
    return { status: "pass", name: "QRY022" };
  },
  QRY030: async (context) => {
    for (const db of databases(context.fixture)) {
      const expected = emptyResultError(context);
      if (expected === undefined) assert.equal(await db.maybeOne(context.fixture.queries.zero), undefined);
      else await assertEmptyResult(context, () => db.maybeOne(context.fixture.queries.zero));
    }
    return { status: "pass", name: "QRY030" };
  },
  QRY031: async ({ fixture }) => {
    for (const db of databases(fixture))
      assert.deepEqual(await db.maybeOne(fixture.queries.one), fixture.queries.expected?.one);
    return { status: "pass", name: "QRY031" };
  },
  QRY032: async ({ fixture }) => {
    for (const db of databases(fixture))
      await assert.rejects(
        () => db.maybeOne(fixture.queries.many),
        (error: unknown) => {
          assertCardinality(error, "maybeOne", 2);
          return true;
        },
      );
    return { status: "pass", name: "QRY032" };
  },

  ...(Object.fromEntries(
    RESULT_CASES.map((id) => [
      id,
      async ({ fixture }: CaseContext) => {
        await runResultCase(fixture, id);
        return { status: "pass", name: id };
      },
    ]),
  ) as Record<(typeof RESULT_CASES)[number], (context: CaseContext) => Promise<CertificationCaseResult>>),

  VAL001: async (context) =>
    supported(context, "VAL001", "sql.native-transparency", async ({ fixture }) => {
  ...
  VAL002: async (context) =>
    supported(context, "VAL002", "sql.native-transparency", async ({ fixture }) => {
      const values = fidelityQueries(fixture);
      const row = await fixture.db.one(values.exactDecimal);
      assert.deepEqual(row, values.expected.exactDecimal);
    }),
  VAL003: async (context) =>
    supported(context, "VAL003", "sql.native-transparency", async ({ fixture }) => {
      const values = fidelityQueries(fixture);
      const row = await fixture.db.one(values.temporal);
      assert.deepEqual(row, values.expected.temporal);
    }),
  VAL004: async (context) =>
    supported(context, "VAL004", "sql.native-transparency", async ({ fixture }) => {
      const values = fidelityQueries(fixture);
      const metrics = fixtureMetrics(fixture);
      const expectedInjection = (values.expected.injection as { readonly value?: unknown }).value;
      assert.ok(
        values.injection.render().parameters.some((parameter) => parameter.value === expectedInjection),
        "VAL004 requires the injection payload to be a rendered bind.",
      );
  ...
  SES001: async (context) =>
    supported(context, "SES001", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        const ids: string[] = [];
        await db.session(async (session) => {
          ...
          if (emptyResultError(context) === undefined)
  ...
  SES002: async (context) =>
    supported(context, "SES002", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        const before = await fixtureMetrics(fixture).snapshot();
        await db.session(async (session) => {
          await session.one(fixture.queries.identity);
        });
  ...
  SES003: async (context) =>
    supported(context, "SES003", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        const before = await fixtureMetrics(fixture).snapshot();
        await assert.rejects(() =>
          db.session(async () => {
            throw new Error("cert-session-callback");
          }),
        );
        await cleanResources(fixture, before);
      }
    }),
  SES004: async (context) =>
    supported(context, "SES004", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        ...
        await assert.rejects(
          () => scoped!.one(fixture.queries.identity),
          (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_SESSION_CLOSED",
        );
      }
    }),
  SES005: async (context) =>
    supported(context, "SES005", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        await assert.rejects(
          () =>
            db.session(async () => {
          ...
          (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_SESSION_SCOPE",
        );
      }
    }),
  SES006: async (context) =>
    supported(context, "SES006", "session.pinned", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        const before = await fixtureMetrics(fixture).snapshot();
        await assert.rejects(
          () =>
            db.session(async (session) => {
              await session.one(fixture.queries.failure);
            }),
          (error: unknown) => {
  ...
  SES007: async (context) =>
    supportedAll(context, "SES007", ["session.pinned", "statement.stream"], async ({ fixture }) => {
      ...
      for (const db of databases(fixture)) {
        ...
        await db.session(async (session) => {
          ...
          if (fixture.pooled !== undefined && db === fixture.pooled)
            assert.ok(inside.borrowedLeases > before.borrowedLeases);
        });
        await cleanResources(fixture, before);
      }
    }),
  SES008: async (context) =>
    supportedAll(context, "SES008", ["session.pinned", "transaction"], async ({ fixture }) => {
      for (const db of databases(fixture)) {
        await db.session(async (session) => {
          ...
          await session.session(async (nestedSession) => {
            ...
            await assert.rejects(
              () => nestedTransaction!.one(fixture.queries.identity),
              (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_TX_CLOSED",
            );
          });
          await assert.rejects(
            () => nestedHandle!.one(fixture.queries.identity),
            (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_SESSION_CLOSED",
          );
        });
      }
    ...
    supported(context, "TX002", "transaction", async ({ fixture }) => {
      ...
      for (const db of databases(fixture)) {
        ...
        await assert.rejects(() =>
          db.tx(async (tx) => {
            await tx.execute(fixture.queries.transaction!.insert);
            throw new Error("cert-rollback");
          }),
        );
        if (emptyResultError(context) === undefined)
          assert.equal((await db.all(fixture.queries.transaction!.visible)).length, 0);
        else await assertEmptyResult(context, () => db.all(fixture.queries.transaction!.visible));
      }
    }),
  TX003: async (context) =>
    supported(context, "TX003", "transaction", async ({ fixture }) => {
      ...
      for (const db of databases(fixture)) {
        ...
        await assert.rejects(
          () =>
            db.tx(async (tx) => {
              await tx.execute(fixture.queries.transaction!.insert);
              await tx.execute(fixture.queries.failure);
            }),
          (error: unknown) => {
            const expectedCode = fixture.queries.expected?.failureCode;
            if (expectedCode !== undefined) assert.equal((error as { readonly code?: unknown }).code, expectedCode);
  ...
  TX004: async (context) =>
    supported(context, "TX004", "transaction", async ({ fixture }) => {
      ...
      for (const db of databases(fixture)) {
        ...
        await db.tx(async (tx) => {
          ...
          if (emptyResultError(context) === undefined)
            assert.deepEqual(await tx.maybeOne(fixture.queries.zero), undefined);
  ...
  TX005: async (context) =>
    supported(context, "TX005", "transaction", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        await assert.rejects(
          () =>
            db.tx(async () => {
          ...
          (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_TX_SCOPE",
        );
      }
    }),
  TX006: async (context) =>
    supported(context, "TX006", "transaction", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        let scoped: Database | undefined;
        await db.tx(async (tx) => {
          scoped = tx;
  ...
  TX007: async (context) =>
    supported(context, "TX007", "transaction", async ({ fixture }) => {
  ...
  TX008: async (context) =>
    supported(context, "TX008", "transaction", async ({ fixture }) => {
      for (const db of databases(fixture)) {
        await db.tx(async (tx) => {
          await tx.one(fixture.queries.identity);
        });
        await db.one(fixture.queries.identity);
      }
    }),
  TX009: async (context) =>
    supported(context, "TX009", "transaction", async ({ fixture }) => {
  ...
  TX010: async (context) =>
    supported(context, "TX010", "transaction.savepoint", async ({ fixture }) => {
      ...
      for (const db of databases(fixture)) {
        ...
        await db.tx(async (outer) => {
          await outer.tx(async (inner) => {
            await inner.execute(fixture.queries.transaction!.savepointInsert);
          });
        });
        assert.equal((await db.all(fixture.queries.transaction.visible)).length, 1);
      }
  ...
  TX011: async (context) =>
    supported(context, "TX011", "transaction.savepoint", async ({ fixture }) => {
        ...
        await db.tx(async (outer) => {
          await assert.rejects(() =>
            outer.tx(async (inner) => {
              await inner.execute(fixture.queries.transaction!.savepointInsert);
              throw new Error("cert-savepoint-rollback");
            }),
          );
        });
        if (emptyResultError(context) === undefined)
  ...
  TX012: async (context) =>
    supported(context, "TX012", "transaction", async ({ fixture }) => {
  ...
  TX013: async (context) =>
    supported(context, "TX013", "transaction.savepoint", async ({ fixture }) => {
      if (!fixture.queries.transaction) throw new Error("TX013 transaction fixture missing.");
      for (const db of databases(fixture)) {
        await fixture.reset();
        await db.tx(async (outer) => {
          await outer.tx(async (inner) => {
            await inner.tx(async (deep) => {
              await deep.execute(fixture.queries.transaction!.savepointInsert);
            });
          });
        });
        assert.equal((await db.all(fixture.queries.transaction.visible)).length, 1);
  ...
  TX020: async (context) =>
    runTransactionOption(context, "TX020", "isolation:read-uncommitted", { isolation: "read-uncommitted" }),
  TX021: async (context) => runTransactionOption(context, "TX021", "readOnly:true", { readOnly: true }),
  TX022: async (context) =>
    runTransactionOption(context, "TX022", "combination:read-committed+readOnly", {
      isolation: "read-committed",
      readOnly: true,
    }),
  TX023: async (context) =>
    runTransactionOption(context, "TX023", "isolation:read-committed", { isolation: "read-committed" }),
  TX024: async (context) =>
    runTransactionOption(context, "TX024", "isolation:repeatable-read", { isolation: "repeatable-read" }),
  TX025: async (context) =>
    runTransactionOption(context, "TX025", "isolation:serializable", { isolation: "serializable" }),
  TX026: async (context) => runTransactionOption(context, "TX026", "readOnly:false", { readOnly: false }),
  TX027: async (context) =>
    runTransactionOption(context, "TX027", "combination:read-uncommitted+readOnly", {
      isolation: "read-uncommitted",
      readOnly: true,
    }),
  TX028: async (context) =>
    runTransactionOption(context, "TX028", "combination:serializable+readOnly", {
      isolation: "serializable",
      readOnly: true,
    }),
  TX029: async (context) =>
    runTransactionOption(context, "TX029", "combination:read-uncommitted+readWrite", {
      isolation: "read-uncommitted",
      readOnly: false,
    }),
  TX030: async (context) =>
    runTransactionOption(context, "TX030", "combination:read-committed+readWrite", {
      isolation: "read-committed",
      readOnly: false,
    }),
  TX031: async (context) =>
    runTransactionOption(context, "TX031", "combination:repeatable-read+readOnly", {
      isolation: "repeatable-read",
      readOnly: true,
    }),
  TX032: async (context) =>
    runTransactionOption(context, "TX032", "combination:repeatable-read+readWrite", {
      isolation: "repeatable-read",
      readOnly: false,
    }),
  TX033: async (context) =>
    runTransactionOption(context, "TX033", "combination:serializable+readWrite", {
      isolation: "serializable",
      readOnly: false,
    }),

  PRE001: async (context) =>
    supported(context, "PRE001", "statement.prepare", async ({ fixture }) => {
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("PRE001 prepared fixture missing.");
      ...
      const result = await (
        handle as unknown as { execute(input: unknown): Promise<{ readonly kind: string }> }
      ).execute(prepared.input);
      assert.equal(result.kind, "command");
      assert.equal(prepared.factoryCalls(), before + 1);
    }),
  PRE002: async (context) =>
    supported(context, "PRE002", "statement.prepare", async ({ fixture }) => {
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("PRE002 prepared fixture missing.");
      const before = prepared.factoryCalls();
      const handle = fixture.db.prepare("cert-prepared-rows", (input: unknown) => prepared.rows(input));
      const rows = await (handle as unknown as { all(input: unknown): Promise<readonly unknown[]> }).all(
        prepared.input,
      );
      assert.deepEqual(rows, fixture.queries.expected?.many ?? [fixture.queries.expected?.one]);
      assert.equal(prepared.factoryCalls(), before + 1);
    }),
  PRE003: async (context) =>
    supportedAll(context, "PRE003", ["statement.prepare", "statement.stream"], async ({ fixture }) => {
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("PRE003 prepared fixture missing.");
      const before = prepared.factoryCalls();
      ...
      for await (const row of stream) values.push(row);
  ...
  PRE004: async (context) =>
    supportedAll(context, "PRE004", ["statement.prepare", "statement.stream"], async ({ fixture }) => {
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("PRE004 prepared fixture missing.");
      const before = prepared.factoryCalls();
  ...
  PRE005: async (context) =>
    supportedAll(context, "PRE005", ["statement.prepare", "statement.stream"], async ({ fixture }) => {
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("PRE005 prepared fixture missing.");
      const before = prepared.factoryCalls();
      const handle = fixture.db.prepare("cert-prepared-break", (input: unknown) => prepared.rows(input));
      for await (const row of (handle as unknown as { stream(input: unknown): AsyncIterable<unknown> }).stream(
        prepared.input,
      )) {
        void row;
        break;
  ...
  PRE006: async (context) =>
    supported(context, "PRE006", "statement.prepare", async ({ fixture }) => {
  ...
  PRE007: async (context) =>
    supported(context, "PRE007", "statement.prepare", async ({ fixture }) => {
      ...
      await assert.rejects(
        () => (many as unknown as { maybeOne(): Promise<unknown> }).maybeOne(),
        (error: unknown) => {
          assertCardinality(error, "maybeOne", 2);
          return true;
        },
  ...
  PRE008: async (context) =>
    supported(context, "PRE008", "routine.call", async ({ fixture }) => {
  ...
  PRE009: async (context) =>
    supported(context, "PRE009", "statement.prepare", async ({ fixture }) => {
      const prepare = fixture.db.prepare as unknown as (
        name: string,
        factory: (input: string) => import("@sqlbraid/core").PreparableQuery,
      ) => unknown;
      const handle = prepare("cert-prepared-shape", (input: string) =>
        input === "one" ? fixture.queries.one : fixture.queries.command,
      );
      await (handle as { execute(input: string): Promise<unknown> }).execute("one");
      await assert.rejects(
        () => (handle as { execute(input: string): Promise<unknown> }).execute("two"),
        (error: unknown) => (error as { readonly code?: unknown }).code === "BRAID_PREPARED_SHAPE",
      );
    }),
  PRE010: async (context) =>
    supported(context, "PRE010", "statement.prepare", async ({ fixture }) => {
  ...
  PRE011: async (context) =>
    supportedAll(
      context,
      "PRE011",
      ["statement.prepare", "statement.stream", "statement.cancel"],
      async ({ fixture }) => {
        const controller = new AbortController();
        const abortError = new Error("cert-prepared-abort");
        const handle = preparedQuery(fixture, "cert-prepared-abort", fixture.queries.stream ?? fixture.queries.many, {
          input: "none",
        });
        await assert.rejects(
          async () => {
            for await (const row of (
              handle as unknown as { stream(options?: { signal?: AbortSignal }): AsyncIterable<unknown> }
          ...
          (error: unknown) =>
            error === abortError || (error instanceof AggregateError && error.errors.includes(abortError)),
        );
      },
    ),

  STR001: async (context) =>
    supported(context, "STR001", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR001", () => ({ ...stream, close: undefined }));
    }),
  STR002: async (context) =>
    supported(context, "STR002", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR002", () => ({ ...stream, close: undefined }));
    }),
  STR003: async (context) =>
    supported(context, "STR003", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR003", () => ({ ...stream, close: undefined }));
    }),
  STR004: async (context) =>
    supported(context, "STR004", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR004", () => ({ ...stream, close: undefined }));
    }),
  STR005: async (context) =>
    supported(context, "STR005", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR005", () => ({ ...stream, close: undefined }));
    }),
  STR006: async (context) =>
    supported(context, "STR006", "statement.cancel", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR006", () => ({ ...stream, close: undefined }));
    }),
  STR007: async (context) =>
    supported(context, "STR007", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR007", () => ({ ...stream, close: undefined }));
    }),
  STR008: async (context) =>
    supported(context, "STR008", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR008", () => ({ ...stream, close: undefined }));
    }),
  STR009: async (context) =>
    supported(context, "STR009", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR009", () => ({ ...stream, close: undefined }));
    }),
  STR010: async (context) =>
    supported(context, "STR010", "statement.cancel", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR006", () => ({ ...stream, close: undefined }), {
        abortError: new Error("cert-custom-abort"),
      });
    }),
  STR011: async (context) =>
    supported(context, "STR011", "statement.stream", async ({ fixture }) => {
      const stream = streamFixture(fixture);
      await runStreamingConformanceCase("STR011", () => ({ ...stream, close: undefined }));
    }),

  CALL001: async (context) =>
    supported(context, "CALL001", "routine.call", async ({ fixture }) => {
      if (!fixture.queries.routines) throw new Error("CALL001 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.call);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL001);
    }),
  CALL002: async (context) =>
    supported(context, "CALL002", "routine.out", async ({ fixture }) => {
      if (!fixture.queries.routines?.out) throw new Error("CALL002 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.out);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL002);
    }),
  CALL003: async (context) =>
    supported(context, "CALL003", "routine.inout", async ({ fixture }) => {
      if (!fixture.queries.routines?.inout) throw new Error("CALL003 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.inout);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL003);
    }),
  CALL004: async (context) =>
    supported(context, "CALL004", "routine.result-sets", async ({ fixture }) => {
      if (!fixture.queries.routines?.resultSets) throw new Error("CALL004 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.resultSets);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL004);
    }),
  CALL005: async (context) =>
    supported(context, "CALL005", "routine.out-cursor", async ({ fixture }) => {
      if (!fixture.queries.routines?.cursor) throw new Error("CALL005 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.cursor);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL005);
    }),
  CALL006: async (context) =>
    supported(context, "CALL006", "routine.return-value", async ({ fixture }) => {
      if (!fixture.queries.routines?.returnValue) throw new Error("CALL006 routine fixture missing.");
      const result = await callRoutine(fixture, fixture.queries.routines.returnValue);
      assert.deepEqual(result, fixture.queries.expected?.special.CALL006);
    }),
  CALL007: async (context) =>
    supported(context, "CALL007", "routine.call", async ({ fixture }) => {
      const proof = fixtureMetrics(fixture).routineCleanup;
      if (!proof || !fixture.queries.routines?.lob)
        throw new Error("CALL007 requires a native routine LOB cleanup proof.");
      await proof(fixture.queries.routines.lob);
    }),

  BULK001: async (context) =>
    supported(context, "BULK001", "statement.bulk", async ({ fixture }) => {
      if (!fixture.bulk) throw new Error("BULK001 bulk fixture missing.");
      await runBulkConformanceCase("BULK001", { ...fixture.bulk, close: undefined });
    }),
  BULK002: async (context) =>
    supported(context, "BULK002", "statement.bulk", async ({ fixture }) => {
      if (!fixture.bulk) throw new Error("BULK002 bulk fixture missing.");
      await runBulkConformanceCase("BULK002", { ...fixture.bulk, close: undefined });
    }),
  BULK003: async (context) =>
    supported(context, "BULK003", "statement.bulk", async ({ fixture }) => {
      if (!fixture.bulk) throw new Error("BULK003 bulk fixture missing.");
      await runBulkConformanceCase("BULK003", { ...fixture.bulk, close: undefined });
    }),
  BULK004: async (context) =>
    supportedAll(context, "BULK004", ["statement.bulk", "transaction"], async ({ fixture }) => {
  ...
  BAT002: async ({ fixture }) => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.equal((error as { readonly code?: unknown }).code, fixture.queries.expected?.failureCode);
        return true;
      },
    );
  ...
  CAP001: async ({ target, fixture }) => {
    ...
    const expected = Object.fromEntries(
      Object.entries(target.expectedCapabilities).map(([key, value]) => {
  ...
  CAP002: async (context) => {
    ...
    if (
      context.target.expectedCapabilities["result.multiple-sets"]?.status === "unsupported" &&
    ...
    for (const [feature] of apiUnsupported) {
      ...
      if (
        proof &&
        representationFeature(feature) &&
        !(REQUIRED_API_CAPABILITY_IDS as readonly string[]).includes(feature)
      ) {
        await proof.prove();
    ...
    for (const probe of probes) {
      ...
      if (value && value.status !== "unsupported")
        throw new Error(`CAP002 probe ${feature} contradicts expected capability.`);
      const before = probe.sideEffects();
      let error: unknown;
      try {
        await probe.run();
      ...
      assert.equal(error.code, probe.expectedCode);
      if (probe.expectedErrorFeature === undefined && value?.unsupportedCode)
        assert.equal(error.code, value.unsupportedCode);
      assert.equal(probe.sideEffects(), before);
    }
    for (const [feature, proof] of Object.entries(context.fixture.guarded ?? {})) {
      ...
      if (context.target.expectedCapabilities[feature]?.status !== "guarded")
        throw new Error(`CAP002 guarded proof ${feature} is not declared guarded.`);
      await proof.prove();
    }
  ...
  ERR001: async ({ fixture }) => {
    ...
    for (const [, probe] of probes) {
      ...
      assert.ok(
        isPublicUnsupportedFeatureError(error),
        "ERR001 requires a registered public UnsupportedFeatureError pair.",
      );
      assert.equal(error.code, probe.expectedCode);
      assert.equal(error.feature, probe.expectedErrorFeature ?? probe.feature);
      assert.equal(probe.sideEffects(), before);
  ...
  STRESS001: async ({ fixture, stress }) => {
    const metrics = fixtureMetrics(fixture);
    const before = await metrics.snapshot();
    const count = stress ? 1000 : 10;
    for (let index = 0; index < count; index += 1) await fixture.db.one(fixture.queries.one);
    assertStableStress(before, await metrics.snapshot());
    await fixture.db.one(fixture.queries.one);
    return { status: "pass", name: "STRESS001" };
  },
  STRESS002: async (context) =>
    supported(context, "STRESS002", "transaction", async ({ fixture, stress }) => {
      const metrics = fixtureMetrics(fixture);
      const before = await metrics.snapshot();
      const count = stress ? 1000 : 10;
      for (let index = 0; index < count; index += 1)
        await fixture.db.tx(async (tx) => {
          await tx.one(fixture.queries.identity);
        });
      assertStableStress(before, await metrics.snapshot());
      await fixture.db.one(fixture.queries.one);
    }),
  STRESS003: async (context) =>
    supported(context, "STRESS003", "statement.prepare", async ({ fixture, stress }) => {
      const metrics = fixtureMetrics(fixture);
      const before = await metrics.snapshot();
      const count = stress ? 500 : 10;
      const prepared = fixture.queries.prepared;
      if (!prepared) throw new Error("STRESS003 prepared fixture missing.");
      for (let index = 0; index < count; index += 1) {
        const handle = fixture.db.prepare(`cert-stress-${index}`, (input: unknown) => prepared.rows(input));
        await (handle as unknown as { all(input: unknown): Promise<readonly unknown[]> }).all(prepared.input);
      }
      assertStableStress(before, await metrics.snapshot());
      await fixture.db.one(fixture.queries.one);
    }),
  STRESS004: async (context) =>
    supported(context, "STRESS004", "statement.stream", async ({ fixture, stress }) => {
      const metrics = fixtureMetrics(fixture);
      const stream = streamFixture(fixture);
      const before = await metrics.snapshot();
      const count = stress ? 500 : 10;
      for (let index = 0; index < count; index += 1) {
        for await (const row of fixture.db.stream(stream.query)) {
          void row;
          break;
        }
      }
  ...
  STRESS005: async (context) =>
    supported(context, "STRESS005", "transaction", async ({ fixture, stress }) => {
      const metrics = fixtureMetrics(fixture);
      const before = await metrics.snapshot();
      const count = stress ? 1000 : 10;
      for (let index = 0; index < count; index += 1)
        await assert.rejects(() =>
          fixture.db.tx(async () => {
            throw new Error("stress-rollback");
          }),
        );
      assertStableStress(before, await metrics.snapshot());
      await fixture.db.one(fixture.queries.one);
    }),
  STRESS006: async (context) =>
    supported(context, "STRESS006", "session.pinned", async ({ fixture, stress }) => {
      const metrics = fixtureMetrics(fixture);
      const before = await metrics.snapshot();
      const count = stress ? 1000 : 10;
      for (let index = 0; index < count; index += 1)
...
export function capabilityStatus(
  capabilities: Readonly<Record<string, EnvironmentCapability>>,
  feature: string,
): CapabilityStatus {
