...
export const MARIADB_EXPECTED_TRANSACTION_OPTIONS: Readonly<
  Record<TransactionOptionKey, "guaranteed" | "unsupported">
> = {
  "isolation:read-uncommitted": "guaranteed",
  "isolation:read-committed": "guaranteed",
  "isolation:repeatable-read": "guaranteed",
...
function sqliteUnsupportedCapabilities(
  stream: boolean,
  session: boolean,
  ...
  return {
    ...
    "transaction.read-only": localReadOnly
      ? {
          ...
          unsupportedCode: "BRAID_TX_OPTION_UNSUPPORTED",
        }
    ...
    "transaction.isolation.serializable": localReadOnly
      ? unsupported("BRAID_TX_OPTION_UNSUPPORTED")
      : { status: "guaranteed" },
    "statement.prepare": { status: "guaranteed" },
...
export function LIBSQL_EXPECTED_CAPABILITIES(): ExpectedCapabilityContract {
  return {
    ...
    "session.pinned": {
      status: "unsupported",
      conditionCode: "libsql.client-no-session-pinning",
      unsupportedCode: "BRAID_SESSION_UNSUPPORTED",
    },
  };
}
...
function capability(
  status: ExpectedCapability["status"],
  fields: Partial<ExpectedCapability> = {},
): ExpectedCapability {
...
export function BUN_EXPECTED_CAPABILITIES(dialect: BunSqlDialect): ExpectedCapabilityContract {
  ...
  return Object.freeze({
    ...
    "result.rows": capability(
      dialect === "postgres" ? "guaranteed" : "guarded",
      dialect === "postgres"
        ? {}
        : { conditionCode: dialect === "sqlite" ? "bun-sql.sqlite-result-parser" : "bun-sql.result-kind-metadata" },
    ),
    ...
    "numeric.exact-integer": capability("guarded", {
      canonical: "string",
      rawRepresentations: ["number", "string", "bigint"],
      conditionCode: "bun-sql.integer-width-profile",
    }),
    "numeric.exact-decimal":
      dialect === "postgres"
        ? capability("guaranteed", { canonical: "string", rawRepresentations: ["string"] })
        : capability("unsupported", {
            canonical: "string",
            rawRepresentations: [mysqlFamily ? "Uint8Array" : "number"],
          }),
    "numeric.approximate-float": capability("guarded", {
      canonical: "number",
      rawRepresentations: ["number"],
      conditionCode: "bun-sql.float-profile",
    }),
    "numeric.approximate-special": mysqlFamily
      ? capability("unsupported", { canonical: "number", rawRepresentations: ["null", "number"] })
      : capability("guarded", {
          canonical: "number",
          rawRepresentations: ["number"],
          conditionCode: "bun-sql.special-float-profile",
        }),
    "numeric.bind-exact": capability("guarded", {
      canonical: "string",
      rawRepresentations: ["string", "number", "bigint"],
      conditionCode: "bun-sql.numeric-bind-profile",
    }),
    "numeric.command-metadata": capability("guarded", {
      canonical: "number",
      rawRepresentations: ["number", "bigint"],
      conditionCode: "bun-sql.command-count-profile",
    }),
    "data.json-parsed": jsonNative
      ? capability("guarded", { rawRepresentations: ["object", "array"], conditionCode: "bun-sql.json-parser-profile" })
      : capability("unsupported", { rawRepresentations: ["string"] }),
    "data.json-lossless-text": jsonNative
      ? capability("unsupported")
      : capability("guaranteed", { canonical: "string", rawRepresentations: ["string"] }),
    "data.binary": mysqlFamily
      ? capability("unsupported", { canonical: "Uint8Array", rawRepresentations: ["Uint8Array"] })
      : capability("guaranteed", { canonical: "Uint8Array", rawRepresentations: ["Uint8Array"] }),
    "data.temporal-native":
      dialect === "sqlite"
        ? capability("unsupported", { rawRepresentations: ["string"] })
        : capability("guarded", { rawRepresentations: ["Date", "string"], conditionCode: "bun-sql.temporal-profile" }),
    "data.temporal-lossless":
      dialect === "sqlite"
        ? capability("guaranteed", { canonical: "string", rawRepresentations: ["string"] })
        : capability("unsupported"),
    "data.timezone":
      dialect === "sqlite"
        ? capability("unsupported", { rawRepresentations: ["string"] })
        : capability("guarded", { rawRepresentations: ["Date"], conditionCode: "bun-sql.timezone-profile" }),
    "metadata.command-safe": capability("guarded", {
      canonical: "number",
      rawRepresentations: ["number", "bigint"],
      conditionCode: "bun-sql.command-count-profile",
    }),
    "dml.insert-returning":
      server && dialect !== "postgres" && dialect !== "mariadb" ? capability("unsupported") : capability("guaranteed"),
    ...
    "execution.bulk-fidelity": capability("guarded", {
      rawRepresentations: ["prepared-loop"],
      conditionCode: "bun-sql.bulk-profile",
    }),
    ...
    "transaction.isolation.read-uncommitted": server
      ? capability("guarded", { conditionCode: "bun-sql.transaction-options" })
      : capability("unsupported"),
    "transaction.isolation.read-committed": server
      ? capability("guarded", { conditionCode: "bun-sql.transaction-options" })
      : capability("unsupported"),
    "transaction.isolation.repeatable-read": server
      ? capability("guarded", { conditionCode: "bun-sql.transaction-options" })
      : capability("unsupported"),
    "transaction.isolation.serializable": server
      ? capability("guarded", { conditionCode: "bun-sql.transaction-options" })
...
export function BUN_EXPECTED_TRANSACTION_OPTIONS(
  dialect: BunSqlDialect,
): Readonly<Record<TransactionOptionKey, "guaranteed" | "unsupported">> {
  return Object.freeze(
    Object.fromEntries(
      OPTION_KEYS.map((key) => [
        key,
...
export const CERTIFICATION_TARGET_TUPLES: Readonly<Record<string, CertificationTuple>> = {
  "postgres-pg-node-16-4": {
    database: { product: "postgres", version: "16.4", edition: "alpine", versionStatus: "measured" },
    driver: { id: "pg", package: "@sqlbraid/postgres", version: "8.23.0", profile: "pg-lossless-text" },
    runtime: { id: "node", version: "22.18.0" },
  },
  "postgres-current": {
    database: { product: "postgres", version: "18.6", edition: "alpine", versionStatus: "measured" },
    driver: { id: "pg", package: "@sqlbraid/postgres", version: "8.23.0", profile: "pg-lossless-text" },
    runtime: { id: "node", version: "22.18.0" },
  },
  ...
  "mysql-mysql2-node-8-4-2": {
    database: { product: "mysql", version: "8.4.2", edition: "community", versionStatus: "measured" },
    driver: { id: "mysql2", package: "@sqlbraid/mysql", version: "3.24.4", profile: "mysql2-lossless-text" },
    runtime: { id: "node", version: "22.18.0" },
  },
  ...
  "mariadb-connector-node-11-8-9": {
    database: { product: "mariadb", version: "11.8.9", edition: "community", versionStatus: "measured" },
    driver: { id: "mariadb", package: "@sqlbraid/mariadb", version: "3.5.4", profile: "mariadb-lossless-text" },
    runtime: { id: "node", version: "22.18.0" },
  },
  "oracle-oracledb-thin-node-23-9": {
    database: { product: "oracle", version: "23.9", edition: "Free", versionStatus: "measured" },
    driver: { id: "node-oracledb", package: "@sqlbraid/oracle", version: "7.0.1", profile: "oracle-thin" },
    runtime: { id: "node", version: "22.18.0" },
  },
  "mssql-tedious-developer-node-2022-cu18": {
    database: { product: "mssql", version: "2022-CU18", edition: "Developer", versionStatus: "measured" },
    driver: { id: "tedious", package: "@sqlbraid/mssql", version: "20.0.0", profile: "mssql-tedious" },
    runtime: { id: "node", version: "22.18.0" },
  },
  "sqlite-node-sqlite-node-22-18-0": {
    database: { product: "sqlite", version: "3.50.2", edition: "Node bundled SQLite", versionStatus: "measured" },
    driver: {
      id: "node-sqlite",
      package: "@sqlbraid/sqlite/node-sqlite",
      version: "22.18.0",
      profile: "sqlite-exact-string",
    },
    runtime: { id: "node", version: "22.18.0" },
  },
  "sqlite-node-sqlite-deno-2-9-3": {
    database: { product: "sqlite", version: "3.53.2", edition: "Deno bundled SQLite", versionStatus: "measured" },
    driver: {
      id: "node-sqlite",
      package: "@sqlbraid/sqlite/node-sqlite",
      version: "2.9.3",
  ...
  "better-sqlite3-node-22-18-0": {
    ...
    driver: {
      ...
      version: "13.0.3",
      profile: "better-sqlite3-exact-string",
    },
    runtime: { id: "node", version: "22.18.0" },
  },
  ...
  "bun-sql-postgres": {
    database: { product: "postgres", version: "16.4", edition: "alpine", versionStatus: "measured" },
    driver: { id: "bun-sql", package: "@sqlbraid/bun-sql", version: "1.3.14", profile: "bun-sql-postgres-1.3.14" },
    runtime: { id: "bun", version: "1.3.14" },
  },
  "bun-sql-mysql": {
    database: { product: "mysql", version: "8.4.2", edition: "community", versionStatus: "measured" },
    driver: { id: "bun-sql", package: "@sqlbraid/bun-sql", version: "1.3.14", profile: "bun-sql-mysql-1.3.14" },
    runtime: { id: "bun", version: "1.3.14" },
  },
  "bun-sql-mariadb": {
    database: { product: "mariadb", version: "11.8.9", edition: "community", versionStatus: "measured" },
    driver: { id: "bun-sql", package: "@sqlbraid/bun-sql", version: "1.3.14", profile: "bun-sql-mariadb-1.3.14" },
    runtime: { id: "bun", version: "1.3.14" },
  },
  "bun-sql-sqlite": {
    database: { product: "sqlite", version: "3.53.0", edition: "bun-embedded", versionStatus: "measured" },
    driver: { id: "bun-sql", package: "@sqlbraid/bun-sql", version: "1.3.14", profile: "bun-sql-sqlite-1.3.14" },
    runtime: { id: "bun", version: "1.3.14" },
  },
  "d1-cloudflare-workerd-2026-07-30": {
    database: { product: "sqlite", edition: "Cloudflare D1 managed SQLite", versionStatus: "unknown" },
    driver: {
      id: "cloudflare-d1",
      package: "@sqlbraid/sqlite/d1",
      version: "1.20260730.1",
      profile: "d1-guarded-safe-integer",
    },
    runtime: { id: "workerd", version: "1.20260730.1" },
  },
  "sqlite-wasm-browser-3-53-4": {
    database: { product: "sqlite", version: "3.53.4", edition: "official SQLite WASM OO1", versionStatus: "measured" },
    driver: {
      id: "sqlite-wasm",
      package: "@sqlbraid/sqlite/wasm",
      version: "3.53.4-build1",
      profile: "sqlite-wasm-exact-string",
    },
    runtime: { id: "browser", version: "153.0.8010.12" },
  },
};
...
export const CERTIFICATION_CONTRACTS: Readonly<Record<string, CertificationTargetContract>> = {
  "postgres-pg-node-16-4": {
    expectedCapabilities: POSTGRES_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: POSTGRES_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["postgres-pg-node-16-4"],
  },
  "postgres-current": {
    expectedCapabilities: POSTGRES_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: POSTGRES_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["postgres-current"],
  },
  "postgres-pg-deno-2-9-3": {
    expectedCapabilities: POSTGRES_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: POSTGRES_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["postgres-pg-deno-2-9-3"],
  },
  "mysql-mysql2-node-8-4-2": {
    expectedCapabilities: MYSQL2_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: MYSQL2_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["mysql-mysql2-node-8-4-2"],
  },
  "mysql-mysql2-deno-2-9-3": {
    expectedCapabilities: MYSQL2_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: MYSQL2_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["mysql-mysql2-deno-2-9-3"],
  },
  "mariadb-connector-node-11-8-9": {
    expectedCapabilities: MARIADB_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: MARIADB_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["mariadb-connector-node-11-8-9"],
  },
  "oracle-oracledb-thin-node-23-9": {
    expectedCapabilities: ORACLE_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: ORACLE_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["oracle-oracledb-thin-node-23-9"],
  },
  "mssql-tedious-developer-node-2022-cu18": {
    expectedCapabilities: MSSQL_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: MSSQL_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["mssql-tedious-developer-node-2022-cu18"],
  },
  "sqlite-node-sqlite-node-22-18-0": {
  ...
  "better-sqlite3-node-22-18-0": {
    expectedCapabilities: SQLITE_EXPECTED_CAPABILITIES(),
    expectedTransactionOptions: SQLITE_TRANSACTION_OPTIONS(),
    tuple: CERTIFICATION_TARGET_TUPLES["better-sqlite3-node-22-18-0"],
  },
  "libsql-local-node-22-18-0": {
    expectedCapabilities: LIBSQL_EXPECTED_CAPABILITIES(),
    expectedTransactionOptions: LIBSQL_TRANSACTION_OPTIONS(),
    tuple: CERTIFICATION_TARGET_TUPLES["libsql-local-node-22-18-0"],
  },
  "bun-sql-postgres": {
    expectedCapabilities: BUN_EXPECTED_CAPABILITIES("postgres"),
    expectedTransactionOptions: BUN_EXPECTED_TRANSACTION_OPTIONS("postgres"),
    expectedGuardedCases: BUN_EXPECTED_GUARDED_CASES("postgres"),
    tuple: CERTIFICATION_TARGET_TUPLES["bun-sql-postgres"],
  },
  "bun-sql-mysql": {
    expectedCapabilities: BUN_EXPECTED_CAPABILITIES("mysql"),
    expectedTransactionOptions: BUN_EXPECTED_TRANSACTION_OPTIONS("mysql"),
    expectedGuardedCases: BUN_EXPECTED_GUARDED_CASES("mysql"),
    tuple: CERTIFICATION_TARGET_TUPLES["bun-sql-mysql"],
  },
  "bun-sql-mariadb": {
    expectedCapabilities: BUN_EXPECTED_CAPABILITIES("mariadb"),
    expectedTransactionOptions: BUN_EXPECTED_TRANSACTION_OPTIONS("mariadb"),
    expectedGuardedCases: BUN_EXPECTED_GUARDED_CASES("mariadb"),
    tuple: CERTIFICATION_TARGET_TUPLES["bun-sql-mariadb"],
  },
  "bun-sql-sqlite": {
    expectedCapabilities: BUN_EXPECTED_CAPABILITIES("sqlite"),
    expectedTransactionOptions: BUN_EXPECTED_TRANSACTION_OPTIONS("sqlite"),
    expectedGuardedCases: BUN_EXPECTED_GUARDED_CASES("sqlite"),
    tuple: CERTIFICATION_TARGET_TUPLES["bun-sql-sqlite"],
  },
  "d1-cloudflare-workerd-2026-07-30": {
    expectedCapabilities: D1_EXPECTED_CAPABILITIES,
    expectedTransactionOptions: D1_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["d1-cloudflare-workerd-2026-07-30"],
  },
  "sqlite-wasm-browser-3-53-4": {
    expectedCapabilities: WASM_EXPECTED_CAPABILITIES(),
    expectedTransactionOptions: WASM_EXPECTED_TRANSACTION_OPTIONS,
    tuple: CERTIFICATION_TARGET_TUPLES["sqlite-wasm-browser-3-53-4"],
  },
};
