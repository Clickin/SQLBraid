...
function createExecutor(state: SyntheticState, definitions: Map<string, Definition>, cancelUnsupported: boolean, environmentValue: DatabaseEnvironment): QueryExecutor {
  ...
  return {
    ...
    async begin(options) {
      if (options && optionKey(options) === "combination:serializable+readOnly") throw new UnsupportedFeatureError(optionKey(options), "BRAID_TX_OPTION_UNSUPPORTED", "synthetic option combination unsupported");
