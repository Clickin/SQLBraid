...
import {
  ...
  type OracleExecuteResultLike,
  type OraclePoolConnectionLike,
  type OraclePoolLike,
  type OracleResultSetLike,
...
async function ensureSchema(connection: OracleConnectionLike): Promise<void> {
  await exec(
    connection,
    `BEGIN EXECUTE IMMEDIATE 'CREATE TABLE ${TABLE} (id NUMBER PRIMARY KEY, value VARCHAR2(80))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;`,
  );
  await exec(
    connection,
    `BEGIN EXECUTE IMMEDIATE 'CREATE TABLE ${BULK_TABLE} (id NUMBER PRIMARY KEY, value VARCHAR2(80))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;`,
  );
  await exec(
    connection,
    `BEGIN EXECUTE IMMEDIATE 'CREATE SEQUENCE ${SEQUENCE} START WITH 1'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;`,
  );
  await exec(
    connection,
    `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_SCALAR (p_answer OUT NUMBER) IS BEGIN p_answer := 42; END;`,
  );
  await exec(connection, `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_NOOP IS BEGIN NULL; END;`);
  await exec(
    connection,
    `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_INOUT (p_value IN OUT NUMBER) IS BEGIN p_value := p_value + 1; END;`,
  );
  await exec(
    connection,
    `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_SETS (p_users OUT SYS_REFCURSOR, p_payments OUT SYS_REFCURSOR) IS l_implicit SYS_REFCURSOR; BEGIN OPEN p_users FOR SELECT 'user-1' AS USER_ID FROM dual; OPEN p_payments FOR SELECT 'payment-1' AS PAYMENT_ID FROM dual; OPEN l_implicit FOR SELECT 'summary-1' AS SUMMARY FROM dual; DBMS_SQL.RETURN_RESULT(l_implicit); END;`,
  );
  await exec(
    connection,
    `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_CURSOR (p_cursor OUT SYS_REFCURSOR) IS BEGIN OPEN p_cursor FOR SELECT 'cursor-1' AS CURSOR_ID FROM dual; END;`,
  );
  await exec(
    connection,
    `CREATE OR REPLACE PROCEDURE BRAID_RC3_CERT_LOB (p_payload OUT CLOB) IS BEGIN p_payload := TO_CLOB(RPAD('x', 4096, 'x')); END;`,
  );
}
...
function makeQueries(): CertificationFixture["queries"] {
  ...
  let factoryCalls = 0;
  const prepared = {
    command: (_input: unknown) => {
      factoryCalls += 1;
      return command(`INSERT INTO ${TABLE} (id, value) VALUES (${SEQUENCE}.NEXTVAL, 'prepared')`);
    },
    rows: (_input: unknown) => {
      factoryCalls += 1;
      return row("SELECT VALUE FROM (SELECT 'one' AS VALUE FROM dual UNION ALL SELECT 'two' FROM dual) ORDER BY VALUE");
    },
    input: "prepared",
...
function streamFixture(
  ...
  const initFailureQuery = row(
    "SELECT TO_CHAR(LEVEL) AS VALUE FROM dual CONNECT BY LEVEL <= 3 /* CERT_INIT_FAILURE */",
  );
  ...
  return {
    ...
    firstNextFailureQuery: row(
      "SELECT TO_CHAR(LEVEL) AS VALUE FROM dual CONNECT BY LEVEL <= 3 /* CERT_FIRST_NEXT_FAILURE */",
    ),
    firstNextFailure: faults.firstNextFailure,
    midStreamFailureQuery: row(
      "SELECT TO_CHAR(LEVEL) AS VALUE FROM dual CONNECT BY LEVEL <= 3 /* CERT_MID_STREAM_FAILURE */",
    ),
    midStreamFailure: faults.midStreamFailure,
    cleanupFailureQuery: row(
      "SELECT TO_CHAR(LEVEL) AS VALUE FROM dual CONNECT BY LEVEL <= 3 /* CERT_CLEANUP_FAILURE */",
    ),
...
function bulkFixture(
  db: Pick<Database, "bulk" | "all" | "execute">,
  nativeBulkCalls: () => number,
  ...
  return {
    ...
    middleFailure: async () => {
      ...
      const rows = await db.all<{ readonly id: unknown; readonly value: unknown }>(
...
export async function createOracleOracledbTarget(
  options: OracleCertificationConnectionOptions,
): Promise<CertificationTarget & { close(): Promise<void> }> {
  const pool = (await oracledb.createPool({
    ...
    poolMax: 4,
    poolIncrement: 1,
  })) as unknown as OraclePoolLike & { close(): Promise<void> };
  const target: CertificationTarget & { close(): Promise<void> } = {
    ...
    async createFixture(): Promise<CertificationFixture> {
      ...
      const instrumentStreamConnection = (
        candidate: OracleConnectionLike,
        countSideEffects: boolean,
        ...
        const instrumented: OracleConnectionLike = {
          execute: async (text: string, binds?: unknown, executeOptions?: unknown): Promise<unknown> => {
            ...
            let result = (
              executeOptions === undefined ? await execute(text, binds) : await execute(text, binds, executeOptions)
            ) as OracleExecuteResultLike;
            const wrapLob = (value: unknown): unknown => {
              if (
                value === null ||
                typeof value !== "object" ||
                typeof (value as { readonly destroy?: unknown }).destroy !== "function"
              )
                return value;
      ...
      if (nativeExecuteMany) {
        directConnection.executeMany = async (
          statement: string,
          binds: unknown,
            ...
            close: async (closeOptions) => {
              ...
              const closeNative =
                closeOptions === undefined
                  ...
                  : (): Promise<void> => Promise.resolve(nativeClose(closeOptions));
              if (releaseFailure) {
                await closeNative();
      ...
      sideEffects = 0;
      executeStarts = 0;
      const unsupportedTransaction = async (
        transactionOptions: Parameters<NonNullable<Database["tx"]>>[0],
      ): Promise<void> => {
        const probeConnection = (await oracledb.getConnection({
          user: options.user,
          password: options.password,
          connectString: options.connectionUri,
        })) as unknown as OracleConnectionLike;
      ...
      const unsupported = {
        CALL006: {
          ...
          run: () => direct.call(makeQueries().routines!.returnValue!),
          sideEffects: () => sideEffects,
        },
        TX020: {
          feature: "transaction.isolation.read-uncommitted",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "read-uncommitted" }),
          sideEffects: () => sideEffects,
        },
        TX024: {
          feature: "transaction.isolation.repeatable-read",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "repeatable-read" }),
          sideEffects: () => sideEffects,
        },
        TX022: {
          feature: "combination:read-committed+readOnly",
          expectedErrorFeature: "transaction.isolation.read-committed",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "read-committed", readOnly: true }),
          sideEffects: () => sideEffects,
        },
        TX027: {
          feature: "combination:read-uncommitted+readOnly",
          expectedErrorFeature: "transaction.isolation.read-uncommitted",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "read-uncommitted", readOnly: true }),
          sideEffects: () => sideEffects,
        },
        TX028: {
          feature: "combination:serializable+readOnly",
          expectedErrorFeature: "transaction.isolation.serializable",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "serializable", readOnly: true }),
          sideEffects: () => sideEffects,
        },
        TX031: {
          feature: "combination:repeatable-read+readOnly",
          expectedErrorFeature: "transaction.isolation.repeatable-read",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "repeatable-read", readOnly: true }),
          sideEffects: () => sideEffects,
        },
        TX029: {
          feature: "combination:read-uncommitted+readWrite",
          expectedErrorFeature: "transaction.isolation.read-uncommitted",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "read-uncommitted", readOnly: false }),
          sideEffects: () => sideEffects,
        },
        TX032: {
          feature: "combination:repeatable-read+readWrite",
          expectedErrorFeature: "transaction.isolation.repeatable-read",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "repeatable-read", readOnly: false }),
          sideEffects: () => sideEffects,
        },
        TX030: {
          feature: "combination:read-committed+readWrite",
          expectedErrorFeature: "transaction.isolation.read-committed",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "read-committed", readOnly: false }),
          sideEffects: () => sideEffects,
        },
        TX033: {
          feature: "combination:serializable+readWrite",
          expectedErrorFeature: "transaction.isolation.serializable",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED" as const,
          run: () => unsupportedTransaction({ isolation: "serializable", readOnly: false }),
          sideEffects: () => sideEffects,
        },
      };
      const metrics = {
        snapshot: (): ResourceSnapshot => ({
          borrowedLeases: pooledConnections(),
          cleanupBalance: pooledConnections(),
        }),
        sideEffects: () => sideEffects,
        mutationSentinel: async (): Promise<unknown> =>
          direct.one(row(`SELECT value AS VALUE FROM ${TABLE} WHERE id = 999999`)),
        ...
        readOnlyWrite: async (): Promise<void> => {
          ...
          await pooled.tx({ readOnly: false }, (tx) =>
            tx.execute(command(`INSERT INTO ${TABLE} (id, value) VALUES (999998, 'rw')`)),
          );
          await assert.rejects(() =>
            pooled.tx({ readOnly: true }, (tx) =>
              tx.execute(command(`INSERT INTO ${TABLE} (id, value) VALUES (999997, 'ro')`)),
            ),
          );
          const after = await direct.one(row(`SELECT COUNT(*) AS VALUE FROM ${TABLE} WHERE id = 999998`));
          assert.equal(
            Number((after as { readonly VALUE: number | string }).VALUE),
            Number((before as { readonly VALUE: number | string }).VALUE) + 1,
          );
        ...
        routineCleanup: async (query = makeQueries().routines!.lob): Promise<void> => {
          ...
          const nativeResult = (await directConnection.execute("BEGIN BRAID_RC3_CERT_LOB(:payload); END;", {
          ...
          const lob = (nativeResult.outBinds as { readonly payload?: unknown } | undefined)?.payload as
            ...
            | undefined;
          if (lob === undefined || typeof lob.destroy !== "function" || typeof lob.once !== "function")
            throw new Error("Oracle native LOB resource missing.");
      ...
      return {
        ...
        stream: streamFixture(pooled, faults, streamCounters, async () => {
          await pooled.one(makeQueries().identity);
        }),
        ...
        representationUnsupported: {
          "numeric.exact-integer": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(`SELECT CAST(9007199254740991 AS NUMBER(19,0)) AS "value" FROM dual`),
              );
              if (typeof result.value !== "string")
                throw new Error("Oracle exact integer did not expose its string representation.");
            },
          },
          "numeric.bind-exact": {
            prove: async () => {
              const result = await direct.one(
                ...
                }>`SELECT CAST(${sql.bind("12.34", oracleParameter.varchar2())} AS NUMBER(10,2)) AS "value" FROM dual`,
              );
          ...
          "data.json-lossless-text": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(
                  `SELECT JSON_OBJECT('ok' VALUE 1 RETURNING VARCHAR2(100)) AS "value" FROM dual`,
                ),
              );
              if (typeof result.value !== "string")
                throw new Error("Oracle JSON lossless text proof did not return serialized JSON.");
            },
          },
          "data.oracle-object": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(`SELECT SYS.ANYDATA.ConvertNumber(1) AS "value" FROM dual`),
              );
              if (result.value === null || typeof result.value !== "object")
                throw new Error("Oracle object proof did not return a native object.");
            },
          },
          "data.oracle-collection": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(`SELECT SYS.ODCINUMBERLIST(1, 2) AS "value" FROM dual`),
              );
              if (result.value === null || typeof result.value !== "object")
                throw new Error("Oracle collection proof did not return a native collection.");
            },
          },
          "data.vector": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(`SELECT TO_VECTOR('[1,2]') AS "value" FROM dual`),
              );
              if (result.value === null || typeof result.value !== "object")
                throw new Error("Oracle vector proof did not return a native vector.");
            },
          },
          "data.temporal-lossless": {
            prove: async () => {
              const result = await direct.one(
                row<{ readonly value: unknown }>(
                  `SELECT CAST(TIMESTAMP '2026-09-14 12:34:56.789123' AS TIMESTAMP) AS "value" FROM dual`,
                ),
              );
              if (!(result.value instanceof Date) || result.value.getUTCMilliseconds() !== 789)
                throw new Error("Oracle temporal result did not expose the declared precision loss.");
            },
          },
        },
        guarded: {
          "statement.cancel": {
            prove: async () => {
              ...
              const pending = direct.execute(command("BEGIN DBMS_SESSION.SLEEP(60); END;"), {
          ...
          "data.temporal-native": {
            prove: async () => {
              const result = await direct.one(
                row("SELECT CAST(TIMESTAMP '2026-09-14 12:34:56.789' AS TIMESTAMP) AS VALUE FROM dual"),
              );
              if (!((result as { readonly VALUE?: unknown }).VALUE instanceof Date))
                throw new Error("Oracle temporal native guard did not return Date.");
            },
          },
          "metadata.command-safe": {
            prove: async () => {
              const result = await direct.execute(
                command(`INSERT INTO ${BULK_TABLE} (id, value) VALUES (999999, 'metadata')`),
              );
              if ((result as { readonly command: { readonly affectedRows: number } }).command.affectedRows !== 1)
                throw new Error("Oracle command metadata guard failed.");
