...
import {
  BUN_EXPECTED_CAPABILITIES,
  BUN_EXPECTED_GUARDED_CASES,
  BUN_EXPECTED_TRANSACTION_OPTIONS,
} from "../contracts.js";
...
function exactIntegerBindQuery(tag: SqlTag, dialect: BunSqlDialect): RowQuery<unknown> {
  ...
  if (dialect === "mysql" || dialect === "mariadb")
    return tag.rows`SELECT CAST(${"9007199254740993"} AS SIGNED) AS value`;
  return tag.rows`SELECT CAST(${"9007199254740993"} AS INTEGER) AS value`;
}
...
const temporalExpectedIso = (dialect: BunSqlDialect): string =>
  dialect === "postgres" ? "2026-09-14T12:34:56.789Z" : temporalWallClockIso;
...
function exactDecimalQuery(tag: SqlTag, dialect: BunSqlDialect): RowQuery<unknown> {
  if (dialect === "mysql" || dialect === "mariadb")
    return tag.rows`SELECT CAST('12345678901234567890.123456789' AS DECIMAL(30, 9)) AS value`;
...
function exactDecimalBindQuery(tag: SqlTag, dialect: BunSqlDialect): RowQuery<unknown> {
  if (dialect === "postgres")
    return tag.rows`SELECT CAST(CAST(${"12345678901234567890.123456789"} AS NUMERIC) AS TEXT) AS value`;
  if (dialect === "mysql" || dialect === "mariadb")
    return tag.rows`SELECT CAST(CAST(${"12345678901234567890.123456789"} AS DECIMAL(30, 9)) AS CHAR) AS value`;
...
async function nativeRow(
  ...
  if (!Array.isArray(rows) || rows.length !== 1 || rows[0] === undefined)
    throw new Error("Bun.SQL native proof did not return exactly one row.");
  return rows[0];
}
...
function optionFor(id: string): TransactionOptions {
  const options: Record<string, TransactionOptions> = {
    TX020: { isolation: "read-uncommitted" },
    TX021: { readOnly: true },
    TX022: { isolation: "read-committed", readOnly: true },
    TX023: { isolation: "read-committed" },
    TX024: { isolation: "repeatable-read" },
    TX026: { readOnly: false },
    TX027: { isolation: "read-uncommitted", readOnly: true },
    TX028: { isolation: "serializable", readOnly: true },
    TX029: { isolation: "read-uncommitted", readOnly: false },
    TX030: { isolation: "read-committed", readOnly: false },
    TX031: { isolation: "repeatable-read", readOnly: true },
    TX032: { isolation: "repeatable-read", readOnly: false },
    TX033: { isolation: "serializable", readOnly: false },
  };
...
function nativeErrorHas(error: unknown, key: "errno" | "code", expected: number | string): boolean {
  if (error !== null && typeof error === "object") {
    ...
    if (error instanceof AggregateError && error.errors.some((entry) => nativeErrorHas(entry, key, expected)))
      return true;
...
async function proveTransactionOption(
  ...
  if (dialect === "postgres") {
    ...
    const postgresReadUncommittedAlias =
    ...
    if (settings.value !== isolation && !postgresReadUncommittedAlias) {
      throw new Error(
        `Bun.SQL ${optionKey(options)} observed isolation=${String(settings.value)} instead of ${isolation}.`,
      );
    }
    if (options.readOnly !== undefined && settings.read_only !== options.readOnly) {
      throw new Error(
        `Bun.SQL ${optionKey(options)} observed read_only=${String(settings.read_only)} instead of ${String(options.readOnly)}.`,
      );
    }
    return;
  }
...
async function createFixture(options: BunCertificationTargetOptions): Promise<CertificationFixture> {
  ...
  await client.unsafe(
    `CREATE TABLE ${quoteIdentifier(dialect, table)} (${idColumn}, value TEXT CHECK (value <> 'middle-failure'))`,
    [],
  );
  await client.unsafe(
    `CREATE TABLE ${quoteIdentifier(dialect, sentinelTable)} (id INTEGER PRIMARY KEY, value TEXT, marker INTEGER NOT NULL)`,
    [],
  );
  await client.unsafe(
    `INSERT INTO ${quoteIdentifier(dialect, sentinelTable)} (id, value, marker) VALUES (1, '${sentinelValue.replaceAll("'", "''")}', 7)`,
    [],
  );
  ...
  const expectedSpecial: Record<string, unknown> = {
    ...
    RES006: { value_text: "hello" },
    RES007: { value_empty: "" },
    RES008: { value_null: null },
    RES009: { value_unicode: "안녕하세요" },
    RES010: { value_binary: dialect === "postgres" ? Buffer.from([0, 255, 16]) : new Uint8Array([0, 255, 16]) },
    RES011: { value: "second" },
  };
  let preparedCalls = 0;
  const queries = {
    ...
    identity:
      dialect === "postgres"
        ? tag.rows<{ readonly id: string }>`SELECT CAST(pg_backend_pid() AS TEXT) AS id`
        : dialect === "mysql" || dialect === "mariadb"
          ? tag.rows<{ readonly id: string }>`SELECT CAST(CONNECTION_ID() AS CHAR) AS id`
          : tag.rows<{ readonly id: string }>`SELECT 'bun-sqlite-direct' AS id`,
    ...
    prepared: {
      ...
      rows: (input: unknown): RowQuery<unknown> => {
        preparedCalls += 1;
        return tag.rows`SELECT ${input} AS value UNION ALL SELECT 'two' AS value`;
      },
      input: "one",
    ...
    fidelity: {
      ...
      temporal:
        dialect === "sqlite" ? tag.rows`SELECT ${"2026-09-14T12:34:56.789Z"} AS value` : temporalQuery(tag, dialect),
    ...
    expected: {
      ...
      specialErrors:
        dialect === "mysql" || dialect === "mariadb" ? { RES010: { code: "BRAID_RESULT_EXACTNESS" } } : undefined,
  ...
  const streamIds: readonly CertificationCaseId[] = [
    "PRE003",
    "PRE004",
    "PRE005",
    "PRE011",
  ...
  unsupported.STR006 = makeProbe(
    "statement.cancel",
    () => db.execute(queries.one, { signal: new AbortController().signal }),
    () => counters.sideEffects,
    "BRAID_CANCEL_UNSUPPORTED",
  );
  unsupported.STR010 = makeProbe(
    "statement.cancel",
    () => db.execute(queries.one, { signal: new AbortController().signal }),
    () => counters.sideEffects,
    "BRAID_CANCEL_UNSUPPORTED",
  ...
  const routineCases: readonly [CertificationCaseId, string, CallQuery][] = [
    ["CALL001", "routine.call", queries.routines.call],
    ["CALL002", "routine.out", queries.routines.out],
    ["CALL003", "routine.inout", queries.routines.inout],
    ["CALL004", "routine.result-sets", queries.routines.resultSets],
    ["CALL005", "routine.out-cursor", queries.routines.cursor],
    ["CALL006", "routine.return-value", queries.routines.returnValue],
  ];
  for (const [id, feature, query] of routineCases)
    unsupported[id] = makeProbe(
      feature,
      () => db.call(query),
      () => counters.sideEffects,
      ...
      const rejectionFeature =
        ...
        (transactionOptions.readOnly !== undefined && transactionOptions.isolation === "serializable")
          ...
          : transactionOptions.isolation === undefined
            ? "transaction.read-only"
            : `transaction.isolation.${transactionOptions.isolation}`;
      unsupported[id] = makeProbe(
        rejectionFeature,
        () =>
          db.tx(transactionOptions, async (tx) => {
            await tx.one(queries.identity);
          }),
        () => counters.sideEffects,
  ...
  const approximateSpecialProof = async (): Promise<void> => {
    if (dialect === "postgres") {
      ...
      if (!Number.isNaN(row.nan) || row.pos_inf !== Infinity || row.neg_inf !== -Infinity)
        throw new Error("Bun.SQL PostgreSQL special-float guard failed.");
      return;
    }
    if (dialect === "sqlite") {
      const value = await db.one(approximateSpecialQuery(tag, dialect));
      const row = value as { readonly pos_inf?: unknown; readonly neg_inf?: unknown };
      if (row.pos_inf !== Infinity || row.neg_inf !== -Infinity)
        throw new Error("Bun.SQL SQLite special-float guard failed.");
      return;
    }
    let value: Record<string, unknown>;
    try {
      value = await nativeRow(
        client,
        "SELECT CAST(? AS DOUBLE) AS nan, CAST(? AS DOUBLE) AS pos_inf, CAST(? AS DOUBLE) AS neg_inf",
      ...
      if (
        ...
        (error as { readonly code?: unknown }).code !== "ERR_MYSQL_SERVER_ERROR"
      )
        throw error;
      return;
    }
    if (
      value.nan !== null ||
      typeof value.pos_inf !== "number" ||
      typeof value.neg_inf !== "number" ||
      !Number.isFinite(value.pos_inf) ||
      !Number.isFinite(value.neg_inf)
    ) {
  ...
  const guarded: NonNullable<CertificationFixture["guarded"]> = {
    "numeric.exact-integer": {
      prove: async () => {
        ...
        if ((value as { readonly value?: unknown }).value !== "9007199254740993")
          throw new Error("Bun.SQL exact-integer canonical guard failed.");
      },
    },
    "numeric.approximate-float": {
      prove: async () => {
        const value = await db.one(approximateFloatQuery(tag, dialect));
        if (!Object.is((value as { readonly value?: unknown }).value, 1.5))
          throw new Error("Bun.SQL float guard failed.");
      },
    },
    "numeric.bind-exact": {
      prove: async () => {
        ...
        if ((value as { readonly value?: unknown }).value !== "9007199254740993")
          throw new Error("Bun.SQL exact bind canonical guard failed.");
      },
    },
    ...(dialect === "postgres" || dialect === "sqlite"
      ? { "numeric.approximate-special": { prove: approximateSpecialProof } }
      : {}),
    ...
    ...(dialect !== "postgres"
      ? {
          "result.rows": {
            prove: async () => {
              const result = await db.all(queries.one);
              if (result.length !== 1) throw new Error("Bun.SQL row-kind proof did not return one row.");
            },
          },
          "result.command": {
            prove: async () => {
              const result = await db.execute(queries.command);
              if (result.kind !== "command" || result.command.affectedRows !== 1)
                throw new Error("Bun.SQL command-kind proof did not return one affected row.");
              await reset();
            },
          },
        }
    ...
    "metadata.command-safe": {
      prove: async () => {
        const result = await db.execute(queries.command);
        if (!Number.isSafeInteger(result.command.affectedRows))
          throw new Error("Bun.SQL command metadata was not safe.");
        await reset();
      },
    },
    "execution.bulk-fidelity": {
      prove: async () => {
        const result = await db.bulk(
          ["proof-one", "proof-two"],
          (input) => tag.command`INSERT INTO ${tableSql} (value) VALUES (${input})`,
        );
        if (result.inputCount !== 2 || result.affectedRows !== 2)
          throw new Error("Bun.SQL bulk fidelity proof failed.");
        await reset();
      },
    },
  };
  if (dialect === "postgres" || dialect === "mysql") {
    guarded["data.json-parsed"] = {
      prove: async () => {
        const row = await db.one(
          tag.rows`SELECT ${tag.raw(dialect === "postgres" ? "'{\"value\": 1}'::jsonb" : "JSON_OBJECT('value', 1)")} AS value`,
        );
        if (row === null || typeof (row as { readonly value?: unknown }).value !== "object")
          throw new Error("Bun.SQL JSON parser guard failed.");
      },
    };
  }
  ...
  if (dialect === "mysql" || dialect === "mariadb" || dialect === "sqlite") {
    representationUnsupported["numeric.exact-decimal"] = {
      prove: async () => {
        let value: unknown;
        try {
          value = (await db.one(exactDecimalQuery(tag, dialect))) as { readonly value?: unknown };
        } catch (error) {
          if (!(error instanceof Error) || (error as { readonly code?: unknown }).code !== "BRAID_RESULT_EXACTNESS")
            throw error;
          return;
        }
        if (dialect === "sqlite") {
          if (typeof (value as { readonly value?: unknown }).value !== "number")
            throw new Error("Bun.SQL SQLite decimal representation changed.");
          return;
        }
  ...
  if (dialect === "mysql" || dialect === "mariadb" || dialect === "sqlite") {
    representationUnsupported["data.json-parsed"] = {
      prove: async () => {
        const row = await db.one(jsonQuery(tag, dialect));
        if (typeof (row as { readonly value?: unknown }).value !== "string")
          throw new Error(`Bun.SQL ${dialect} JSON representation changed.`);
      },
    };
  }
  if (dialect === "postgres" || dialect === "mysql" || dialect === "mariadb") {
    representationUnsupported["data.json-lossless-text"] = {
      prove: async () => {
        const row = await db.one(jsonQuery(tag, dialect));
        if (
          (row as { readonly value?: unknown }).value === null ||
          typeof (row as { readonly value?: unknown }).value !== "object"
        ) {
          throw new Error(`Bun.SQL ${dialect} JSON parser representation changed.`);
        }
      },
    };
    representationUnsupported["data.temporal-lossless"] = {
      prove: async () => {
        const row = await db.one(temporalQuery(tag, dialect));
        assertExactDate(
          (row as { readonly value?: unknown }).value,
          temporalExpectedIso(dialect),
          `${dialect} temporal`,
        );
      },
    };
  ...
  if (dialect === "mysql" || dialect === "mariadb") {
    representationUnsupported["data.binary"] = {
      prove: async () => {
        try {
  ...
  if (dialect === "postgres" || dialect === "mysql" || dialect === "mariadb") {
    for (const isolation of ["read-uncommitted", "read-committed", "repeatable-read", "serializable"] as const) {
      guarded[`transaction.isolation.${isolation}`] = {
        prove: () =>
          proveTransactionOption(
            db,
  ...
  if (dialect === "postgres") {
    guarded["transaction.read-only"] = {
      prove: async () => {
        if (dialect === "postgres") {
          const settings = (await db.tx({ readOnly: true }, async (tx) => tx.one(settingQuery(tag, dialect)))) as {
            readonly read_only?: unknown;
          };
          if (settings.read_only !== true)
            throw new Error("Bun.SQL PostgreSQL read-only setting was not active inside the transaction.");
        }
        let caught: unknown;
        try {
  ...
  const mutationSentinel = async (): Promise<unknown> => {
    const rows = await client.unsafe<readonly Record<string, unknown>[]>(
      `SELECT id, value, marker FROM ${quoteIdentifier(dialect, sentinelTable)} ORDER BY id`,
      [],
    );
    if (
      !Array.isArray(rows) ||
      rows.length !== 1 ||
      String(rows[0]?.id) !== "1" ||
      rows[0]?.value !== sentinelValue ||
      Number(rows[0]?.marker) !== 7
    ) {
  ...
  const transactionCleanup = async (): Promise<void> => {
    ...
    const identitySql =
      dialect === "postgres" ? "SELECT pg_backend_pid()::text AS id" : "SELECT CAST(CONNECTION_ID() AS CHAR) AS id";
    counters.failRollbackNative = true;
    counters.rollbackError = undefined;
    let caught: unknown;
    try {
      const before =
        peer === undefined ? undefined : (await peer.unsafe<readonly { readonly id: string }[]>(identitySql))[0]?.id;
      try {
        await db.tx(async () => {
          throw primary;
        });
    ...
    if (counters.borrowed !== 0 || counters.cleanupBalance !== 0) {
      throw new Error(
        `Bun.SQL transaction cleanup leaked resources: borrowed=${counters.borrowed}, balance=${counters.cleanupBalance}.`,
      );
    }
  };
  ...
  const fixture = {
    ...
    bulk: {
      ...
      middleFailure: async () => {
        ...
        const observedRows = (await db.all(queries.transaction!.visible)).map(
          (row) => (row as { readonly value?: unknown }).value,
        );
        const expectedRows = ["middle-one"];
        if (JSON.stringify(observedRows) !== JSON.stringify(expectedRows))
          throw new Error("Bun.SQL bulk middle-item durability changed.");
    ...
    close: async () => {
      ...
      try {
        await client.unsafe(`DROP TABLE ${quoteIdentifier(dialect, table)}`, []);
      } finally {
        try {
          await client.unsafe(`DROP TABLE ${quoteIdentifier(dialect, sentinelTable)}`, []);
        }
