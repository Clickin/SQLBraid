...
import {
  ...
  type TediousPoolConnectionLike,
...
function trackedConnection(raw: Connection, stats: Stats): TediousPoolConnectionLike {
  ...
  return {
    execSql(request) {
      stats.requests += 1;
      return physical.execSql(request);
    },
    prepare(request) {
      stats.requests += 1;
      return physical.prepare!(request);
    },
    execute(request, parameters) {
      stats.requests += 1;
      return physical.execute!(request, parameters);
    },
    unprepare(request) {
      stats.requests += 1;
      return physical.unprepare!(request);
    },
    callProcedure(request) {
      stats.requests += 1;
      return physical.callProcedure!(request);
    },
    beginTransaction(callback, name, isolation) {
      stats.requests += 1;
      return physical.beginTransaction(callback, name, isolation);
    },
    commitTransaction(callback) {
      stats.requests += 1;
      return physical.commitTransaction(callback);
    },
    ...
    saveTransaction(callback, name) {
      stats.requests += 1;
      return physical.saveTransaction(callback, name);
    },
    cancel() {
      stats.requests += 1;
      return physical.cancel?.();
    },
    close() {
...
async function executeSetup(connection: TediousConnectionLike): Promise<void> {
  ...
  await setup.execute(
    sql`CREATE PROCEDURE ${sql.raw(PROCEDURES[0])} @answer int OUTPUT AS BEGIN SET NOCOUNT ON; SET @answer = 42; END`,
  );
  await setup.execute(
    sql`CREATE PROCEDURE ${sql.raw(PROCEDURES[1])} @answer int OUTPUT, @delta int AS BEGIN SET NOCOUNT ON; SET @answer = @answer + @delta; END`,
  );
  await setup.execute(
    sql`CREATE PROCEDURE ${sql.raw(PROCEDURES[2])} @minimum int AS BEGIN SET NOCOUNT ON; SELECT @minimum AS user_id; SELECT CONCAT(N'payment-', @minimum) AS payment_id; END`,
  );
  await setup.execute(
    sql`CREATE PROCEDURE ${sql.raw(PROCEDURES[3])} @value int AS BEGIN SET NOCOUNT ON; RETURN 17; END`,
  );
  await setup.execute(
    sql`CREATE PROCEDURE ${sql.raw(PROCEDURES[4])} AS BEGIN SET NOCOUNT ON; SELECT REPLICATE(CAST(N'x' AS nvarchar(max)), 4096) AS payload; END`,
  );
}
...
function queries(): CertificationFixture["queries"] {
  ...
  const routines = {
    ...
    out: sql.call({
      procedure: { name: PROCEDURES[0], parameterNames: ["answer"] },
    })`${sql.out("answer", mssqlParameter.int())}`,
    inout: sql.call({
      procedure: { name: PROCEDURES[1], parameterNames: ["answer", "delta"] },
    })`${sql.inOut("answer", 1, mssqlParameter.int())}, ${41}`,
    resultSets: sql.call({ procedure: { name: PROCEDURES[2], parameterNames: ["minimum"] } })`${1}`,
    cursor: sql.call({
      procedure: { name: "dbo.braid_cert_mssql_cursor", parameterNames: ["cursor"] },
  ...
  const prepared = {
    command: (input: unknown) => {
      factoryCalls += 1;
      return sql.command`UPDATE ${sql.raw(TABLE)} SET value = ${String(input)} WHERE id = 1`;
    },
    rows: (input: unknown) => {
      factoryCalls += 1;
      return sql.rows`SELECT value FROM ${sql.raw(TABLE)} WHERE id >= ${Number(input)} ORDER BY id`;
    },
    input: 1,
  ...
  return {
    ...
    expected: {
      ...
      special: {
        ...
        CALL004: {
          output: {},
          resultSets: [{ rows: [{ user_id: "1" }] }, { rows: [{ payment_id: "payment-1" }] }],
          returnValue: 0,
        },
    ...
    async createFixture(): Promise<CertificationFixture> {
          ...
          return {
            ...
            stream<Row>(
              ...
              if (sourceSql?.includes("CERT_STREAM_CLEANUP_FAILURE")) {
                stats.streamCleanupFailure = Object.assign(new Error("mssql-cert-stream-cleanup-failure"), {
                  code: "EREQUEST",
                });
              }
      ...
      const reset = async (): Promise<void> => {
        ...
        await database.execute(
          sql`UPDATE ${sql.raw(TABLE)} SET value = CASE id WHEN 1 THEN N'one' WHEN 2 THEN N'two' END WHERE id IN (1, 2)`,
        );
      ...
      const unsupported: NonNullable<CertificationFixture["unsupported"]> = {
        CALL005: {
          ...
          expectedCode: "BRAID_CALL_CURSOR_UNSUPPORTED",
          run: () => database.call(fixtureQueries.routines!.cursor!),
          sideEffects: () => stats.requests,
        },
      };
      const txOptions: Array<
        [
          string,
          string,
          {
      ...
      > = [
        ["TX021", "transaction.read-only", { readOnly: true }],
        ["TX022", "transaction.read-only", { isolation: "read-committed", readOnly: true }],
        ["TX026", "transaction.read-only", { readOnly: false }],
        ["TX027", "transaction.read-only", { isolation: "read-uncommitted", readOnly: true }],
        ["TX028", "transaction.read-only", { isolation: "serializable", readOnly: true }],
        ["TX029", "transaction.read-only", { isolation: "read-uncommitted", readOnly: false }],
        ["TX030", "transaction.read-only", { isolation: "read-committed", readOnly: false }],
        ["TX031", "transaction.read-only", { isolation: "repeatable-read", readOnly: true }],
        ["TX032", "transaction.read-only", { isolation: "repeatable-read", readOnly: false }],
        ["TX033", "transaction.read-only", { isolation: "serializable", readOnly: false }],
      ];
      for (const [id, feature, options] of txOptions) {
        unsupported[id as keyof typeof unsupported] = {
          feature,
          expectedErrorFeature: "transaction.read-only",
          expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
          run: () => database.tx(options, async () => undefined),
          sideEffects: () => stats.requests,
        };
      }
      ...
      const bulk: BulkConformanceFixture<unknown> = {
        ...
        factory: (input) =>
          sql.command`INSERT INTO ${sql.raw(TABLE)} (id, value) VALUES (${sql.bind((input as { id: number }).id, mssqlParameter.int())}, ${sql.bind((input as { value: string }).value, mssqlParameter.nvarchar(80))})`,
        ...
        middleFailure: async () => {
          ...
          const rows = await database.all<{ readonly id: unknown; readonly value: unknown }>(
            sql.rows`SELECT id, value FROM ${sql.raw(TABLE)} WHERE id IN (12, 13) ORDER BY id`,
          );
      ...
      const metrics = {
        snapshot: (): ResourceSnapshot => ({
          borrowedLeases: stats.leased ? 1 : 0,
          cleanupBalance: stats.leased ? 1 : 0,
        }),
        sideEffects: () => stats.requests,
        ...
        readOnlyWrite: async (): Promise<void> => {
          ...
          await assert.rejects(
            () =>
              database.tx({ readOnly: true }, (tx) =>
                tx.execute(sql`INSERT INTO ${sql.raw(TABLE)} (id, value) VALUES (999997, N'ro')`),
              ),
            (error: unknown) => (error as { readonly code?: string }).code === "BRAID_TX_OPTION_UNSUPPORTED",
          );
          const row = await database.maybeOne<{ readonly value?: unknown }>(
            sql.rows`SELECT value FROM ${sql.raw(TABLE)} WHERE id = 999997`,
          );
      ...
      return {
        ...
        representationUnsupported: {
          "numeric.exact-decimal": {
            prove: async () => {
              await assert.rejects(
                () =>
                  database.one(
                    sql.rows`SELECT CAST(${sql.bind("12.34", mssqlParameter.nvarchar(20))} AS decimal(10, 2)) AS value`,
                  ),
          ...
          "numeric.aggregate": {
            prove: async () => {
              await assert.rejects(
                () =>
                  database.one(
                    sql.rows`SELECT SUM(CAST(${sql.bind("12.34", mssqlParameter.nvarchar(20))} AS decimal(10, 2))) AS value`,
                  ),
                (error: unknown) => (error as { readonly code?: string }).code === "BRAID_RESULT_EXACTNESS",
              );
            },
          ...
          "data.json-parsed": {
            prove: async () => {
              const row = await database.one<{ readonly value: unknown }>(
                sql.rows`SELECT CAST(N'{"ok":true}' AS nvarchar(max)) AS value`,
              );
              if (typeof row.value !== "string")
                throw new Error("MSSQL JSON result is not the declared text representation.");
            },
          },
          "data.sql-variant": {
            prove: async () => {
              const row = await database.one<{ readonly value: unknown }>(
                sql.rows`SELECT CAST(1 AS sql_variant) AS value`,
              );
          ...
          "data.temporal-lossless": {
            prove: async () => {
              const row = await database.one<{ readonly value: unknown }>(
                sql.rows`SELECT DATETIME2FROMPARTS(2026, 9, 14, 12, 34, 56, 7891234, 7) AS value`,
              );
              if (!(row.value instanceof Date) || row.value.getUTCMilliseconds() !== 789)
                throw new Error("MSSQL temporal result did not expose driver precision loss.");
            },
          },
        },
        guarded: {
          "numeric.bind-exact": {
            prove: async () => {
              ...
              const row = await database.one(
                ...
                }>`SELECT CONVERT(varchar(64), CAST(${sql.bind("12.34", mssqlParameter.nvarchar(20))} AS decimal(10, 2))) AS value`,
              );
          ...
          "metadata.command-safe": {
            prove: async () => {
              const result = await database.execute(fixtureQueries.command);
              if (result.command.affectedRows !== 1) throw new Error("safe count guard failed");
            },
          },
          "data.temporal-native": {
            prove: async () => {
              const row = await database.one(
                ...
                }>`SELECT DATETIME2FROMPARTS(2026, 9, 14, 12, 34, 56, 1234567, 7) AS value`,
              );
              if (!(row.value instanceof Date)) throw new Error("temporal guard failed");
            },
          },
        },
        close: async () => {
          await close(activeRaw);
        },
      };
    },
  };
...
export {
  expectedCapabilities as mssqlTediousExpectedCapabilities,
  expectedTransactionOptions as mssqlTediousExpectedTransactionOptions,
};
