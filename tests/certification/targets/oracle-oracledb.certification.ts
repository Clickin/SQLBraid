...
test("Oracle node-oracledb passes the complete independent certification matrix", { timeout: 1_800_000 }, async () => {
  if (!SOURCE_SHA || !isSourceSha(SOURCE_SHA))
  ...
  try {
    artifact = await certifyTarget(target, {
    ...
    await writeFile(
      ...
      [
        ...
        "",
        "```text",
        ...Object.values(artifact.cases).map(
          (result) => `${result.status}\t${result.name}${result.code ? `\t${result.code}` : ""}`,
        ),
        "```",
        "",
