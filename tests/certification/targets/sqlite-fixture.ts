...
export {
  SQLITE_EXPECTED_CAPABILITIES as sqliteCapabilities,
  LIBSQL_EXPECTED_CAPABILITIES as libsqlCapabilities,
  SQLITE_TRANSACTION_OPTIONS as sqliteTransactionOptions,
  LIBSQL_TRANSACTION_OPTIONS as libsqlTransactionOptions,
} from "../contracts.js";
...
function makeUnsupported(
  ...
  const runStream = async () => {
    for await (const row of db.stream(queries.one)) void row;
  };
  const runCall = async (query: CallQuery) => {
    await db.call(query);
  };
  const runRows = async (query: AnyRowQuery) => {
    await db.all(query);
  };
  const probes = {
    ...(targetKind === "libsql"
      ? {
          SES001: {
            feature: "session.pinned",
            expectedCode: "BRAID_SESSION_UNSUPPORTED",
    ...
    TX020: {
      ...
      run: () => db.tx({ isolation: "read-uncommitted" }, async () => undefined),
      sideEffects,
    },
    TX021: {
      feature: "transaction.read-only",
      expectedErrorFeature: "transaction.read-only",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ readOnly: true }, async () => undefined),
      sideEffects,
    },
    TX022: {
      ...
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "read-committed", readOnly: true }, async () => undefined),
      sideEffects,
    },
    TX023: {
      ...
      expectedErrorFeature: "transaction.isolation.read-committed",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "read-committed" }, async () => undefined),
      sideEffects,
    },
    TX024: {
      ...
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "repeatable-read" }, async () => undefined),
      sideEffects,
    },
    TX025:
      targetKind === "libsql"
        ? {
            ...
            expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
            run: () => db.tx({ isolation: "serializable" }, async () => undefined),
            sideEffects,
          }
        : undefined,
    TX026:
      targetKind === "sqlite"
        ? {
            feature: "transaction.read-only",
            expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
            run: () => db.tx({ readOnly: false }, async () => undefined),
            sideEffects,
          }
        : undefined,
    TX027: {
      ...
      expectedErrorFeature: "transaction.isolation.read-uncommitted",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "read-uncommitted", readOnly: true }, async () => undefined),
      sideEffects,
    },
    TX028: {
      feature: "combination:serializable+readOnly",
      expectedErrorFeature: targetKind === "sqlite" ? "transaction.read-only" : "transaction.isolation.serializable",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "serializable", readOnly: true }, async () => undefined),
      sideEffects,
    },
    TX029: {
      ...
      expectedErrorFeature: "transaction.isolation.read-uncommitted",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "read-uncommitted", readOnly: false }, async () => undefined),
      sideEffects,
    },
    TX030: {
      ...
      expectedErrorFeature: "transaction.isolation.read-committed",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "read-committed", readOnly: false }, async () => undefined),
      sideEffects,
    },
    TX031: {
      ...
      expectedErrorFeature: "transaction.isolation.repeatable-read",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "repeatable-read", readOnly: true }, async () => undefined),
      sideEffects,
    },
    TX032: {
      ...
      expectedErrorFeature: "transaction.isolation.repeatable-read",
      expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
      run: () => db.tx({ isolation: "repeatable-read", readOnly: false }, async () => undefined),
      sideEffects,
    },
    TX033:
      targetKind === "sqlite"
        ? {
            ...
            expectedErrorFeature: "transaction.read-only",
            expectedCode: "BRAID_TX_OPTION_UNSUPPORTED",
            run: () => db.tx({ isolation: "serializable", readOnly: false }, async () => undefined),
            sideEffects,
          }
        : {
            feature: "combination:serializable+readWrite",
    ...
    CALL006: {
      ...
      expectedErrorFeature: "routine.call",
      expectedCode: "BRAID_CALL_UNSUPPORTED",
      run: () => runCall(returnValue),
      sideEffects,
    },
    CALL007: { feature: "routine.call", expectedCode: "BRAID_CALL_UNSUPPORTED", run: () => runCall(call), sideEffects },
  };
  return Object.fromEntries(Object.entries(probes).filter(([, probe]) => probe !== undefined)) as Partial<
    Record<CertificationCaseId, UnsupportedProbe>
  >;
}
...
export function createSqliteFixture(options: SqliteFixtureOptions): CertificationFixture {
  ...
  const stream: StreamingConformanceFixture<unknown> | undefined = options.streamSupported
    ? {
        ...
        reuseAfterBreak: async () => {
          ...
          if (row.id !== options.physicalSessionId)
            throw new Error("SQLite stream reuse changed physical session identity.");
        },
  ...
  const bulk: BulkConformanceFixture<unknown> = {
    ...
    middleFailure: async () => {
      ...
      const observedRows = await options.db.all(
        sql.rows<{ readonly value: string }>`SELECT value FROM cert_items ORDER BY rowid`,
      );
      ...
      throw new Error(
        `SQLite bulk middle-item durability was not the expected prefix or atomic state: ${observedText}.`,
      );
    },
  };
  ...
  queries.prepared = {
    command: () => {
      preparedCalls.count += 1;
      return bulkFactory("prepared");
    },
    rows: () => {
      preparedCalls.count += 1;
      return queries.many;
  ...
  const metrics = {
    ...
    mutationSentinel: async () => {
      const row = await options.db.one(
    ...
    readOnlyWrite: async () => {
      ...
      if (before.length !== 1)
        throw new Error(`SQLite read-write transaction proof expected one row, got ${before.length}.`);
      let error: unknown;
      try {
        await options.db.tx({ readOnly: true }, async (tx) => {
      ...
      if ((error as { readonly code?: unknown } | undefined)?.code !== "BRAID_TX_OPTION_UNSUPPORTED") {
        throw new Error("SQLite read-only transaction did not reject with BRAID_TX_OPTION_UNSUPPORTED.", {
          cause: error,
        });
      }
...
export async function makeLibsqlDirectory(): Promise<{
