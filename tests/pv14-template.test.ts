...
test("renders immutable logical segments and atomic parameter records", () => {
  ...
  assert.deepEqual(
    rendered.parameters.map(({ value, interpolation, hint: parameterHint }) => ({
      value,
      interpolation,
      hint: parameterHint,
    })),
...
function generatedStatement(lines: number, binds: number) {
  ...
  const strings = Array.from({ length: binds + 1 }, (_, index) =>
    index === 0
...
test("renders MyBatis-scale logical statements without bind loss or reordering", () => {
  ...
  const smallDescription = createStatementBindingDescription(
    small,
  ...
  assert.equal(
    smallParameterized,
    small.segments
      .map((segment, index) => `${segment}${index < small.parameters.length ? `$${index + 1}` : ""}`)
      .join(""),
  );
  ...
  assert.equal(
    ...
    small.segments
      .map(
        (segment, index) => `${segment}${index < small.parameters.length ? String(small.parameters[index].value) : ""}`,
      )
      .join(""),
  );
  ...
  const largeDescription = createStatementBindingDescription(
    large,
    { dialectId: "postgres", requestedReuse: "auto" },
    {
      adapterId: "large-fixture",
  ...
  assert.equal(
    largeDescription.parameterizedSql,
    large.segments
      .map((segment, index) => `${segment}${index < large.parameters.length ? `$${index + 1}` : ""}`)
      .join(""),
  );
  assert.equal(
    largeDescription.literalizedSql({ values: "inline" }).text,
    large.segments
      .map(
        (segment, index) => `${segment}${index < large.parameters.length ? String(large.parameters[index].value) : ""}`,
      )
      .join(""),
  );
});
