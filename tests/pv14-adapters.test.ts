...
import { createNodeSqliteExecutor, createNodeSqliteDatabase, nodeSqliteStatementBinding } from "@sqlbraid/sqlite/node-sqlite";
...
test("positional executors reject stale and forged descriptions before physical I/O", async () => {
  let ioCalls = 0;
  const io = () => { ioCalls += 1; throw new Error("unexpected physical I/O"); };
  const executors = [
...
const fixtures: Fixture[] = [
  {
    ...
    create: (io, events) => createPgPoolDatabase(
      { connect: async () => io() },
  ...
  },
  {
    ...
    create: (io, events) => createMysql2PoolDatabase(
  ...
  },
  {
    ...
    create: (io, events) => createNodeSqliteDatabase(
      { prepare: io },
      { observers: [{ onEvent(event) { events.push(event); } }] },
    ),
  },
  {
    ...
    create: (io, events) => createOracledbPoolDatabase(
      { getConnection: async () => io() },
      { observers: [{ onEvent(event) { events.push(event); } }] },
    ),
  },
...
for (const fixture of fixtures) {
  test(`${fixture.name} fails materialization before physical acquisition`, async () => {
    ...
    const db = fixture.create(() => { ioCalls += 1; throw new Error("unexpected physical I/O"); }, events);
    ...
    assert.equal(events.some((event) => event.type === "query:result"), false);
  });
}
...
for (const [dialectId, adapter, marker] of [
  ...
  test(`${dialectId} transport handles empty and 1000 ordered parameters without scanning source SQL`, () => {
    ...
    assert.equal(binding.parameterizedSql, `SELECT ${parameters.map((_, index) => marker(index + 1)).join(", ")} /* $1 ? :1 @p1 */`);
    assert.deepEqual(binding.bindings.map((parameter) => parameter.interpolation), parameters.map((parameter) => parameter.interpolation));
    assert.equal(binding.literalizedSql({ values: "inline" }).text, `SELECT ${parameters.map((parameter) => parameter.value).join(", ")} /* $1 ? :1 @p1 */`);
  });
}
