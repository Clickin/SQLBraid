...
assert.ok(
  selectedPhase === undefined || selectedPhase === "contracts" || selectedPhase === "profile",
...
async function runTransactionContracts(dialect) {
  ...
  assert.ok(
    selected.length > 0 || metadataSelected,
    `Unknown or inapplicable ${dialect} scenario: ${selectedScenario}`,
  );
  ...
  if (metadataSelected) {
    ...
    try {
      ...
      await commandMetadataContract(
        ...
        dialect === "sqlite" ? "lossless" : "guarded",
      );
...
async function runDialect(dialect) {
  ...
  `);
    ...
    try {
      const zeroDml = await db.execute(
        sqlTag.command`UPDATE ${sqlTag.raw(bulkTable)} SET value = ${"zero"} WHERE 1 = 0`,
      );
    ...
    if (dialect === "mysql" || dialect === "mariadb") {
      ...
      try {
        for (const method of ["query", "bulk"]) {
          ...
          assert.equal(
            (await observer.unsafe("SELECT ID FROM information_schema.PROCESSLIST WHERE ID = ?", [before])).length,
            0,
          );
