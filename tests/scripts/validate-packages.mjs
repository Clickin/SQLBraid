...
try {
  for (const name of requiredPackageNames)
    if (!packageNames.includes(name)) throw new Error(`Missing required package ${name}.`);
  ...
  if (packInputDir) {
    ...
    for (const tarball of inputFiles) {
      ...
      assert.ok(
        recorded && join(packInputDir, recorded.file) === tarball,
        `Unrecorded release artifact: ${manifest.name}`,
      );
  ...
  for (const packageName of packageNames) {
    ...
    if ((await stat(tarball)).size > MAX_TARBALL_BYTES)
      throw new Error(`Tarball for ${packageName} exceeds ${MAX_TARBALL_BYTES} bytes.`);
    ...
    for (const file of packageFiles) {
      ...
      if (
        /(^|[/])(?:test|tests|fixture|fixtures|__tests__)(?:[-_./]|$)/iu.test(name) ||
        /(^|[/])(?:\.env(?:\..*)?|[^/]+\.(?:pem|key|p12|secret))$/iu.test(name)
      ) {
        throw new Error(`Test, fixture, or secret file in ${manifest.name} tarball: ${name}`);
      }
      if ((await stat(join(packageDir, name))).size > MAX_FILE_BYTES)
        throw new Error(`File ${manifest.name}/${name} exceeds ${MAX_FILE_BYTES} bytes.`);
      const text = await readFile(join(packageDir, name), "utf8");
      if (name.startsWith("dist/") || name === "README.md") {
        assert.equal(
          text,
          await readFile(join(packageRoot, packageName, name), "utf8"),
          `${manifest.name}/${name} differs from the current build.`,
        );
      }
      if (/\.(?:js|ts)$/u.test(name)) {
        for (const imported of ts.preProcessFile(text, true, true).importedFiles) {
          const specifier = imported.fileName;
          if (specifier.startsWith(".") || specifier.startsWith("node:") || builtinModules.includes(specifier))
            continue;
    ...
    for (const required of ["LICENSE", "README.md"])
      if (!fileNames.has(required)) throw new Error(`Packed ${manifest.name} is missing package/${required}.`);
    ...
    for (const packagePath of [
      ...
      ...collectPackagePaths(manifest.imports),
      ...collectPackagePaths(manifest.bin),
    ]) {
    ...
    if (packageName === "bun-sql") {
      if (manifest.engines?.bun !== ">=1.3.14")
        throw new Error(`Unexpected Bun engine for ${manifest.name}: ${manifest.engines?.bun ?? "missing"}`);
    ...
    for (const validator of ["valibot", "zod", "arktype"]) {
      if (
        ...
        manifest.peerDependencies?.[validator] ||
        manifest.optionalDependencies?.[validator]
      ) {
    ...
    if (manifest.name === "@sqlbraid/opentelemetry") {
      const productionOtelDependencies = ["dependencies", "optionalDependencies", "peerDependencies"].flatMap((field) =>
        Object.keys(manifest[field] ?? {}).filter(
          (name) => name.startsWith("@opentelemetry/") && name !== "@opentelemetry/api",
        ),
      );
      if (
        !manifest.peerDependencies?.["@opentelemetry/api"] ||
        ...
        throw new Error(
          "OpenTelemetry integration must keep @opentelemetry/api as its only OTel peer and install no SDK/exporter.",
        );
      }
    }
    if (manifest.name === "@sqlbraid/codegen") {
      ...
      if (
        ...
        dependencyNames[0] !== "@sqlbraid/core" ||
        dependencyNames[1] !== "@sqlbraid/metadata" ||
    ...
    if (manifest.name === "sqlbraid") {
      ...
      if (
        ...
        throw new Error(
          "The unscoped sqlbraid package must expose only first-party runtime dependencies and no CLI/bin or driver peers.",
        );
      }
    }
    if (
      manifest.name === "@sqlbraid/sqlite" &&
      ["better-sqlite3", "@libsql/client"].some(
        (name) =>
          manifest.dependencies?.[name] || manifest.optionalDependencies?.[name] || manifest.peerDependencies?.[name],
      )
    ) {
      throw new Error(
        "@sqlbraid/sqlite must keep better-sqlite3 and @libsql/client structural and out of package dependencies.",
      );
    }
    for (const tooling of [
      ...
      if (
        runtimePackages.includes(packageName) &&
        (manifest.dependencies?.[tooling] || manifest.optionalDependencies?.[tooling])
      ) {
  ...
  if (packInputDir && inputTarballs.size !== packageNames.length)
    throw new Error(
      `Release artifact directory contains ${inputTarballs.size} tarballs; expected ${packageNames.length}.`,
    );
  ...
  const runtimeDependencies = Object.fromEntries(
    runtimePackages.map((name) => [`@sqlbraid/${name}`, dependencies[`@sqlbraid/${name}`]]),
  );
  await writeFile(
    join(boundaryConsumer, "package.json"),
    JSON.stringify({
      name: "sqlbraid-boundary-consumer",
      private: true,
      type: "module",
      dependencies: {
        ...runtimeDependencies,
  ...
  if (runtimeTopLevelPackages.includes("sqlbraid"))
    throw new Error("Runtime consumer installed the canonical facade transitively.");
  if (["oracledb", "tedious", "mariadb"].some((name) => runtimeTopLevelPackages.includes(name)))
    throw new Error("Runtime consumer installed a Node-only database driver.");
  await writeFile(
    join(boundaryConsumer, "runtime.mjs"),
    [
      'import assert from "node:assert/strict";',
  ...
  const mssqlDevDependencies = JSON.parse(
    await readFile(join(packageRoot, "mssql", "package.json"), "utf8"),
  ).devDependencies;
  await run(
    "npm",
    ["install", "--ignore-scripts", "--no-audit", "--no-fund", `tedious@${mssqlDevDependencies.tedious}`],
    boundaryConsumer,
  );
  ...
  await writeFile(
    join(boundaryConsumer, "metadata-absent.mjs"),
    [
  ...
  await run(
    ...
    [
      ...
      "--no-audit",
      "--no-fund",
      dependencies["@sqlbraid/metadata"],
      dependencies["@sqlbraid/codegen"],
    ],
    boundaryConsumer,
  );
  ...
  await writeFile(
    join(boundaryConsumer, "metadata.mjs"),
    [
      ...metadataImports,
      'import assert from "node:assert/strict";',
  ...
  await writeFile(
    join(boundaryConsumer, "codegen.mjs"),
    [
      'import assert from "node:assert/strict";',
  ...
  await writeFile(
    join(boundaryConsumer, "metadata.ts"),
    [
      ...metadataImports,
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "sqlbraid-packed-consumer", private: true, type: "module", dependencies }, null, 2),
  );
  ...
  await writeFile(
    ...
    JSON.stringify({
      name: "sqlbraid-runtime-consumer",
      private: true,
      type: "module",
  ...
  const unscopedManifest = JSON.parse(
    await readFile(join(runtimeConsumer, "node_modules/sqlbraid/package.json"), "utf8"),
  );
  if (
    unscopedManifest.bin !== undefined ||
    Object.hasOwn(unscopedManifest.dependencies ?? {}, "@sqlbraid/cli") ||
  ...
  if (
    [
      ...
      "operations",
      "opentelemetry",
    ].some((name) => runtimeConsumerPackages.includes(name))
  ) {
  ...
  if (
    ["oracledb", "tedious", "pg", "mysql2", "mariadb", "better-sqlite3", "@libsql"].some((name) =>
      packedTopLevelPackages.includes(name),
    )
  ) {
    throw new Error("Installing the unscoped sqlbraid package pulled in a database driver.");
  }
  await writeFile(
    join(runtimeConsumer, "index.mjs"),
    [
      'import assert from "node:assert/strict";',
  ...
  await writeFile(
    join(cliConsumer, "sqlbraid.config.mjs"),
    [
  ...
  await writeFile(
    join(cliConsumer, "packed-metadata.json"),
    JSON.stringify({
  ...
  const cliProbe = await execFile(
    process.execPath,
    [join(cliConsumer, "node_modules/@sqlbraid/cli/dist/index.js"), "codegen", "--json"],
    { cwd: cliConsumer, maxBuffer: 2 * 1024 * 1024 },
  );
  assert.equal(JSON.parse(cliProbe.stdout)[0].status, "written");
  const cliCheck = await execFile("npx", ["--no-install", "sqlbraid", "codegen", "--check", "--json"], {
    cwd: cliConsumer,
  ...
  await writeFile(
    join(consumer, "facade-drivers.mjs"),
    [
  ...
  packedDatabaseEnv.SQLBRAID_ORACLE_URL ??=
    packedDatabaseEnv.SQLBRAID_ORACLE_CONNECTION_STRING ?? packedDatabaseEnv.ORACLE_URL;
  ...
  if (!packedDatabaseEnv.SQLBRAID_ORACLE_URL || !packedDatabaseEnv.SQLBRAID_MSSQL_URL) {
    ...
    if (!packedDatabaseEnv.SQLBRAID_ORACLE_URL) {
      const container = await new GenericContainer("gvenzl/oracle-free:23.9-slim-faststart")
        .withEnvironment({
          ORACLE_PASSWORD: "SqlbraidTest13",
          APP_USER: "sqlbraid",
          APP_USER_PASSWORD: "SqlbraidTest13",
        })
    ...
    if (!packedDatabaseEnv.SQLBRAID_MSSQL_URL) {
      const container = await new GenericContainer("mcr.microsoft.com/mssql/server:2022-CU18-ubuntu-22.04")
        ...
        .start();
      packedContainers.push(container);
      packedDatabaseEnv.SQLBRAID_MSSQL_URL =
        `sqlserver://sa:${encodeURIComponent("Sqlbraid_Test13!")}` +
  ...
  await writeFile(
    cliFile,
    'import { sql as templateSql } from "@sqlbraid/template"; import { sql as postgresSql } from "@sqlbraid/postgres"; const queries = [templateSql`SELECT 1`, postgresSql`SELECT 1`]; void queries;\n',
  );
  ...
  const agentConsumer = await execFile(process.execPath, ["agent-tooling-consumer.mjs"], {
    cwd: consumer,
    maxBuffer: 1024 * 1024,
  });
    ...
    for (const file of files) {
      ...
      for (const needle of forbidden)
        if (text.includes(needle))
          throw new Error(`Monorepo path leaked into ${packageEntry.name}/${file.name}: ${needle}`);
    }
  }
  ...
  if (skipVsix) {
    ...
    console.info(
      `Validated ${tarballs.length} packed npm packages with ESM, types, subpaths, CLI, engine metadata, tooling consumers and leakage checks.`,
    );
  } else {
    ...
    if (suppliedVsix && !packInputDir)
      throw new Error("Supplied VSIX validation requires a supplied release artifact directory.");
    if (suppliedVsix && resolve(dirname(suppliedVsix)) !== packInputDir)
      throw new Error("Supplied VSIX must be stored beside the supplied release manifest.");
    const extensionDependencies = Object.fromEntries(
      Object.entries(extensionManifest.dependencies).map(([name, version]) => [
        name,
        version.replace(/^workspace:/u, ""),
      ]),
    );
    const bundledVersions = {};
    for (const [key, packageName] of [
      ["cli", "@sqlbraid/cli"],
      ["languageServer", "@sqlbraid/language-server"],
    ]) {
      const bundledManifest = JSON.parse(
        await readFile(join(packageRoot, packageName.slice("@sqlbraid/".length), "package.json"), "utf8"),
      );
      assert.equal(
        extensionDependencies[packageName],
        bundledManifest.version,
        `${packageName} dependency must match the bundled source version.`,
      );
    ...
    async function includeTooling(name) {
      ...
      const manifest = JSON.parse(
        await readFile(join(packageRoot, name.slice("@sqlbraid/".length), "package.json"), "utf8"),
      );
      for (const dependency of Object.keys(manifest.dependencies ?? {}))
        if (dependency.startsWith("@sqlbraid/")) await includeTooling(dependency);
    }
    for (const name of Object.keys(extensionDependencies))
      if (name.startsWith("@sqlbraid/")) await includeTooling(name);
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        dependencies: {
          ...
          ...Object.fromEntries([...bundledNames].map((name) => [name, dependencies[name]])),
        },
      }),
    );
    ...
    assert.equal(
      ...
      "VSIX must ship the same Apache-2.0 license as npm.",
    );
    ...
    for (const [key, packageName] of [
      ...
      assert.equal(
        bundledManifest.version,
        bundledVersions[key],
        `VSIX must bundle the validated ${packageName} version.`,
      );
    }
    await mkdir(dirname(vsixOutput), { recursive: true });
    if (suppliedVsix) {
      if (vsixOutput !== suppliedVsix)
        throw new Error("Supplied VSIX output must be validated in place without copying or repacking.");
    } else {
    ...
    if (packInputDir) {
      assert.equal(
        resolve(dirname(vsixOutput)),
        packInputDir,
        "Validated VSIX must be stored beside the release manifest.",
      );
    ...
    console.info(
      `Validated ${tarballs.length} packed packages with ESM, types, subpaths, CLI, engine metadata, tooling/editor consumers and leakage checks.`,
    );
  }
} finally {
