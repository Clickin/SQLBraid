...
test("query-bound mapping is intrinsic to every row API", async () => {
  ...
  const batch = await one.batch([sql.rows(mapper)`SELECT first`, sql.rows(secondMapper)`SELECT second`] as const);
  assert.deepEqual(
    batch.map((result) => result.rows),
    [[{ id: 3, source: "first" }], [{ id: 3, source: "second" }]],
  );
});
...
test("query-bound and execution schemas compose in order", async () => {
  ...
  await assert.rejects(
    () => many.all(sql.rows(querySchema)`SELECT users`, { schema: failingExecution }),
    DatabaseResultValidationError,
  );
...
test("cardinality is checked before either mapper runs", async () => {
  ...
  await assert.rejects(
    () => many.one(sql.rows(querySchema)`SELECT users`, { schema: executionSchema }),
    DatabaseCardinalityError,
  );
  await assert.rejects(
    () => many.maybeOne(sql.rows(querySchema)`SELECT users`, { schema: executionSchema }),
    DatabaseCardinalityError,
  );
...
test("all and streaming map rows sequentially and close at the first failure", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DatabaseResultValidationError &&
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DatabaseResultValidationError && error.stage === "query" && error.rowIndex === 1,
  );
...
test("result kinds are asserted before mapping and unknown rows stay raw", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DatabaseResultKindError && error.declaredKind === "rows" && error.actualKind === "command",
  );
...
test("validation failures identify query and execution stages", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DatabaseResultValidationError && error.stage === "query" && error.rowIndex === 0,
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DatabaseResultValidationError && error.stage === "execution" && error.rowIndex === 0,
  );
});
...
test("validator exceptions propagate unchanged and malformed results are rejected", async () => {
  ...
  await assert.rejects(
    () => db.all(sql.rows(throwing)`SELECT user`),
  ...
  await assert.rejects(
    () => db.all(sql.rows(rejecting)`SELECT user`),
    (error: unknown) => error === thrown,
  );
  ...
  await assert.rejects(
    () =>
      db.all(sql.rows(schema<User>((value) => ({ value: value as User })))`SELECT user`, {
        schema: malformedExecution,
      }),
    TypeError,
  );
