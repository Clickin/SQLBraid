...
import {
  ...
  streamTransports,
  transports,
} from "./contracts/resources.faults.js";

test("[contract:sqlite-wasm:metadata.affected-rows:boundary] unavailable native wide counts reject before mutation", () => {
  const unsupported = new Error("WASM build does not support changes64");
  let nativeStatements = 0;
  assert.throws(
    () =>
      createSqliteWasmDatabase({
        ...
        changes(_total, wide) {
          if (wide) throw unsupported;
          return 0;
        },
      }),
    (error) => error === unsupported,
  );
  assert.equal(nativeStatements, 0);
});
...
for (const id of transports) {
  for (const ownership of resourceOwnerships(id))
    test(`[contract:${id}:cancellation.pre-io:boundary] [ownership:${ownership}] pre-aborted signal preserves reason without checkout or native execution`, async () => {
  ...
  test(`[contract:${id}:metadata.affected-rows:boundary] accepts safe counts and rejects unsafe and malformed native counts`, async () => {
    ...
    for (const count of [Number.MAX_SAFE_INTEGER + 1, -1, 0.5, "not-a-count"]) {
      ...
      await assert.rejects(
        affectedRows(),
        (error) => error instanceof Error && "code" in error && error.code === "BRAID_RESULT_EXACTNESS",
      );
    }
  });
}
...
for (const id of streamTransports)
  for (const ownership of resourceOwnerships(id)) {
    test(`[contract:${id}:resource.stream-return:boundary] [ownership:${ownership}] early return closes the native resource before the pool lease`, async () => {
      ...
      for await (const row of db.stream(rows())) {
    ...
    test(`[contract:${id}:resource.stream-read-failure:boundary] [ownership:${ownership}] native read failure closes resources and retains the primary error`, async () => {
      ...
      await assert.rejects(
        async () => {
          for await (const row of db.stream(rows())) void row;
        },
        (error) => containsError(error, primary),
      );
      assert.equal(state.created, 1);
      assert.equal(state.closed, 1);
      assert.equal(state.released + state.discarded, state.acquired);
    ...
    test(`[contract:${id}:resource.cleanup-failure:boundary] [ownership:${ownership}] simultaneous read and close failures preserve both and prevent healthy reuse`, async () => {
      ...
      await assert.rejects(
        ...
        (error) => containsError(error, primary) && containsError(error, cleanup),
      );
      assert.equal(state.closed, 1);
...
for (const id of ["pg", "mysql2", "mariadb", "node-oracledb", "sqlite-wasm"] as const)
  for (const ownership of resourceOwnerships(id)) {
    test(`[contract:${id}:resource.init-failure:boundary] [ownership:${ownership}] resource created before initialization failure is closed exactly once`, async () => {
      ...
      await assert.rejects(
        ...
        (error) => containsError(error, failure),
      );
      assert.equal(state.created, 1);
      assert.equal(state.closed, 1);
      assert.equal(state.released + state.discarded, state.acquired);
...
for (const id of ["pg", "mysql2", "mariadb", "node-oracledb"] as const)
  for (const ownership of resourceOwnerships(id)) {
    test(`[contract:${id}:cancellation.before-handoff:boundary] [ownership:${ownership}] abort immediately after native creation closes before any row escapes`, async () => {
      ...
      await assert.rejects(
        async () => {
          for await (const row of db.stream(rows(), { signal: controller.signal })) {
            void row;
            yielded++;
          }
        },
        (error) => containsError(error, reason),
      );
