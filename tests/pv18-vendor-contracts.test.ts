...
function snapshot(
  dialect: string,
  columns: readonly { readonly name: string; readonly type: string; readonly nullable?: boolean }[],
) {
  return {
...
test("PV18 first-party vendor policies are deeply immutable and cover inspector spellings", () => {
  ...
  for (const databaseType of [
    ...
    assert.ok(
      oracleTypePolicy.mappings.some((mapping) => mapping.databaseType === databaseType),
      databaseType,
    );
  }
  for (const databaseType of [
    ...
    assert.ok(
      mssqlTypePolicy.mappings.some((mapping) => mapping.databaseType === databaseType),
      databaseType,
    );
  }
});
...
test("Oracle DB_TYPE_JSON keeps every parsed JSON root in the native domain", async () => {
  ...
  const result = await createOracledbExecutor(connection, { driver }).query(
    oracleSql.rows`SELECT payload FROM dual`.render(),
  );
...
test("Oracle and MSSQL known types remain codegen-visible without recursive container claims", () => {
  ...
  assert.deepEqual(
    oracle.diagnostics.filter((diagnostic) => diagnostic.code === "CODEGEN_UNKNOWN_DATABASE_TYPE"),
    [],
  );
  ...
  assert.deepEqual(
    mssql.diagnostics.filter((diagnostic) => diagnostic.code === "CODEGEN_UNKNOWN_DATABASE_TYPE"),
    [],
  );
