...
interface DatabaseLike {
  ...
  tx<T>(callback: (database: DatabaseLike) => Promise<T>): Promise<T>;
}
...
export async function runW01(options: W01Options): Promise<void> {
  ...
  await clear();
  await expect(db.tx(async () => db.execute(sql`SELECT 1`))).rejects.toMatchObject({ code: "BRAID_TX_SCOPE" });
  expect(await rows()).toEqual([]);

  await clear();
  await expect(db.tx(async (tx) => tx.execute(sql`INSERT INTO braid_w01 (missing) VALUES (${"K"})`))).rejects.toThrow();
  expect(await rows()).toEqual([]);
}
