...
test("postgres.pv17.numeric-transport", { timeout: 30_000 }, async () => {
  ...
  const db = createPgDatabase(client);
  try {
    await client.query("CREATE TEMP TABLE braid_pv17_fidelity (id bigint NOT NULL, amount numeric(40, 20) NOT NULL, payload json NOT NULL, measured_at timestamp(6) NOT NULL)");
    const values = [
      { id: "9007199254740993", amount: "12345678901234567890.12345678901234567890" },
      { id: "-9223372036854775808", amount: "-0.00000000000000000001" },
    ];
    const bulk = await db.bulk(values, (input) => sql.command`
    ...
    for (const [index, expected] of binary64Finite.entries()) {
      const key = ["f64_0", "f64_neg_zero", "f64_fraction", "f64_value", "f64_min_normal", "f64_max", "f64_subnormal", "f64_roundtrip"][index]!;
      assertFloatBits(floats[key], expected, 64);
    }
    for (const [index, expected] of binary32Finite.entries()) {
      const key = ["f32_0", "f32_neg_zero", "f32_fraction", "f32_value", "f32_min_normal", "f32_max", "f32_subnormal", "f32_roundtrip"][index]!;
...
test("postgres.dml.insert-returning", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    const using = await db.all(sql.rows<{ readonly id: string }>`
      ...
      RETURNING target.id
    `);
    assert.deepEqual([...using].sort((left, right) => Number(left.id) - Number(right.id)), [{ id: "1" }, { id: "4" }]);
    ...
    if (version >= 180000) {
      const merge = await db.all(sql.rows<{ readonly action: string; readonly old_id: string | null; readonly new_id: string | null }>`
      ...
      `);
      assert.deepEqual([...merge].sort((left, right) => Number(left.new_id ?? -1) - Number(right.new_id ?? -1)), [
