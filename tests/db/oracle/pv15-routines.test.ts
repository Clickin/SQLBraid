...
test("Oracle routine fixture exposes scalar and two heterogeneous REF CURSOR outputs", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(result.resultSets.map((set) => set.rows), [[{ USER_ID: "user-1" }], [{ PAYMENT_ID: "7" }], [{ SUMMARY: "summary-1" }]]);
...
test("Oracle routine materializes CLOB and BLOB OUT and INOUT binds", { timeout: 60_000 }, async () => {
  const { connection } = await connect();
  const fill = async (lob: NodeJS.WritableStream & { once(event: string, listener: (...args: readonly unknown[]) => void): unknown; end(value: string | Uint8Array): void }, value: string | Uint8Array): Promise<void> => {
  ...
  try {
    await connection.execute("CREATE OR REPLACE PROCEDURE braid_pv15_lob (p_clob_out OUT CLOB, p_blob_out OUT BLOB, p_clob_io IN OUT CLOB, p_blob_io IN OUT BLOB) IS l_blob_suffix RAW(32) := UTL_RAW.CAST_TO_RAW('-blob-updated'); BEGIN p_clob_out := TO_CLOB('clob-out'); p_blob_out := TO_BLOB(UTL_RAW.CAST_TO_RAW('blob-out')); DBMS_LOB.WRITEAPPEND(p_clob_io, LENGTH('-clob-updated'), '-clob-updated'); DBMS_LOB.WRITEAPPEND(p_blob_io, UTL_RAW.LENGTH(l_blob_suffix), l_blob_suffix); END;");
...
test("Oracle transaction streams retain the pinned physical session until iterator cleanup", { timeout: 30_000 }, async () => {
  const { connection } = await connect();
  try {
    const db = createOracledbDatabase(connection);
    const query = sql.rows<{ readonly VALUE: string }>`SELECT TO_CHAR(LEVEL) AS VALUE FROM dual CONNECT BY LEVEL <= 2`;
    await db.tx(async (tx) => {
      const iterator = tx.stream(query)[Symbol.asyncIterator]();
      assert.equal((await iterator.next()).value?.VALUE, "1");
      await assert.rejects(
        () => tx.one(sql.rows<{ readonly VALUE: string }>`SELECT TO_CHAR(7) AS VALUE FROM dual`),
        { code: "BRAID_STREAM_SCOPE" },
      );
      await iterator.return?.();
      assert.equal((await tx.one(sql.rows<{ readonly VALUE: string }>`SELECT TO_CHAR(7) AS VALUE FROM dual`)).VALUE, "7");
    });
  } finally {
    await connection.close();
  }
