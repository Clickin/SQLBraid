...
test("exact integer normalization keeps strings canonical and rejects unsafe transport", () => {
  assert.equal(normalizeExactInteger("00042"), "00042");
  assert.equal(normalizeExactInteger("+42"), "+42");
  assert.equal(normalizeExactInteger(42), "42");
  assert.equal(normalizeExactInteger(42n), "42");
  for (const value of [Number.MAX_SAFE_INTEGER + 1, 1.5, Number.NaN, "1.0", null, undefined]) {
    assertExactnessFailure(() => normalizeExactInteger(value));
  }
});

test("database counts remain non-negative safe operational numbers", () => {
  assert.equal(safeDatabaseCount(0), 0);
  assert.equal(safeDatabaseCount("42"), 42);
  assert.equal(safeDatabaseCount(42n), 42);
  assert.equal(safeDatabaseCount(Number.MAX_SAFE_INTEGER), Number.MAX_SAFE_INTEGER);
  for (const value of [-1, 1.5, Number.MAX_SAFE_INTEGER + 1, -1n, 9007199254740992n, "9007199254740992", undefined]) {
    assertExactnessFailure(() => safeDatabaseCount(value));
  }
});
...
test("first-party exact numeric policies reject unsafe raw values instead of stringifying them", () => {
  for (const [policy, databaseType] of [
    ...
    assert.equal(policy.decode(databaseType, "9223372036854775807"), "9223372036854775807");
  ...
  assertExactnessFailure(() => mssqlTypePolicy.decode("decimal", 12.5));
});

test("SQLite INTEGER uses the same exact string output contract", () => {
  assert.equal(sqliteTypePolicy.decode("INTEGER", "9223372036854775807"), "9223372036854775807");
  assert.equal(sqliteTypePolicy.decode("INTEGER", 42), "42");
  assert.equal(sqliteTypePolicy.decode("INTEGER", 42n), "42");
  assertExactnessFailure(() => sqliteTypePolicy.decode("INTEGER", Number.MAX_SAFE_INTEGER + 1));
});
