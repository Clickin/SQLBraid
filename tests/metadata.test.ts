...
test("metadata drift reports exact changed database fact paths", () => {
  ...
  assert.deepEqual(
    diffSnapshots(snapshot, changed).map((entry) => entry.path),
    ["relations.public.users.columns[0].nullable"],
  );
});
...
test("qualified identities escape delimiters without colliding", () => {
  ...
  assert.equal(
    qualifiedIdentitySegmentsWithSuffix(["owner", "package", "procedure"], "1.2"),
    "owner.package.procedure:1\\.2",
  );
});
...
test("JSON snapshots reject malformed optional codegen facts at their exact paths", () => {
  ...
  const cases: readonly ["column" | "type" | "argument" | "routine" | "result" | "relation", unknown, string][] = [
    ...["nullabilityEvidence", "defaultExpression", "charset", "collation"].map(
      (field): ["column", unknown, string] => ["column", 1, field],
    ),
    ...["generated", "identity", "insertable", "updatable"].map((field): ["column", unknown, string] => [
      "column",
      "false",
      field,
    ]),
    ["type", false, "elementType"],
    ["type", 1, "baseType"],
    ["type", ["ok", 1], "values"],
    ["argument", 1, "name"],
    ["argument", "false", "nullable"],
    ["argument", 1, "hasDefault"],
    ["routine", "false", "argumentsComplete"],
    ["result", "true", "nullable"],
    ["relation", 1, "namespace"],
    ["relation", "true", "strict"],
  ];
  for (const [target, value, field] of cases) {
    const candidate = {
      ...
      relations: {
        users: {
          ...snapshot.relations["public.users"],
          ...(target === "relation" ? { [field]: value } : {}),
          columns: [{ ...column, ...(target === "column" ? { [field]: value } : {}) }],
        },
      },
      routines: {
        f: [
          {
            ...
            name: "f",
            kind: "function",
            arguments: [{ mode: "in", type: "t", ...(target === "argument" ? { [field]: value } : {}) }],
            ...(target === "routine" ? { [field]: value } : {}),
            result: { kind: "scalar", type: "t", ...(target === "result" ? { [field]: value } : {}) },
          },
        ],
      },
    };
    const prefix = {
      column: "relations.users.columns[0]",
      type: "types.t",
      argument: "routines.f[0].arguments[0]",
      routine: "routines.f[0]",
      result: "routines.f[0].result",
      relation: "relations.users",
    }[target];
    assert.throws(
      () => metadata.parseSnapshotJson(JSON.stringify(candidate)),
      (error: unknown) =>
        error instanceof metadata.SnapshotValidationError &&
        error.diagnostics.some((diagnostic) => diagnostic.path === `${prefix}.${field}`),
    );
  }
});
...
test("routine argument completeness is canonicalized and reported as drift", () => {
  ...
  const complete: MetadataSnapshot = {
    ...incomplete,
    routines: { calculate_fee: [{ ...routine, argumentsComplete: true }] },
  };
  ...
  assert.deepEqual(
    diffSnapshots(incomplete, complete).map((entry) => entry.path),
    ["routines.calculate_fee[0].argumentsComplete"],
  );
});
