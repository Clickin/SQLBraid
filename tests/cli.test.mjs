...
test('sqlbraid check and manifest operate on source files', async () => {
  ...
  const check = await exec(process.execPath, ['dist/packages/cli/src/index.js', 'check', '--file', file]);
  assert.equal(check.stdout, '');
  const manifest = await exec(process.execPath, ['dist/packages/cli/src/index.js', 'manifest', '--file', file]);
