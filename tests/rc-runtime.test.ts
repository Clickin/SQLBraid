...
function rowsExecutor(log: string[], env = environment()): QueryExecutor {
  return {
    ...
    async begin(options?: TransactionOptions) {
      log.push(`begin:${JSON.stringify(options ?? null)}`);
    },
    async commit() {
      log.push("commit");
    },
...
test("session pins one pooled lease, reuses it for nesting and session transactions, then closes", async () => {
  ...
  await assert.rejects(
    () => leaked!.execute(sql`SELECT closed`),
    (error: unknown) => error instanceof DatabaseScopeError && error.code === "BRAID_SESSION_CLOSED",
  );
  await assert.rejects(
    () => prepared!.execute(1),
    (error: unknown) => error instanceof DatabaseScopeError && error.code === "BRAID_SESSION_CLOSED",
  ...
  await assert.rejects(
    () => inert![Symbol.asyncIterator]().next(),
    (error: unknown) => error instanceof DatabaseScopeError && error.code === "BRAID_SESSION_CLOSED",
  );
});
...
test("prepared names are scoped to each live database handle", async () => {
  ...
  const shapeLocked = root.prepare("shape-locked", () => (alternateShape ? sql.rows`SELECT 2` : sql.rows`SELECT 1`), {
    input: "none",
  });
...
test("outer pooled root use is rejected from a session callback", async () => {
  ...
  await assert.rejects(
    db.session(async () => db.execute(sql`SELECT escaped`)),
    (error: unknown) => error instanceof DatabaseScopeError && error.code === "BRAID_SESSION_SCOPE",
  );
});
...
test("prepared input queries cover row, command, and call kinds without construction-time invocation", async () => {
  ...
  const command = db.prepare(
    "command",
    (input: { readonly id: number }) => sql.command`UPDATE users SET id = ${input.id}`,
  );
...
test("prepared invocation modes keep input/options distinct for every query kind", async () => {
  ...
  const rest = db.prepare("rest-input", (...[input]: [{ readonly value: number }]) => sql.rows`SELECT ${input.value}`, {
    input: "required",
  });
  const defaulted = db.prepare("default-input", (input = "default") => sql.rows`SELECT ${input}`, {
    input: "required",
  });
  const wrappedFactory = ((input: { readonly value: string }) => sql.rows`SELECT ${input.value}`) as (
    ...args: [{ readonly value: string }]
  ) => ReturnType<typeof sql.rows>;
  ...
  assert.deepEqual(schemaCalls, [{ value: "ordinary" }, { value: 1 }, { value: "provided" }, { value: "wrapped" }]);
...
test("prepared execution options preflight before I/O without guessing application input fields", async () => {
  ...
  const command = db.prepare(
    "aborted-command",
    (input: { readonly signal: string }) => sql.command`UPDATE users SET name = ${input.signal}`,
  );
  ...
  const call = db.prepare(
    "aborted-call",
    (input: { readonly signal: string }) => sql.call`CALL routine(${input.signal})`,
  );
...
test("transaction options are forwarded, nested options reject, and malformed options fail before I/O", async () => {
  ...
  await nestedDb.tx(async (tx) => {
    await assert.rejects(
      ...
      (error: unknown) => error instanceof DatabaseScopeError && error.code === "BRAID_TX_OPTIONS_NESTED",
    );
  });
  const malformed = createDatabase(rowsExecutor(log));
  await assert.rejects(
    () => malformed.tx({ isolation: "invalid" } as never, async () => undefined),
    (error: unknown) => {
...
test("active cancellation preflights before acquisition while pre-aborted returns its exact reason", async () => {
  ...
  controller.abort(reason);
  await assert.rejects(
    () => db.execute(sql`SELECT aborted`, { signal: controller.signal }),
    (error: unknown) => error === reason,
  );
  assert.equal(acquires, 0);
  ...
  await assert.rejects(
    () => db.execute(sql`SELECT unsupported`, { signal: active.signal }),
    (error: unknown) => {
      return (
...
test("session capability rejection happens before pool acquisition", async () => {
  ...
  const provider: ConnectionProvider = {
    ...
    environment: {
      ...environment(),
      capabilities: { ...environment().capabilities, "session.pinned": { status: "unsupported" as const } },
    },
    async acquire() {
      acquires += 1;
      return { ...rowsExecutor([]), release() {} };
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      return (
        ...
        error.code === "BRAID_SESSION_UNSUPPORTED"
      );
    },
  );
  assert.equal(acquires, 0);
...
test("a pending stream is tracked before observers and cannot acquire after session closure", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      return (
        (error instanceof DatabaseScopeError && error.code === "BRAID_STREAM_SCOPE") ||
        (error instanceof AggregateError &&
          error.errors.some((item) => item instanceof DatabaseScopeError && item.code === "BRAID_STREAM_SCOPE"))
      );
    },
  );
  await assert.rejects(
