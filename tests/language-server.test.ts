...
test("completion is metadata-backed and reload replaces metadata", () => {
  ...
  assert.deepEqual(
    withMetadata.complete(relationSource, "relation.ts", relationSource.indexOf("us`") + 2).map((item) => item.label),
    ["users"],
  );
  assert.deepEqual(
    withMetadata.complete(columnSource, "column.ts", columnSource.indexOf("id FROM") + 2).map((item) => item.label),
    ["id"],
  );
  assert.equal(
    withMetadata.complete(columnSource, "column.ts", columnSource.indexOf("id FROM") + 2)[0]?.detail,
    "int8",
  );
  const reloaded = service.reload(metadata);
  assert.deepEqual(
    reloaded.complete(relationSource, "relation.ts", relationSource.indexOf("us`") + 2).map((item) => item.label),
    ["users"],
  );
  assert.deepEqual(service.complete(relationSource, "relation.ts", relationSource.indexOf("us`") + 2), []);
  const replacement = { ...metadata, relations: {} } satisfies MetadataSnapshot;
  assert.deepEqual(
    reloaded.reload(replacement).complete(relationSource, "relation.ts", relationSource.indexOf("us`") + 2),
    [],
  );
});
...
const mapped = sql.rows(UserSchema)\`SELECT id FROM users\`;`;
  const service = createLanguageService({ moduleSpecifier: "@sqlbraid/template" });
  assert.match(
    service.hover(mappedSource, "mapped-hover.ts", mappedSource.indexOf("SELECT"))?.contents ?? "",
    /RowQuery<UserRow>/u,
  );
});
...
test("canonical facade hover uses catalog dialects and MariaDB comment boundaries", () => {
  ...
  for (const [moduleSpecifier, dialect] of facades) {
    ...
    assert.match(
      service.hover(source, `${dialect}.ts`, source.indexOf("SELECT"))?.contents ?? "",
      new RegExp(`^RowQuery<unknown>\\ndialect: ${dialect}$`, "mu"),
    );
  }
...
test("workspace discovery follows aliased facade re-exports through the project checker", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.deepEqual(
        service.documentSymbols(source, sourcePath).map((symbol) => symbol.name),
        ["sql.rows"],
      );
      ...
      assert.deepEqual(
        editedService.documentSymbols(edited, sourcePath).map((symbol) => symbol.name),
        ["sql.rows"],
      );
      assert.match(
        editedService.hover(edited, sourcePath, edited.indexOf("SELECT"))?.contents ?? "",
        /dialect: sqlite/u,
      );
    } finally {
      workspace.dispose();
...
test("facade-only metadata defaults cover every navigation surface", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        service
          .complete(source.replace("main.users", "main."), sourcePath, source.indexOf("main.users") + "main.".length)
          .map((item) => item.label)
          .join(","),
        "users",
      );
      ...
      assert.equal(
        service.workspaceSymbols("users").some((symbol) => symbol.name === "users"),
        true,
      );
