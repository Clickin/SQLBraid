...
test("query tags expose declared result kinds", () => {
  ...
  const call: CallQuery<CallResult> = sql.call<CallResult>`CALL refresh_users()`;
  const untyped: Query<unknown, "unknown"> = sql`SELECT 1`;
  const captured: RowQuery<UserRow> = capture<UserRow, "rows">(sql.rows, ["SELECT ", ""], (values) => {
    values[0] = 1;
  });
...
test("schema-bound rows compose with capture and guarded without losing the mapper", () => {
  ...
  const mappedRows = sql.rows(schema);
  const captured = capture(mappedRows, ["SELECT ", ""], (values) => {
    values[0] = 1;
  });
...
test("sql.out is legal only for rows and calls, while sql.inOut remains call-only", () => {
  ...
  assert.throws(
    () => sql.rows`UPDATE users SET name = ${sql.inOut("name", "Ada")}`.render(),
    hasCode("BRAID_CALL_ONLY"),
  );
});
