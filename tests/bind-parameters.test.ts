...
test("bound wrappers are rejected as directive conditions in normal and captured paths", () => {
  ...
  assert.throws(
    () => sql`SELECT 1 /*@braid if ${condition}*/ AND 1 = 1 /*@braid end*/`.render(),
    hasCode("BRAID_BIND_HINT_CONTEXT"),
  );
  assert.throws(
    () => guarded(sql, ["SELECT 1 /*@braid if ", "*/ AND 1 = 1 /*@braid end*/"], [() => condition]),
    hasCode("BRAID_BIND_HINT_CONTEXT"),
  );
  const captured = capture(sql, ["SELECT 1 /*@braid if ", "*/ AND 1 = 1 /*@braid end*/"], (values) => {
    values[0] = condition;
  });
...
test("guarded capture remains lazy for inactive bind branches", () => {
  ...
  const query = guarded(
    sql,
    ["SELECT 1 /*@braid if ", "*/ AND id = ", " /*@braid end*/"],
    [
      () => false,
      () => {
        evaluated += 1;
        return sql.bind(1, integerHint);
      },
    ],
  );
