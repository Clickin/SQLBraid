...
test("template scanner preserves marker text in SQL lexical regions", () => {
  assert.equal(
    postgres`SELECT '/*@braid where*/ x /*@braid end*/' AS marker`.render().segments.join(""),
    "SELECT '/*@braid where*/ x /*@braid end*/' AS marker",
  );
  assert.equal(
    postgres`SELECT $$ /*@braid where*/ $$ AS marker`.render().segments.join(""),
    "SELECT $$ /*@braid where*/ $$ AS marker",
  );
  assert.equal(
    postgres`SELECT '\u00000\u0000' AS marker`.render().segments.join(""),
    "SELECT '\u00000\u0000' AS marker",
  );
  assert.equal(postgres`-- /*@braid end*/\nSELECT 1`.render().segments.join(""), "-- /*@braid end*/\nSELECT 1");
  const parsed = discoverQueries(
    "import {sql} from '@sqlbraid/template'; const q=sql`SELECT '${name}'`;",
    "fixture.ts",
...
test("trim rejects empty SET, omits comment-only WHERE, and preserves line comment LF", () => {
  ...
  const line =
    postgres`SELECT id FROM users /*@braid where*/ AND id = ${1} -- condition\n/*@braid end*/ ORDER BY id`.render();
  assert.match(postgresSql(line), /-- condition\n\s*ORDER BY id/);
  assert.equal(
    postgres`SELECT/*@braid if ${false}*/ DISTINCT /*@braid end*/id FROM users`.render().segments.join(""),
...
test("structural inputs are captured and bounded", () => {
  ...
  assert.throws(
    () => createSqlTag({ limits: { maxSqlBytes: 20 } })`SELECT '가나다라마'`.render(),
    hasCode("BRAID_SQL_LIMIT"),
  );
  const limited = createSqlTag({ limits: { maxStructuralItems: 1 } });
  assert.throws(
    () => limited`SELECT ${limited.join([limited.fragment`1`, limited.fragment`2`], limited.fragment`, `)}`.render(),
    hasCode("BRAID_STRUCTURE_LIMIT"),
  );
});
...
test("compiler discovers symbols, checks downstream row types, and lowers guarded evaluation", () => {
  ...
  const discovered = discoverQueries(
    "import {sql} from '@sqlbraid/template'; function f(sql) { return sql`bad`; } import * as braid from '@sqlbraid/template'; const q=braid.sql`SELECT id FROM users`;",
    "fixture.ts",
    options,
  );
  assert.equal(discovered.queries.length, 1);
  const source =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; type UserRow = { id: number; name: string }; declare const user: {name:string}|null; const q=sql.rows<UserRow>`SELECT custom_company_function(id) AS id, name FROM vendor_table /*@braid where*/ /*@braid if ${user != null}*/ AND name = ${user.name} /*@braid end*/ /*@braid end*/`; async function f(){ const rows=await db.all(q); return rows[0].missing; }";
  ...
  const commandSource =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; const q=sql`UPDATE users SET name = 'Ada'`; db.all(q);";
  assert.ok(
    checkSource(commandSource, join(tmpdir(), "sqlbraid-command.ts"), options).some(
      (diagnostic) => diagnostic.code === "TS2345",
    ),
  );
  const contractSource =
    "import {sql} from '@sqlbraid/template'; type UserRow = {id:string}; const q=sql.rows<UserRow>`SELECT opaque_vendor_function(id) AS id FROM vendor_table`;";
  assert.equal(
    checkSource(contractSource, join(tmpdir(), "sqlbraid-contract.ts"), {
      moduleSpecifier: "@sqlbraid/template",
      compilerOptions,
    }).length,
    0,
  ...
  assert.deepEqual(
    checkSource(
      source.replace("return rows[0].missing;", "return rows[0].id;"),
      join(tmpdir(), "sqlbraid-consumer.ts"),
      options,
    ),
    [],
  );
...
test("PV1 query contracts and explicit kinds are checked by TypeScript", () => {
  const options = { moduleSpecifier: "@sqlbraid/template", compilerOptions };
  const typed =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; type UserRow = {id:number}; const query = sql.rows<UserRow>`SELECT custom_company_function(id) AS id FROM vendor_table`; async function read(){ const rows = await db.all(query); return rows[0].missing; }";
  ...
  const untyped =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; const query = sql`SELECT 1`; db.all(query);";
  assert.ok(
    checkSource(untyped, join(tmpdir(), "sqlbraid-pv1-untyped.ts"), options).some(
      (diagnostic) => diagnostic.code === "TS2345",
    ),
  );

  const command =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; const query = sql.command`UPDATE users SET active = ${true}`; db.all(query); db.execute(query);";
  assert.ok(
    checkSource(command, join(tmpdir(), "sqlbraid-pv1-command.ts"), options).some(
      (diagnostic) => diagnostic.code === "TS2345",
    ),
  );

  const rows =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db: Database; type UserRow = {id:number}; const query = sql.rows<UserRow>`SELECT custom_company_function(id) AS id FROM vendor_table`; db.all(query);";
  assert.equal(checkSource(rows, join(tmpdir(), "sqlbraid-pv1-rows.ts"), options).length, 0);

  const call =
    "import {sql} from '@sqlbraid/template'; import type {Database} from '@sqlbraid/core'; declare const db:Database; const query = sql.call`CALL vendor_procedure()`; db.call(query);";
...
test("emitted guarded JavaScript evaluates only the active branch", async () => {
  const source =
    "import {sql} from '@sqlbraid/template'; export function build(user, sideEffect){ return sql`SELECT id FROM users /*@braid where*/ /*@braid if ${user != null}*/ AND name = ${user.name} AND value = ${sideEffect()} /*@braid end*/ /*@braid end*/`; }";
  ...
  try {
    ...
    assert.deepEqual(
      active.parameters.map(({ value }: { readonly value: unknown }) => value),
      ["Ada", 7],
    );
    assert.deepEqual(
      module
        ...
        .render(),
      module.build({ name: "Ada" }, () => 8).render(),
    );
  } finally {
...
test("guarded bind checking uses real control-flow narrowing", () => {
  const safe =
    "import {sql} from '@sqlbraid/template'; declare const user: {name:string}|null; const q=sql`SELECT id /*@braid if ${user != null}*/ AND name=${user.name} /*@braid end*/`;";
  assert.equal(
    checkSource(safe, join(tmpdir(), "sqlbraid-guard-safe.ts"), {
      moduleSpecifier: "@sqlbraid/template",
      compilerOptions,
    }).some((diagnostic) => diagnostic.code === "TS18047"),
    false,
  );
});
...
test("unguarded nullable binds report diagnostics at their original source range", () => {
  const unsafe =
    "import {sql} from '@sqlbraid/template'; declare const enabled: boolean; declare const user: {name:string}|null; const q=sql`SELECT id /*@braid if ${enabled}*/ AND name=${user.name} /*@braid end*/`;";
  const diagnostics = checkSource(unsafe, join(tmpdir(), "sqlbraid-guard-unsafe.ts"), {
    moduleSpecifier: "@sqlbraid/template",
    compilerOptions,
  });
  ...
  assert.ok(
    diagnostics.some(
      (diagnostic) =>
        diagnostic.code === "TS18047" &&
        diagnostic.range.start === start &&
        diagnostic.range.end === start + "user.name".length,
    ),
  );
});
...
test("project checking keeps one TypeScript program for re-exported tags and imported contracts", () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      "export type ImportedRow = { readonly id: number; name?: string | null }\n",
    );
    writeFileSync(
      join(directory, "main.ts"),
      "import {sql} from './bar.js'; import type {ImportedRow} from './types.js'; declare const user: {id:number}|null; export const imported=sql.rows<ImportedRow>`SELECT proprietary_extension(id) FROM users /*@braid if ${user != null}*/ WHERE id=${user.id} /*@braid end*/`;\n",
    );
    writeFileSync(
    ...
    assert.equal(
      diagnostics.some((diagnostic) => diagnostic.code.startsWith("TS") || diagnostic.code.startsWith("BRAID_")),
      false,
    );
    writeFileSync(
      join(directory, "wrong.ts"),
      "import {imported} from './main.js'; import type {Database} from '@sqlbraid/core'; declare const db:Database; async function read(){ return (await db.all(imported))[0].missing; }\n",
    );
    const negative = checkProject(projectFile, { moduleSpecifier: "@sqlbraid/template" });
    assert.deepEqual(
      negative.map((diagnostic) => diagnostic.code),
      ["TS2339"],
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("project checking preserves TSX, MTS, and CTS source identities", () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      "import {sql} from '@sqlbraid/template'; export function View() { const q=sql`SELECT 1`; return <div>{String(q)}</div>; }\n",
    );
    writeFileSync(
      join(directory, "module.mts"),
      "import {sql} from '@sqlbraid/template'; export const q=sql`SELECT 1`;\n",
    );
    writeFileSync(
      join(directory, "module.cts"),
      "import {sql} from '@sqlbraid/template'; export const q=sql`SELECT 1`;\n",
    );
    const projectFile = join(directory, "tsconfig.json");
    writeFileSync(
    ...
    const diagnostics = checkProject(projectFile, { moduleSpecifier: "@sqlbraid/template" });
    assert.equal(
      diagnostics.some((diagnostic) => diagnostic.code.startsWith("TS")),
      false,
    );
  } finally {
    rmSync(directory, { recursive: true, force: true });
...
test("untyped dynamic SQL remains unknown without a declared contract", () => {
  const choose =
    "import {sql} from '@sqlbraid/template'; const q=sql`/*@braid choose*/ /*@braid when ${true}*/ SELECT id FROM users /*@braid when ${true}*/ SELECT missing FROM definitely_missing /*@braid otherwise*/ SELECT id FROM users /*@braid end*/`;";
  const chooseOverlay = createVirtualOverlay(choose, join(tmpdir(), "sqlbraid-dynamic-choose.ts"), {
    moduleSpecifier: "@sqlbraid/template",
  });
  assert.equal(chooseOverlay.queryTypes[0]?.rowType, "unknown");
  assert.equal(chooseOverlay.queryTypes[0]?.resultKind, "unknown");
  assert.deepEqual(
    checkSource(choose, join(tmpdir(), "sqlbraid-dynamic-choose.ts"), {
      moduleSpecifier: "@sqlbraid/template",
      compilerOptions,
    }),
    [],
...
test("emits directive prologues, preserves compiler options, and maps generated JS to original TS", () => {
  ...
  const emitted = emitSource(source, "sqlbraid-source-map.ts", {
    moduleSpecifier: "@sqlbraid/template",
    compilerOptions: {
  ...
  const commonJs = emitSource(
    'import {sql} from "@sqlbraid/template"; export const f = (value: number) => value + 1; export const q=sql`SELECT ${1}`;',
    "sqlbraid-options.ts",
    {
      moduleSpecifier: "@sqlbraid/template",
      compilerOptions: { target: ts.ScriptTarget.ES5, module: ts.ModuleKind.CommonJS, sourceMap: false },
    },
  );
...
test("AST lowering is hygienic and preserves side effects, this, choose order, and multiple queries", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      new module.Builder({ name: "Ada" })
        .build()
        .render()
        .parameters.map(({ value }: { readonly value: unknown }) => value),
      ["Ada"],
    );
    let firstCalls = 0;
    let secondCalls = 0;
...
test("nested async functions and otherwise-only choose emit valid JavaScript", async () => {
  const source =
    "import {sql} from '@sqlbraid/template'; export function nested(){ return sql`SELECT 1 /*@braid if ${true}*/ AND fn=${async () => await Promise.resolve(1)} /*@braid end*/`; } export function otherwise(value){ return sql`SELECT 1 /*@braid choose*/ /*@braid otherwise*/ AND id=${value} /*@braid end*/`; }";
...
test("root execution waits until the transaction scope closes", async () => {
  ...
  const transaction = db.tx(async (tx) => {
    await tx.execute(postgres`SELECT 'inside'`);
    await gate;
  });
...
test("SQLite inspector records strict and dynamic table evidence", async () => {
  ...
  try {
    native.exec(
      "CREATE TABLE ordinary(id INTEGER, payload TEXT); CREATE TABLE strict_table(id INTEGER) STRICT; CREATE TABLE rowid_table(id INTEGER PRIMARY KEY, payload TEXT); CREATE TABLE desc_table(id INTEGER PRIMARY KEY DESC, payload TEXT); CREATE TABLE composite(a INTEGER, b INTEGER, PRIMARY KEY (a, b)); CREATE TABLE no_rowid(id INTEGER PRIMARY KEY, payload TEXT) WITHOUT ROWID;",
    );
...
test("SQLite inspector keeps main table flags separate from same-name temp tables", async () => {
  ...
  try {
    native.exec(
      "CREATE TABLE shadow(id INTEGER PRIMARY KEY) STRICT, WITHOUT ROWID; CREATE TEMP TABLE shadow(id INTEGER PRIMARY KEY);",
    );
...
test("MySQL inspector rejects MariaDB as a different product", async () => {
  const connection = {
    async execute(_sql: string) {
      return [[{ version: "10.11.0-MariaDB", product: "MariaDB" }], []] as const;
    },
    ...
    [
      [
        ...
        },
        {
          ...
          generation_expression: "`external_id` + 1",
        },
      ],
      [],
    ],
...
test("MySQL and MariaDB routine dictionaries accept prototype-named identifiers", async () => {
  const routineRows = [
    {
      routine_schema: "app",
      routine_name: "constructor",
      routine_type: "FUNCTION",
      data_type: "text",
      dtd_identifier: "constructor_1",
      is_deterministic: "NO",
      sql_data_access: "CONTAINS SQL",
    },
    {
      routine_schema: "app",
      routine_name: "toString",
      routine_type: "FUNCTION",
      data_type: "text",
      dtd_identifier: "toString_1",
      is_deterministic: "NO",
      sql_data_access: "CONTAINS SQL",
    },
    {
      routine_schema: "app",
      routine_name: "__proto__",
      routine_type: "FUNCTION",
      data_type: "text",
      dtd_identifier: "__proto___1",
      is_deterministic: "NO",
      sql_data_access: "CONTAINS SQL",
    },
  ] as const;
  ...
  const mysql = await createMysqlInspector({
    async execute() {
      return mysqlResponses[mysqlIndex++];
    },
    async beginTransaction() {},
    async commit() {},
    async rollback() {},
  ...
  const maria = await createMariaDbInspector({
    async execute() {
      return mariaResponses[mariaIndex++];
    },
    async beginTransaction() {},
    async commit() {},
    async rollback() {},
    ...
    },
    {
      rows: [
        ...
        },
        {
          ...
          table_name: "c",
          ordinal_position: "2",
          column_name: "constructor_value",
          data_type: "constructor",
          udt_schema: "pg_catalog",
          udt_name: "constructor",
          is_nullable: "YES",
          is_identity: "NO",
          is_generated: "NEVER",
        },
...
test("prepared queries reject shape drift and streams honor adapter capability", async () => {
  ...
  const db = createDatabase({
    ...
    async query<Row>(rendered: RenderedStatement) {
      return { kind: "rows" as const, rows: [{ text: postgresSql(rendered) }] as unknown as readonly Row[] };
    },
    async *stream<Row>(rendered: RenderedStatement): AsyncIterable<Row> {
      yield { text: postgresSql(rendered) } as unknown as Row;
    },
    async call() {
