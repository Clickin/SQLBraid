...
test("CLI version matches the package manifest", async () => {
  const manifest = JSON.parse(await readFile(resolve(process.cwd(), "packages/cli/package.json"), "utf8")) as {
    version: string;
  };
...
test("CLI manifest follows a local facade re-export through the project checker", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      entries.map((entry) => entry.resultKind),
      ["unknown"],
    );
  } finally {
...
test("CLI checks, manifests, and builds opaque declared queries without a snapshot", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      entries.map((entry) => entry.resultKind),
      ["rows", "command", "call", "unknown"],
    );
    assert.deepEqual(
      entries.map((entry) => entry.resultType),
      ["Row", 'import("@sqlbraid/core").CommandResult', "CallResult", undefined],
    );
    assert.ok(
      entries.every(
        (entry) =>
    ...
    assert.ok(
      entries.every(
        (entry) =>
          ...
          typeof entry.templateFamilyFingerprint === "string" &&
    ...
    assert.deepEqual(
      inactive.map((query: { resultKind: string }) => query.resultKind),
      ["rows", "command", "call", "unknown"],
    );
    const active = build({ id: 7 });
    assert.deepEqual(
      active[0].render().parameters.map(({ value }: { readonly value: unknown }) => value),
      [7],
    );
    ...
    await assert.rejects(
      exec(process.execPath, [
        ...
        "--input-type=module",
        "--eval",
        `import { build } from ${JSON.stringify(pathToFileURL(output).href)}; build({ get id() { throw new Error('source-map-probe'); } });`,
      ]),
...
test("CLI codegen resolves config-relative paths, preserves unchanged mtimes, and checks freshness", async () => {
  ...
  try {
    ...
    await writeFile(
      join(configDirectory, "sqlbraid.config.mjs"),
      [
    ...
    const first = await exec(
      process.execPath,
      [cliEntry, "codegen", "--config", join(configDirectory, "sqlbraid.config.mjs"), "--json"],
      { cwd: process.cwd() },
    );
    ...
    await assert.rejects(
      exec(
        process.execPath,
        [cliEntry, "codegen", "--config", join(configDirectory, "sqlbraid.config.mjs"), "--check", "--json"],
        { cwd: process.cwd() },
      ),
      (error: unknown) =>
...
test("CLI codegen retains unavailable exact numeric evidence across the config worker", async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      `export default { codegen: { targets: [{
    ...
    assert.ok(
      reports[0].diagnostics.some(
        (diagnostic: { code: string }) => diagnostic.code === "CODEGEN_NUMERIC_FIDELITY_UNAVAILABLE",
      ),
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
...
test("CLI codegen discovers configs, selects repeated targets, and blocks error writes", async () => {
  ...
  try {
    ...
    const selected = await exec(
      process.execPath,
      [cliEntry, "codegen", "--target", "other", "--target", "main", "--json"],
      { cwd: directory },
    );
    assert.deepEqual(
      JSON.parse(selected.stdout).map((entry: { target: string }) => entry.target),
      ["main", "other"],
    );
    await assert.rejects(
...
test("CLI codegen rejects case-folded output collisions on simulated Windows", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        return (
          ...
          entries.every((entry) =>
            entry.diagnostics.some((diagnostic) => diagnostic.code === "CODEGEN_OUTPUT_PATH_COLLISION"),
          )
        );
      },
    );
    ...
    await assert.rejects(readFile(join(directory, "generated.ts")));
    assert.equal(
      codegenOutputCollisionKey("/tmp/Generated.ts", "win32"),
      codegenOutputCollisionKey("/tmp/generated.ts", "win32"),
    );
    assert.notEqual(
      codegenOutputCollisionKey("/tmp/Generated.ts", "darwin"),
      codegenOutputCollisionKey("/tmp/generated.ts", "darwin"),
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
...
test("CLI inspect JSON discovers an ancestor SQLBraid config past nested projects", async () => {
  ...
  try {
    ...
    const inspected = await exec(
      process.execPath,
      [cliEntry, "inspect", "query", "--file", file, "--line", "1", "--column", String(column), "--json"],
      { cwd: directory },
    );
    ...
    const diagnostics = await exec(process.execPath, [cliEntry, "inspect", "diagnostics", "--file", file, "--json"], {
      cwd: directory,
    });
...
test("CLI inspect JSON consumes a facade query with metadata-backed defaults", async () => {
  ...
  try {
    ...
    const inspected = await exec(
      process.execPath,
      [cliEntry, "inspect", "query", "--file", file, "--line", "1", "--column", String(relationColumn), "--json"],
      { cwd: directory },
    );
    ...
    const symbols = await exec(process.execPath, [cliEntry, "inspect", "symbol", "users", "--json"], {
      cwd: directory,
    });
    const symbolResult = JSON.parse(symbols.stdout) as {
      operation: string;
      symbols: readonly { name: string; kind: string }[];
    };
    assert.equal(symbolResult.operation, "symbol");
    assert.deepEqual(
      symbolResult.symbols.map((symbol) => [symbol.name, symbol.kind]),
      [["users", "relation"]],
    );
...
test("programmatic inspector snapshots feed the CLI codegen recipe", async () => {
  ...
  try {
    ...
    const first = await exec(process.execPath, [cliEntry, "codegen", "--config", configPath, "--json"], {
      cwd: directory,
    });
    assert.equal(JSON.parse(first.stdout)[0].status, "written");
    ...
    const second = await exec(process.execPath, [cliEntry, "codegen", "--config", configPath, "--check", "--json"], {
      cwd: directory,
    });
    assert.equal(JSON.parse(second.stdout)[0].status, "unchanged");
  } finally {
...
test("CLI facade prebuild executes lazy branch interpolation from generated output", async () => {
  ...
  try {
    ...
    const { run } = (await import(pathToFileURL(output).href)) as {
      run: (include: boolean) => { calls: number; parameters: unknown[] };
    };
