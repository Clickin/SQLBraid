...
const generation = {
  ...
  models: [
    {
      relationIdentity: "public.users",
      modelName: "Users",
      rowName: "UsersRow",
      insertName: "UsersInsert",
      updateName: "UsersUpdate",
    },
  ],
  diagnostics: [],
...
test("completion is scoped to static SQL and does not pollute TypeScript", () => {
  ...
  assert.deepEqual(
    service.complete(sql, "sql.ts", sqlOffset).map((item) => item.label),
    ["users"],
  );
  const qualified = sql.replace("FROM us", "FROM public.");
  assert.deepEqual(
    service
      .complete(qualified, "qualified.ts", qualified.indexOf("public.") + "public.".length)
      .map((item) => item.label),
    ["users"],
  );
  assert.deepEqual(
    service.complete(interpolation, "interpolation.ts", interpolation.indexOf("ordinary.") + "ordinary.".length),
    [],
  );
...
test("open-world SQL remains legal and incomplete routine evidence is explicit", () => {
  ...
  assert.doesNotMatch(
    service.hover(qualifiedElsewhere, "routine-other.ts", qualifiedElsewhere.indexOf("calculate_fee"))?.contents ?? "",
    /^Routine /u,
  );
  assert.equal(
    service.signatureHelp(
      routineSource,
      "routine.ts",
      routineSource.indexOf("calculate_fee(") + "calculate_fee(".length,
    ),
    undefined,
  );
  assert.equal(
    service
      .complete(routineSource, "routine.ts", routineSource.indexOf("calculate_fee") + 3)
      .some((item) => item.label === "calculate_fee"),
    true,
  );
});
...
test.each([
  ...
  "metadata omissions leave %s built-ins, extensions, UDFs, temp objects and CTEs opaque",
  (dialect, sqlText) => {
...
test("generated current output wins navigation and stale output falls back to metadata JSON", () => {
  ...
  assert.deepEqual(
    current.workspaceSymbols("UsersRow").map((symbol) => symbol.name),
    ["UsersRow"],
  );
...
test("identifier folding is deterministic while quoted identifiers preserve case", () => {
  ...
  const service = createLanguageService({
    targets: [{ name: "db", metadata: snapshot, metadataPath: "metadata.json", metadataSource: json }],
  });
  ...
  assert.equal(
    service.hover(source, "folding.ts", source.indexOf("PUBLIC.I"))?.contents.startsWith("Relation public.i"),
    true,
  );
  const quoted = source.replace("PUBLIC.I", '"I"');
  assert.doesNotMatch(
    service.hover(quoted, "quoted-case.ts", quoted.indexOf('"I"'))?.contents ?? "",
    /^Relation public.i/u,
  );
});
...
test("quoted qualified names preserve dotted metadata segments for navigation", () => {
    ...
    targets: [
      {
        name: "db",
        metadata: snapshot,
        metadataPath: "quoted-metadata.json",
        metadataSource: JSON.stringify(snapshot, null, 2),
      },
    ],
  });
  ...
  assert.equal(
    service.hover(other, "quoted-qualified-other.ts", otherOffset)?.contents.split("\n")[0],
    "Relation a.b\\.c",
  );
});
...
test("JSON navigation uses decoded identities rather than relation map keys or folded names", () => {
  ...
  const start =
    lines.slice(0, column.range.start.line).reduce((sum, line) => sum + line.length + 1, 0) +
    column.range.start.character;
  const end =
    lines.slice(0, column.range.end.line).reduce((sum, line) => sum + line.length + 1, 0) + column.range.end.character;
...
test("query symbols and complete routine signature evidence stay bounded", () => {
  ...
  const completeMetadata = {
    ...metadata,
    routines: { "public.calculate_fee": [completeRoutine] },
  } as const satisfies MetadataSnapshot;
  const source =
    "import { sql } from '@sqlbraid/postgres';\nconst query = sql.rows<{}>`SELECT public.calculate_fee(id) FROM public.users`;";
  ...
  const signature = service.signatureHelp(
    source,
    "symbols.ts",
    source.indexOf("calculate_fee(") + "calculate_fee(".length,
  );
