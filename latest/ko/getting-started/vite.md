...
직접 compiler 통합이 필요하면 `@sqlbraid/vite`가 `transformSource(source, filename, options?)`를 다시 내보냅니다. 주변 TypeScript 변환은 다른 bundler가 소유할 때만 사용하세요. [동적 템플릿](/SQLBraid/latest/concepts/dynamic-braid.md), [SQL 태그](/SQLBraid/latest/concepts/sql-tags.md), [Vite package README](https://github.com/Clickin/SQLBraid/tree/main/packages/vite)도 참고하세요.
