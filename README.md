...
> **Status:** pre-release. SQL-first authoring, Standard Schema result mapping, connection leasing/transaction pinning, execution observers, runtime portability and optional metadata/codegen tooling are implemented. Next: codegen CLI and overrides. See [`PLAN.md`](./PLAN.md).
...
lookup or codec execution. It returns standalone TypeScript source, model names,
diagnostics, the metadata hash and TypePolicy id/hash. Map insertion order and
capture timestamps do not affect the generated bytes.
...
Invalid metadata throws `SnapshotValidationError`; malformed TypePolicy input
throws `TypeError`. Conflicting normalized policy entries produce
`CODEGEN_AMBIGUOUS_TYPE_MAPPING` error diagnostics and affected types remain
`unknown`. Inspect diagnostics before using the source. An unresolved model-name
collision throws rather than emitting duplicate declarations.

PV9 is this programmatic core only. PV10 will add CLI/config, filters, naming
and type overrides; no future CLI syntax is supported yet. Arbitrary SELECT/JOIN
inference, runtime validation and input codecs remain outside codegen.
