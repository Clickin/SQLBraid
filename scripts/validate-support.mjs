...
export async function validateSupport({ root = scriptRoot } = {}) {
  ...
  if (!validateProfiles || !validateProfiles(profileRegistry)) {
    fail("SUPPORT_PROFILE_SCHEMA", validateProfiles ? ajv.errorsText(validateProfiles.errors, { separator: "\n" }) : "Missing profile registry schema.");
  }
  let provenance;
  try { provenance = await validateFirstPartyPolicies({ root }); }
  catch (error) { fail(error.code ?? "SUPPORT_TYPE_POLICY", error.message); }
  ...
  for (const [id, profile] of profiles) {
    if (id !== profile.id) fail("SUPPORT_PROFILE_MISMATCH", `Profile registry key ${id} disagrees with descriptor ${profile.id}.`);
    const sourceRecord = sourcePolicies.get(profile.typePolicy.id);
    const sourcePolicy = sourceRecord?.policy;
    if (!sourcePolicy || sourcePolicy.hash !== profile.typePolicy.hash) fail("SUPPORT_TYPE_POLICY_MISMATCH", `${id}: TypePolicy ${profile.typePolicy.id} is not the committed source policy.`);
    if (sourceRecord.profile) {
      const sourceProfile = sourceRecord.profile;
      const requiredOptions = sourceProfile.connectionOptions ?? { parserProfile: { json: sourceProfile.json, temporal: sourceProfile.temporal } };
      if (sourceProfile.id !== id || sourceProfile.json !== profile.json || sourceProfile.temporal !== profile.temporal || !same(requiredOptions, profile.requiredOptions)) {
        fail("SUPPORT_PROFILE_MISMATCH", `${id}: representation options disagree with the exported source descriptor.`);
      }
    }
    if (profile.fixtureTestIds.length === 0) fail("SUPPORT_PROFILE_FIXTURE", `${id}: representation profile has no real fixture.`);
    for (const fixtureId of profile.fixtureTestIds) {
      if (!Object.hasOwn(registry, fixtureId)) fail("SUPPORT_PROFILE_FIXTURE", `${id}: fixture ${fixtureId} is not registered.`);
      if (!registry[fixtureId].file.startsWith("tests/db/")) fail("SUPPORT_PROFILE_FIXTURE", `${id}: fixture ${fixtureId} must exercise a real database.`);
      const fixture = profileRegistry.fixtures?.[fixtureId];
      if (!fixture || fixture.profile !== id) fail("SUPPORT_PROFILE_FIXTURE", `${id}: fixture ${fixtureId} has no matching profile metadata.`);
      for (const [kind, representation] of Object.entries(fixture.driverRawRepresentations ?? {})) {
        if (profile.driverRawRepresentations[kind] !== representation) fail("SUPPORT_REPRESENTATION_MISMATCH", `${id}: fixture ${fixtureId} raw ${kind} disagrees with profile.`);
      }
      for (const [kind, representation] of Object.entries(fixture.sqlbraidRepresentations ?? {})) {
        if (profile.sqlbraidRepresentations[kind] !== representation) fail("SUPPORT_REPRESENTATION_MISMATCH", `${id}: fixture ${fixtureId} canonical ${kind} disagrees with profile.`);
      }
    }
  }
  ...
  for (const target of targets) {
    ...
    if (profile.driverId !== target.driver.id) fail("SUPPORT_PROFILE_MISMATCH", `${target.id}: profile driver ${profile.driverId} disagrees with ${target.driver.id}.`);
    if (!same(target.driver.requiredOptions, profile.requiredOptions)) fail("SUPPORT_PROFILE_MISMATCH", `${target.id}: required driver options disagree with ${target.driver.profile}.`);
    if (!same(target.driver.driverRawRepresentations, profile.driverRawRepresentations)) fail("SUPPORT_REPRESENTATION_MISMATCH", `${target.id}: driver raw representation disagrees with ${target.driver.profile}.`);
    if (!same(target.driver.sqlbraidRepresentations, profile.sqlbraidRepresentations)) fail("SUPPORT_REPRESENTATION_MISMATCH", `${target.id}: SQLBraid representation disagrees with ${target.driver.profile}.`);
    if (!same(target.typePolicy, profile.typePolicy)) fail("SUPPORT_TYPE_POLICY_MISMATCH", `${target.id}: TypePolicy does not match ${target.driver.profile}.`);
    for (const [id, claim] of Object.entries(target.capabilities)) {
      if (claim.status === "guaranteed" && claim.conditionCode) fail("SUPPORT_CAPABILITY_ALTERNATE_PROFILE", `${target.id}: guaranteed ${id} cannot reference alternate profile condition ${claim.conditionCode}.`);
      for (const testId of claim.testIds) {
        const fixture = profileRegistry.fixtures?.[testId];
        if (fixture && fixture.profile !== profile.id) fail("SUPPORT_PROFILE_FIXTURE", `${target.id}: ${testId} belongs to ${fixture.profile}, not ${profile.id}.`);
      }
      const raw = claim.driverRawRepresentations ?? [];
      const mapKey = numericCapabilities.get(id) === "exact-integer" ? "integer" : numericCapabilities.get(id) === "exact-decimal" ? "decimal" : id.includes("json") ? "json" : id.includes("temporal") ? "temporal" : undefined;
      if (claim.status === "guaranteed" && mapKey && raw.length && raw.some((value) => !profile.driverRawRepresentations[mapKey].toLocaleLowerCase().includes(value.toLocaleLowerCase()))) {
        fail("SUPPORT_REPRESENTATION_MISMATCH", `${target.id}: ${id} raw evidence disagrees with ${target.driver.profile}.`);
      }
    }
    ...
    for (const [id, claim] of Object.entries(target.capabilities)) {
      ...
      if (kind) {
        ...
        if (kind !== "approximate-binary" && claim.fidelity === "lossless" && claim.driverRawRepresentations?.some((value) => /(?:^|[\s/,(])number(?:$|[\s/),])/iu.test(value))) {
