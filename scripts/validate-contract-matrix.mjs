...
const cellKey = (transport, scenario, layer, ownership) =>
  `${transport}:${scenario}:${layer}:${ownership}`;
function requireThat(condition, message) {
  if (!condition) throw new Error(`CONTRACT_MATRIX: ${message}`);
}
function sameKeys(actual, expected, label) {
  requireThat(
    Array.isArray(actual) && new Set(actual).size === actual.length &&
...
export function validateMatrix({ catalog, matrix, targets, profiles, registry, capabilityIds }) {
  ...
  for (const scenario of scenarios.values()) {
    ...
    for (const requirement of scenario.requires) {
      ...
      requireThat(extra === undefined && key && ["capability", "native"].includes(kind), `${scenario.id}: malformed requirement`);
  ...
  for (const target of targets) {
    requireThat(profiles.profiles[target.driver.profile]?.driverId === target.driver.id,
  ...
  for (const [transport, entry] of Object.entries(matrix.transports ?? {})) {
    ...
    requireThat(!releaseBlocking || target.ci.releaseBlocking === true, `${transport}: nonblocking canonical target hides a blocking tuple`);
    ...
    for (const [name, fact] of Object.entries(entry.native)) {
      requireThat(typeof fact.supported === "boolean" && typeof fact.reason === "string" && fact.reason.trim().length > 0 &&
        typeof fact.source === "string" && fact.source.startsWith("packages/"), `${transport}: unchecked native semantic ${name}`);
    }
    const pool = entry.native.pool.supported;
    requireThat(Array.isArray(entry.ownership) && entry.ownership.length > 0 &&
      new Set(entry.ownership).size === entry.ownership.length &&
      entry.ownership.every((owner) => owner === "direct" || owner === "pooled") &&
      entry.ownership.includes("pooled") === pool, `${transport}: contradictory ownership classification`);
    // Bun network clients enter through a reserve provider; the public factory has no direct path.
    const expectedOwnership = target.driver.id === "bun-sql" && pool ? ["pooled"] : pool ? ["direct", "pooled"] : ["direct"];
    ...
    for (const [capability, fact] of Object.entries(entry.capabilityEvidence ?? {})) {
      requireThat(capabilityIds.includes(capability) && statuses.includes(fact.status) && fact.reason?.trim() && fact.source,
        `${transport}: invalid supplemental capability ${capability}`);
      requireThat(target.capabilities[capability] === undefined,
    ...
    for (const claim of Object.values(target.capabilities)) {
      for (const testId of claim.testIds ?? []) requireThat(registry[testId], `${target.id}: unknown support test ${testId}`);
    }
    ...
    for (const scenario of scenarios.values()) {
      ...
      for (const requirement of scenario.requires) {
        ...
        if (kind === "capability" && siblings.some((sibling) =>
          ["guaranteed", "guarded"].includes(sibling.capabilities[capability]?.status))) {
          requireThat(supported(entry, target, requirement),
      ...
      if (missing.length > 0) {
        ...
        requireThat(Object.keys(mapping).length === 1 && exclusion && missing.includes(exclusion.requirement) &&
      ...
      for (const layer of scenario.evidence) {
        for (const ownership of owners) {
          cells.push({ transport, scenario: scenario.id, layer, ownership, target: target.id,
            profile: target.driver.profile, releaseBlocking, key: cellKey(transport, scenario.id, layer, ownership) });
        }
      }
    }
  }
...
export async function loadContractMatrix(directory = root) {
  ...
  const [catalog, matrix, profiles, registry, capabilities, targetFiles] = await Promise.all([
    json("tests/contracts/catalog.json"), json("tests/contracts/matrix.json"), json("support/profiles.json"),
    json("support/test-registry.json"), json("support/capabilities.json"), readdir(join(directory, "support/targets")),
  ]);
  const targets = await Promise.all(targetFiles.filter((file) => file.endsWith(".json")).sort().map((file) => json(`support/targets/${file}`)));
  const input = { catalog, matrix, targets, profiles, registry, capabilityIds: capabilities.capabilities.map((capability) => capability.id) };
  const model = validateMatrix(input);
  const sources = new Set(Object.values(matrix.transports).flatMap((entry) =>
...
export function reportEvidence(report, manifest, model, sourceSha) {
  ...
  requireThat(Array.isArray(manifest.allowedLayers) && manifest.allowedLayers.length > 0 &&
    new Set(manifest.allowedLayers).size === manifest.allowedLayers.length &&
    manifest.allowedLayers.every((layer) => layers.includes(layer)), "invalid report layer provenance");
  requireThat(typeof manifest.producer?.node === "string" && typeof manifest.producer?.platform === "string" &&
    typeof manifest.producer?.arch === "string", "missing observed producer runtime");
  requireThat(Array.isArray(manifest.expectedTargets) && manifest.expectedTargets.length > 0, "missing expected target bindings");
  ...
  requireThat(report?.success === true && Array.isArray(report.testResults) && report.testResults.length > 0,
    "report is not a successful Vitest JSON execution");
  requireThat(report.numFailedTests === 0 && report.numFailedTestSuites === 0 && report.wasInterrupted !== true,
  ...
  for (const suite of report.testResults) {
    ...
    for (const assertion of suite.assertionResults) {
      ...
      requireThat(["passed", "pending", "skipped", "todo", "disabled"].includes(assertion.status),
      ...
      for (const [, transport, scenario, layer] of matches) {
        ...
        requireThat(model.catalog.scenarios.some((item) => item.id === scenario), `unknown contract scenario ${scenario}`);
        requireThat(manifest.allowedLayers.includes(layer), `wrong provenance layer ${layer}: ${assertion.title}`);
        requireThat(entry.scenarios[scenario]?.evidence?.includes(layer), `N/A or wrong evidence layer ${transport}:${scenario}:${layer}`);
        ...
        if (layer === "integration" && canonical.runtime.id === "node") {
          requireThat(manifest.producer.node === `v${canonical.runtime.version}`,
            `${transport}: observed Node ${manifest.producer.node} does not match canonical ${canonical.runtime.version}`);
        }
        const scenarioCells = model.cells.filter((cell) => cell.transport === transport && cell.scenario === scenario && cell.layer === layer);
  ...
  requireThat(report.numTotalTests === assertionCount && report.numPassedTests === passedCount,
...
export async function validateReports(directory, sourceSha, model) {
  ...
  for (const file of manifests) {
    ...
    requireThat(manifest.report === basename(reportFile) && manifest.reportSha256 === digest(bytes),
  ...
  requireThat(missing.length === 0, `missing passed execution evidence (${missing.length} cells):\n${missing.map((cell) => cell.key).join("\n")}`);
  return { required: model.cells.filter((cell) => cell.releaseBlocking).length, passed: passed.size, reports: manifests.length };
}
...
export async function recordReport(file, sourceSha, layer, targetIds, model, directory = root) {
  ...
  const selected = targetIds === "all" ? Object.values(model.matrix.transports).map((entry) => entry.target) : targetIds?.split(",");
  requireThat(selected?.length > 0, "--targets must name the exact support targets executed (or all for shared boundary reports)");
  ...
  const manifest = {
    format: manifestFormat, version: 1, sourceSha, report: basename(file), reportSha256: digest(bytes),
...
async function main() {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    requireThat(["--reports", "--source-sha", "--record-report", "--layer", "--targets", "--list"].includes(name) && !options.has(name), `unknown or duplicate option ${name}`);
  ...
  if (options.has("--record-report")) {
    console.log(await recordReport(resolve(options.get("--record-report")), options.get("--source-sha"), options.get("--layer"), options.get("--targets"), model));
  } else if (options.has("--reports")) {
    console.log(JSON.stringify(await validateReports(resolve(options.get("--reports")), options.get("--source-sha"), model)));
  } else if (options.has("--list")) {
    console.log(JSON.stringify({ canonicalTargets: Object.fromEntries(Object.entries(model.matrix.transports).map(([id, entry]) => [id, entry.target])), cells: model.cells, notApplicable: model.exclusions }, null, 2));
  } else {
    requireThat(options.size === 0, "evidence options require --reports or --record-report");
    console.log(`Contract matrix valid: ${model.catalog.scenarios.length} scenarios, ${Object.keys(model.matrix.transports).length} transport/profiles, ${model.cells.filter((cell) => cell.releaseBlocking).length} required evidence cells. Execution evidence was not checked.`);
  }
}
