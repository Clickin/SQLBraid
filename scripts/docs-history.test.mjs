...
async function releaseFixture() {
  ...
  const commit = (
    await execFileAsync("git", ["-C", directory, "rev-parse", "HEAD"], { encoding: "utf8" })
  ).stdout.trim();
...
test("release archives require the exact tagged source commit", async () => {
  ...
  await assertReleaseSource({
    rootDirectory: source.directory,
    version: "1.2.3",
...
test("version routes preserve locale and fall back to the locale root", () => {
  ...
  assert.equal(
    routeForVersion({ version: "latest", locale: "root", page: "runtime/transactions", pages }),
    "/SQLBraid/latest/runtime/transactions/",
  );
  assert.equal(
    routeForVersion({ version: "latest", locale: "ko", page: "runtime/transactions", pages }),
    "/SQLBraid/latest/ko/",
  );
  assert.equal(
    routeForVersion({ version: "0.1.0", locale: "root", page: "runtime/transactions", pages }),
    "/SQLBraid/v/0.1.0/runtime/transactions/",
  );
  assert.equal(
    routeForVersion({ version: "0.1.0", locale: "ko", page: "runtime/transactions", pages }),
    "/SQLBraid/v/0.1.0/ko/",
  );
...
test("latest sync replaces only the mutable channel while staging keeps archives", async () => {
  ...
  try {
    ...
    assert.equal(
      /content="0;url=([^"]+)"/u.exec(await readFile(join(output, "index.html"), "utf8"))?.[1],
      "/SQLBraid/v/1.2.3/",
    );
  } finally {
    await rm(history, { recursive: true, force: true });
...
test("staging migrates the legacy development tree to latest", async () => {
  ...
  try {
    ...
    const staged = await stageDeployment({ historyDirectory: history, outputDirectory: output });
    assert.equal(
      await readFile(join(output, "latest", "index.html"), "utf8"),
      '<script src="/SQLBraid/latest/_astro/app.js"></script>',
    );
    assert.equal(
      await readFile(join(output, "latest", "_astro", "app.js"), "utf8"),
      'new Worker("/SQLBraid/latest/_astro/worker.js")',
    );
    assert.deepEqual(staged.pages.latest.root, [""]);
...
test("legacy links migrate both development and unversioned targets", async () => {
  ...
  try {
    await writeFile(
      ...
      '<a href="/SQLBraid/dev/getting-started/">dev</a><script src="/SQLBraid/dev/_astro/app.js"></script><a href="https://clickin.github.io/SQLBraid/reference/">root</a><a href="/SQLBraid/v/0.1.0/">release</a>\n',
    );
    ...
    await writeFile(
      join(output, "ko", "index.html"),
      '<link href="/SQLBraid/dev/_astro/site.css"><a href="/SQLBraid/dev/ko/concepts/">guide</a><a href="/SQLBraid/reference/">guide</a>',
    );
    await rewriteLegacyBaseLinks(output, "/SQLBraid/v/1.2.3");
