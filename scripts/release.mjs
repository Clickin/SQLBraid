...
function candidateIdentityDigest(manifest) {
  return createHash("sha256")
    .update(JSON.stringify(candidateIdentity(manifest)))
    .digest("hex");
}

function commandErrorText(error) {
  return [error?.message, error?.code, error?.stdout, error?.stderr]
    .filter((value) => typeof value === "string")
    .join("\n");
}

function registryNotFound(error) {
  return /\b(?:E404|ERR_PNPM_FETCH_404|ERR_PNPM_PACKAGE_NOT_FOUND)\b|404 Not Found|No matching version found|No match found/iu.test(
...
function workspaceDependencyNames(manifest) {
  return packageFields.flatMap((field) =>
  ...
  for (const entry of packages) {
    for (const dependency of workspaceDependencyNames(entry.manifest)) {
      if (!byName.has(dependency))
        throw new Error(`${entry.manifest.name} references missing workspace package ${dependency}.`);
...
async function currentSha() {
  ...
  if (process.env.GITHUB_SHA && process.env.GITHUB_SHA !== head)
    throw new Error(`Workflow SHA ${process.env.GITHUB_SHA} does not equal checked out HEAD ${head}.`);
  return head;
}
...
async function assertTaggedSha() {
  ...
  if (ref !== `refs/tags/${expectedTag}`)
    throw new Error(`Final release must run from tag ${expectedTag}; found ${ref || "no tag"}.`);
  const head = await currentSha();
  if (process.env.GITHUB_ACTIONS === "true" && !process.env.GITHUB_SHA)
    throw new Error("GitHub release publication requires GITHUB_SHA.");
  if (process.env.GITHUB_SHA && process.env.GITHUB_SHA !== head)
    throw new Error(`Workflow SHA ${process.env.GITHUB_SHA} does not equal checked out HEAD ${head}.`);
  ...
  if (previous && !/^0+$/u.test(previous) && previous !== head) {
    throw new Error(
      `Candidate tag ${expectedTag} moved from ${previous} to ${head}; create a new release candidate version instead of reusing the tag.`,
    );
  }
  return head;
}
...
async function pack(packages, order, sha) {
    ...
    .filter(
      (entry) =>
        ...
        (entry.name.endsWith(".tgz") ||
          ...
          entry.name === "pack-check-success.json"),
    )
  ...
  const manifest = {
    ...
    packages: order.map((name) => ({
      ...
      integrity: null,
      dependencies: [
        ...new Set(workspaceDependencyNames(packages.find((entry) => entry.manifest.name === name).manifest)),
      ].sort(),
    })),
  };
...
async function readReleaseManifest(
  ...
  { priorCandidateRunId, allowCurrentAttemptMismatch = false } = {},
) {
  ...
  if (priorCandidateRunId !== undefined) {
    if (
      !/^\d+$/u.test(String(priorCandidateRunId)) ||
      !/^\d+$/u.test(manifest.runId ?? "") ||
      !/^\d+$/u.test(manifest.runAttempt ?? "")
    ) {
    ...
    if (process.env.GITHUB_RUN_ID && manifest.runId !== process.env.GITHUB_RUN_ID)
      throw new Error(
        `Validated release artifacts belong to run ${manifest.runId ?? "unknown"}, not ${process.env.GITHUB_RUN_ID}.`,
      );
    if (
      !allowCurrentAttemptMismatch &&
      process.env.GITHUB_RUN_ATTEMPT &&
      manifest.runAttempt !== process.env.GITHUB_RUN_ATTEMPT
    )
      throw new Error(
        `Validated release artifacts belong to attempt ${manifest.runAttempt ?? "unknown"}, not ${process.env.GITHUB_RUN_ATTEMPT}.`,
      );
  }
  const files = new Set();
  ...
  for (const entry of manifest.packages) {
    ...
    const dependencies = [
      ...new Set(
        packageFields
          .flatMap((field) => Object.keys(packedManifest[field] ?? {}))
  ...
  const manifestPackages = manifest.packages
    ...
    .sort()
    .join("\n");
  if (stampPackages !== manifestPackages)
    throw new Error("Pack-check stamp does not match validated release tarball hashes.");
  return manifest;
}
...
async function assertOfficialRegistry() {
  const configured = (await pnpm(["config", "get", "registry"], root, { registryArg: false, quiet: true }))
    .trim()
    .replace(/\/?$/u, "/");
...
async function registryIntegrity(name, releaseVersion) {
  ...
  if (!value && (await pnpmView(spec, "version")))
    throw new Error(`Registry integrity is missing for existing ${spec}.`);
...
function assertStageId(id) {
  if (typeof id !== "string" || !/^[a-f\d]{8}(?:-[a-f\d]{4}){3}-[a-f\d]{12}$/iu.test(id))
    throw new Error("Missing or invalid npm stage ID.");
  return id;
}
...
async function createReleaseEvidence(
  manifest,
  staged,
  { directory = artifactDir, supportEvidencePath, targetEvidenceDirectory, stagedEvidencePath } = {},
) {
  assertManifestIdentity(manifest);
  ...
  if (
    support.format !== "sqlbraid-support-evidence" ||
    support.version !== 1 ||
    support.commit !== manifest.commit ||
    !Array.isArray(support.targets)
  ) {
    throw new Error("Support evidence does not match the release candidate.");
  }
  const supportTargetIds = [
    ...new Set(support.targets.map((entry) => entry?.id).filter((id) => typeof id === "string")),
  ].sort();
  ...
  for (const file of targetFiles) {
    ...
    if (
      target.format !== "sqlbraid-support-evidence" ||
      target.version !== 1 ||
...
function approvalCommands(manifest, records) {
  ...
  return layers.flatMap((ids, index) =>
    ids?.length ? [{ layer: index + 1, command: `pnpm stage approve ${ids.join(" ")} --registry ${registry}` }] : [],
  );
}
...
async function stagingSummary(evidence) {
  ...
  const lines = [
    "## npm staging — human approval required",
    "",
    evidence.complete
  ...
  for (const entry of evidence.packages)
    lines.push(
      `| ${entry.name} | ${entry.version} | ${entry.stageId ?? entry.state} (${entry.state}) | ${entry.tag} | ${entry.candidateSha256} |`,
    );
...
function assertLatestUnchanged(before, after, name) {
  if ((before.latest ?? undefined) !== (after.latest ?? undefined)) {
    throw new Error(
      `Registry latest tag changed unexpectedly for ${name}: ${before.latest ?? "absent"} -> ${after.latest ?? "absent"}.`,
    );
  }
}
...
async function stageCandidates(
  ...
  if (dryRun) {
    for (const entry of manifest.packages) {
      ...
      if (
        ...
        summary.integrity !== entry.integrity ||
        summary.stageId
      )
        throw new Error("Staged dry-run did not report the exact candidate integrity without a stage ID.");
    }
    return;
  }
  ...
  try {
    existingEvidence = await json(join(directory, "staged-publication.json"));
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
  }
  if (existingEvidence && priorEvidence)
    throw new Error("Provide either existing staged evidence or explicit prior evidence, not both.");
  if (existingEvidence) {
...
function assertStagingEvidence(
  manifest,
  evidence,
  { allowPriorIdentity = false, expectedRunId, expectedRunAttempt } = {},
) {
  ...
  if (
    evidence?.format !== "sqlbraid-staged-publication" ||
    (!manifestMatches && !(allowPriorIdentity && candidateMatches)) ||
    ...
    (!allowPriorIdentity &&
      (evidence.runId !== (expectedRunId ?? manifest.runId) ||
  ...
  if (
    evidence.candidateRunId !== undefined &&
    (evidence.candidateRunId !== manifest.runId || evidence.candidateRunAttempt !== manifest.runAttempt)
  ) {
  ...
  for (const [index, entry] of manifest.packages.entries()) {
    ...
    if (
      ...
      throw new Error("Invalid staging package evidence.");
    if (record.stageId) assertStageId(record.stageId);
  }
  if (
    evidence.mode === "reconcile" &&
    (!evidence.reconciledFrom || !/^[a-f\d]{64}$/u.test(evidence.reconciledFrom.manifestSha256 ?? ""))
  ) {
...
function assertPublicationCredentials(mode, env = process.env) {
  ...
  if (!env.ACTIONS_ID_TOKEN_REQUEST_URL || !env.ACTIONS_ID_TOKEN_REQUEST_TOKEN)
    throw new Error("Normal staging requires GitHub Actions id-token permissions.");
}
