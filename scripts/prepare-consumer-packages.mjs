...
if (supplied) {
  ...
  if (artifacts.length < packageNames.length) throw new Error(`Expected at least ${packageNames.length} supplied package artifacts in ${output}; found ${artifacts.length}.`);
  console.info(`Reusing ${artifacts.length} supplied package artifacts from ${output}`);
} else {
  await rm(output, { recursive: true, force: true });
  await mkdir(output, { recursive: true });
  for (const packageName of packageNames) {
    execFileSync("pnpm", ["--dir", join(packageRoot, packageName), "pack", "--pack-destination", output], { cwd: root, stdio: "inherit" });
  }
