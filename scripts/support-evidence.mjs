...
assert.ok(
  ...
  "Usage: node scripts/support-evidence.mjs <test-results.json> <evidence.json> [stamps-directory]",
);
const results = JSON.parse(await readFile(resultsPath, "utf8"));
const commit = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (process.env.GITHUB_SHA)
  assert.equal(commit, process.env.GITHUB_SHA, "Evidence must describe the checked-out CI SHA.");
...
if (results.format === "sqlbraid-runtime-tests") {
  ...
  for (const id of results.testIds) {
    ...
    assert.ok(
      results.observations.some(
        (entry) =>
          entry.testIds?.includes(id) &&
          entry.runtime.id === results.runtime.id &&
          entry.runtime.version === results.runtime.version,
      ),
  ...
  for (const file of results.testResults) {
    for (const test of file.assertionResults) {
      ...
      for (const [id, entry] of Object.entries(registry)) {
        if (resolve(entry.file) === resolve(file.name) && (test.title === entry.title || test.fullName === entry.title))
          passed.set(id, test.fullName);
      }
    }
  }
}
...
for (const name of (await readdir("support/targets")).sort()) {
  ...
  if (
    target.database.product === "postgres" &&
    target.driver.id === "pg" &&
    target.runtime.id === "node" &&
    name !== `${process.env.SQLBRAID_POSTGRES_TARGET ?? "postgres"}.json`
  )
    continue;
  const observed = observations.filter((entry) => entry.targetId === name.slice(0, -5) || entry.targetId === target.id);
  const exactTupleObserved = observed.some(
    (entry) =>
...
const evidence = {
  ...
  runtime:
    results.format === "sqlbraid-runtime-tests"
      ? results.runtime
      : {
          id: "node",
...
console.log(
  `Recorded ${targets.length} target test sets at ${commit}; certification additionally requires matching observed tuples and complete gates.`,
);
