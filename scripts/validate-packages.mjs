...
try {
  ...
  await writeFile(entry, [
    ...
    'if (oracle`SELECT ${1}`.render().text !== "SELECT :1") throw new Error("packed oracle root failed");',
    'if (mssql`SELECT ${1}`.render().text !== "SELECT @p1") throw new Error("packed mssql root failed");',
