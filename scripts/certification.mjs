...
const nodeProjects = new Map([
  ["postgres-pg-node-16-4", "cert-pg"], ["postgres-current", "cert-pg"],
  ["mysql-mysql2-node-8-4-2", "cert-mysql"], ["mariadb-connector-node-11-8-9", "cert-mariadb"],
  ["oracle-oracledb-thin-node-23-9", "cert-oracle"], ["mssql-tedious-developer-node-2022-cu18", "cert-mssql"],
]);
...
const bunTargets = new Map([["bun-sql-postgres", "postgres"], ["bun-sql-mysql", "mysql"], ["bun-sql-mariadb", "mariadb"], ["bun-sql-sqlite", "sqlite"]]);
...
function stress() {
  ...
  if (value !== undefined && !["0", "1", "false", "true"].includes(value)) throw new Error("SQLBRAID_CERT_STRESS must be 0, 1, false, or true.");
...
async function distSha() {
  ...
  for (const entry of packages) {
    ...
    try { await hashTree(directory, join(entry.name, "dist"), files); }
...
async function preparedIntegrity() {
  ...
  if (observedArchiveSha !== identity.preparedBuildSha256) {
    throw new Error(`Prepared build checksum mismatch: expected ${identity.preparedBuildSha256}, received ${observedArchiveSha}.`);
  }
  return { archiveSha256: observedArchiveSha, distSha256: await distSha() };
}
async function verifyPrepared() {
  const sha = sourceSha();
  const directory = resolve(option("--candidate-dir", process.env.SQLBRAID_CERT_PREPARED_DIR ?? join(process.env.RUNNER_TEMP ?? "/tmp", "prepared")));
  const identity = JSON.parse(await readFile(join(directory, "candidate.json"), "utf8"));
  if (identity.sourceSha !== sha || typeof identity.preparedBuildSha256 !== "string") throw new Error("Prepared candidate identity does not match the certification source SHA.");
  const archiveSha256 = await sha256(join(directory, "prepared-build.tar.gz"));
  if (archiveSha256 !== identity.preparedBuildSha256) throw new Error(`Prepared build checksum mismatch: expected ${identity.preparedBuildSha256}, received ${archiveSha256}.`);
  const observedDistSha256 = await distSha();
  const expectedDistSha256 = option("--expected-dist-sha");
  if (expectedDistSha256 !== undefined && expectedDistSha256 !== observedDistSha256) throw new Error(`Prepared dist checksum mismatch: expected ${expectedDistSha256}, received ${observedDistSha256}.`);
  const output = { sourceSha: sha, archiveSha256, distSha256: observedDistSha256 };
  const outputPath = option("--output");
  if (outputPath) { await mkdir(dirname(outputPath), { recursive: true }); await writeFile(outputPath, `${JSON.stringify(output, null, 2)}\n`); }
  console.log(JSON.stringify(output));
}
async function tarManifest(path) {
  return new Promise((resolveManifest, reject) => {
    const child = spawn("tar", ["-xOf", path, "sqlbraid-release-artifacts/release-manifest.json"], { cwd: root, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = ""; let stderr = "";
    child.stdout.setEncoding("utf8"); child.stderr.setEncoding("utf8");
    child.stdout.on("data", (chunk) => { stdout += chunk; }); child.stderr.on("data", (chunk) => { stderr += chunk; });
    child.once("error", reject);
    child.once("exit", (code) => code === 0 ? resolveManifest(JSON.parse(stdout)) : reject(new Error(`Cannot read release manifest from ${path}: ${stderr}`)));
  });
}
async function verifyCandidate() {
  const sha = sourceSha();
  const directory = resolve(option("--candidate-dir", process.env.SQLBRAID_CERT_CANDIDATE_DIR ?? join(process.env.RUNNER_TEMP ?? "/tmp", "prepared")));
  const candidateArchive = join(directory, "release-candidate.tar.gz");
  const preparedBuild = join(directory, "prepared-build.tar.gz");
  const [candidateSha, preparedSha, manifest] = await Promise.all([sha256(candidateArchive), sha256(preparedBuild), tarManifest(candidateArchive)]);
  if (manifest.commit !== sha) throw new Error(`Release candidate manifest commit ${manifest.commit} does not match ${sha}.`);
  ...
  const expectedCandidatePath = expectedCandidateValue === undefined || expectedCandidateValue === "" ? undefined : expectedCandidateValue;
  const runAttempt = process.env.SQLBRAID_CERT_EXPECTED_RUN_ATTEMPT ?? (
  ...
  if (runId !== undefined && String(manifest.runId) !== runId) throw new Error("Release candidate producer run does not match the current run.");
  if (runAttempt !== undefined && String(manifest.runAttempt) !== runAttempt) throw new Error("Release candidate producer attempt does not match the current run.");
  if (runId !== undefined && process.env.GITHUB_RUN_ID !== undefined && runId !== process.env.GITHUB_RUN_ID && expectedCandidatePath === undefined) {
  ...
  try {
    ...
    for (const entry of manifest.packages) {
      ...
      if (packageDirectory === undefined) throw new Error(`Release candidate package ${entry.name} has no workspace package.`);
      ...
      if (await directorySha(preparedDist) !== await directorySha(packedDist)) throw new Error(`Release candidate ${entry.name} dist bytes do not match the prepared build.`);
    }
  } finally {
    await rm(extracted, { recursive: true, force: true });
  }
  const identity = { kind: "release-prepared", sourceSha: sha, producerRunId: String(manifest.runId ?? runId ?? ""), producerRunAttempt: String(manifest.runAttempt ?? runAttempt ?? ""), artifactName: "release-prepared", preparedBuildSha256: preparedSha, releaseCandidateSha256: candidateSha };
  if (expectedCandidatePath !== undefined) {
    const expectedIdentity = JSON.parse(await readFile(resolve(expectedCandidatePath), "utf8"));
    if (!equalNormalized(expectedIdentity, identity)) throw new Error("Release candidate producer identity does not match its validated artifact manifest and digests.");
  }
  const output = option("--output");
  if (output) { await mkdir(dirname(output), { recursive: true }); await writeFile(output, `${JSON.stringify(identity, null, 2)}\n`); }
...
async function aggregate() {
  ...
  const artifacts = await Promise.all((await files(input)).map(async (path) => JSON.parse(await readFile(path, "utf8"))));
  if (artifacts.length !== requiredTargets.length) throw new Error(`Expected ${requiredTargets.length} certification artifacts, found ${artifacts.length}.`);
  await mkdir(dirname(output), { recursive: true });
  await run("pnpm", ["exec", "vitest", "run", "--config", "vitest.certification.config.ts", "--project", "cert-aggregate"], {
...
async function attachCandidate(directory) {
  ...
  for (const path of await files(directory)) {
    ...
    if (artifact.sourceSha !== identity.sourceSha) throw new Error(`Certification artifact ${artifact.target} has a candidate source SHA mismatch.`);
...
async function certifyTarget(target) {
  ...
  const artifactDir = resolve(option("--artifact-dir", process.env.SQLBRAID_CERT_ARTIFACT_DIR ?? join(process.env.RUNNER_TEMP ?? "/tmp", "sqlbraid-certification")));
  await mkdir(artifactDir, { recursive: true });
  const commonEnv = { SQLBRAID_CERT_SOURCE_SHA: sha, SQLBRAID_CERT_STRESS: stress() ? "1" : "0", SQLBRAID_CERT_ARTIFACT_DIR: artifactDir };
  if (nodeProjects.has(target)) {
    const setupEnv = target === "postgres-pg-node-16-4" ? { SQLBRAID_POSTGRES_TARGET: "postgres" }
      : target === "postgres-current" ? { SQLBRAID_POSTGRES_TARGET: "postgres-current" }
        : {};
    await run("pnpm", ["exec", "vitest", "run", "--config", "vitest.certification.config.ts", "--project", nodeProjects.get(target)], { ...commonEnv, ...setupEnv, SQLBRAID_CERT_TARGET: target, SQLBRAID_CERT_ARTIFACT: join(artifactDir, `${target}.json`) });
  ...
  if (["sqlite-node-sqlite-node-22-18-0", "better-sqlite3-node-22-18-0", "libsql-local-node-22-18-0"].includes(target)) {
    const existing = new Set((await files(artifactDir)).map((path) => path.split("/").pop()));
    if (sqliteGroupComplete && existing.has(`${target}.json`)) {
      await attachCandidate(artifactDir);
      return;
    }
    await run("pnpm", ["exec", "vitest", "run", "--config", "vitest.certification.config.ts", "--project", "cert-sqlite"], { ...commonEnv, SQLBRAID_CERT_ARTIFACT: artifactDir });
  ...
  if (bunTargets.has(target)) {
    await run("bun", ["scripts/bun-sql-certification.mjs", "--source-sha", sha, "--artifact", join(artifactDir, `${target}.json`), bunTargets.get(target)], commonEnv);
    await attachCandidate(artifactDir);
    const preparedAfter = await preparedIntegrity();
    if (preparedBefore !== undefined && preparedBefore.distSha256 !== preparedAfter?.distSha256) throw new Error("Prepared build dist changed during certification.");
    return;
  ...
  if (target === "d1-cloudflare-workerd-2026-07-30") {
    await run("pnpm", ["run", "test:d1"], { ...commonEnv, SQLBRAID_CERT_ARTIFACT: join(artifactDir, `${target}.json`) });
    await attachCandidate(artifactDir);
    const preparedAfter = await preparedIntegrity();
    if (preparedBefore !== undefined && preparedBefore.distSha256 !== preparedAfter?.distSha256) throw new Error("Prepared build dist changed during certification.");
    return;
  ...
  if (target === "sqlite-wasm-browser-3-53-4") {
    await run("pnpm", ["run", "test:browser"], { ...commonEnv, SQLBRAID_CERT_ARTIFACT: join(artifactDir, `${target}.json`) });
    await attachCandidate(artifactDir);
    const preparedAfter = await preparedIntegrity();
    if (preparedBefore !== undefined && preparedBefore.distSha256 !== preparedAfter?.distSha256) throw new Error("Prepared build dist changed during certification.");
    return;
...
async function validateArtifact() {
  ...
  await run("pnpm", ["exec", "vitest", "run", "--config", "vitest.certification.config.ts", "--project", "cert-validate"], {
    SQLBRAID_CERT_SOURCE_SHA: sha,
